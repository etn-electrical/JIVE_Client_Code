...
#define I2C_MASTER_TOUT_CNUM_DEFAULT   ( 8 )         /* I2C master timeout cycle number of I2C clock, after which the
	                                                    timeout interrupt will be triggered */
...
static esp_err_t i2c_master_read_slave( i2c_port_t i2c_num, uint8_t*data_rd, size_t size )
{
	...
	uC_I2C::return_type error = uC_I2C::NO_ERROR;

	i2c_cmd_handle_t cmd = i2c_cmd_link_create();

	i2c_control->Read_Data( ESP_SLAVE_ADDR, size, data_rd, nullptr, cmd );
	if ( error == uC_I2C::NO_ERROR )
	{
		return ( ESP_OK );
	}
	else
...
static esp_err_t i2c_master_write_slave( i2c_port_t i2c_num, uint8_t*data_wr, size_t size )
{
	...
	i2c_control->Write_Data( ESP_SLAVE_ADDR, size, data_wr, nullptr, cmd );
	if ( error == uC_I2C::NO_ERROR )
	{
		return ( ESP_OK );
	}
...
static esp_err_t i2c_master_sensor_test( i2c_port_t i2c_num, uint8_t*data_h, uint8_t*data_l )
{
	...
	i2c_master_read_byte( cmd, data_l, ( i2c_ack_type_t )NACK_VAL );
	i2c_master_stop( cmd );
	ret = i2c_master_cmd_begin( i2c_num, cmd, 1000 / portTICK_RATE_MS );
	i2c_cmd_link_delete( cmd );
	return ( ret );
...
{
	uint8_t channel = I2C_NUM_0;

	i2c_control = new I2C_Manager( channel, ESP_SLAVE_ADDR, uC_I2C::I2C_STANDARD_MODE,
								   uC_I2C::ADDRESS_7_BIT, uC_I2C::FASTMODE_DUTYCYCLE_2, nullptr );
...
static esp_err_t i2c_slave_init()
{
	...
	return ( i2c_driver_install( ( i2c_port_t )i2c_slave_port, conf_slave.mode,
...
static void i2c_test_task( void*arg )
{
	...
	while ( 1 )
	{
		...
		{
			ESP_LOGW( I2C_TAG, "%s: No ack, sensor not connected...skip...", esp_err_to_name( ret ) );
		}
		...
		if ( d_size == 0 )
		{
			ESP_LOGW( I2C_TAG, "i2c slave tx buffer full" );
		...
		{
			ESP_LOGW( I2C_TAG, "TASK[%d] %s: Master write slave error, IO not connected....\n",
					  task_idx, esp_err_to_name( ret ) );
		}
		xSemaphoreGive( print_mux );
		vTaskDelay( ( DELAY_TIME_BETWEEN_ITEMS_MS * ( task_idx + 1 ) ) / portTICK_RATE_MS );
