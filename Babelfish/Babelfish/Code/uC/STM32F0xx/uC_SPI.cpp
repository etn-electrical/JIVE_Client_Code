	...
	m_tx_dma( reinterpret_cast<uC_DMA*>( nullptr ) )
{
	...
	if ( ( m_config->tx_dma_channel != static_cast<uint8_t>( uC_DMA_CHANNEL_DNE ) ) &&
		 ( m_config->rx_dma_channel != static_cast<uint8_t>( uC_DMA_CHANNEL_DNE ) ) &&
		 ( enable_dma == true ) )
	{
...
void uC_SPI::Select_Chip( uint8_t chip_select )
{
	...
	temp_cr1 = static_cast<uint16_t>( m_spi->CR1 & static_cast<uint32_t>( SPI_CR1_SPE ) );	// Push the
																							// SPI
																							// enable
																							// register
																							// so we
