...
class uC_Flash : public NV_Ctrl
{
	...
	private:
		...
		bool Write_Now( uint8_t* data, uint32_t recovery_mem_address, uint32_t dest_mem_address,
		...
		bool Read_Now( uint8_t* data, NV_CTRL_ADDRESS_TD mem_address,
					   NV_CTRL_LENGTH_TD length, bool use_protection ) const;

		bool Check_Data( uint8_t* check_data, NV_CTRL_ADDRESS_TD mem_address,
						 NV_CTRL_LENGTH_TD length, bool use_protection ) const;

		bool Check_Range( NV_CTRL_ADDRESS_TD mem_address, NV_CTRL_LENGTH_TD length, bool protection = true ) const;

		bool Erase_Page( uint32_t mem_address );
