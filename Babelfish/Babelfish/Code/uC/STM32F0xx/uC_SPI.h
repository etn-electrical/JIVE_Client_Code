...
class uC_SPI
{
	public:
		 ...
		 *                            full-duplex and simplex communication modes. This parameter
		 * specifies
		 *                            which SPI channel of controller we want to use for
		 * communication.
		 *                            Possible values are mentioned below :
		 * @n                         0 : Selects SPI channel 1
		 * @n                         1 : Selects SPI channel 2
		 * @n                         2 : Selects SPI channel 3
		 * @param[in] master          Specifies if SPI channel is to be used in master mode or slave
		 * mode
		 * @n                         TRUE : Master mode
		 * @n                         FALSE : Slave mode
		 * @param[in] chip_sel_ctrls  Chip Select pin specification. This structure variable shall
		 * specify
		 *                            the GPIO port and pin number which is used for selecting SPI
		 * slave
		 *                            device
		 * @param[in] total_chip_sel  The total number of SPI devices interfaced to the
		 * microcontroller.
		 * @param[in] io_ctrl         Structure pointer which contains information about the pins
		 * which are
		 *                            used for SPI communication. If NULL - it initializes the SPI
		 * pins for
		...
		/**
		 * @brief                   Destructor to delete the uc_SPI instance when it goes out of
		 * scope
		 *                          scope.
		 */
		~uC_SPI( void );
		 ...
		 * @n                                   uC_SPI_BIT_COUNT_8(8) : 8 bits in one frame. 8 bit
		 * transfer
		...
		/**
		 * @brief                   Fills in the SPI peripheral data register for transmission
		 * purpose.
		 ...
		 *                          The byte is received in SPI data register and this function
		 * return it.
		...
		/**
		 * @brief                   Return the word data received over from SPI slave device
		 *                          The word is received in SPI data register and this function return it.
		 *                          16 bit data reception in one frame.
		 * @return uint8_t          The word data received from SPI device
		 * @note                    Implemented as an inline function.
		 * @n
		 * @n
		 */
		inline uint16_t RX_Word( void )
		{
			volatile uint16_t dump;
		...
		{
			return ( ( m_spi->SR & SPI_SR_TXE ) != 0 );
		}

		/**
		 * @brief                   Check if SPI is not busy in communication and data can be
		 * shifted
		 *                          or transfered.
		 * @return bool             Two possible return values
		...
		/**
		 * @brief                    Selects the chip for SPI communication. Selecting the chip
		 * means
		 *                           pulling low(logic 0) the slave select signal(SS) of SPI device.
		 *                           When SS pin is pulled low means controller wants to transfer
		 * data
		 *                           to SPI device.
		 *
		 * @param[in] chip_select   The SPI device number which need to be selected for
		 * communication
		 * @note                    Implemented as an inline function.
		 * @n
		 * @n
		...
		/**
		 * @brief                    Deselect the chip to stop the SPI communication.Deselecting the
		 * chip
		 *                           pulling high(logic 1) the slave select signal(SS) of SPI
		 * device.
		 *
		 * @param[in] chip_select   The SPI device number whose communication need to be
		 * disconnected
		 * @note                    Implemented as an inline function.
		 * @n
		 * @n
		 ...
		 * @n                        Notifies DMA transfer related to SPI receive is finished or
		 * not.
		 ...
		 * @n                       Notifies DMA transfer related to SPI transmit is finished or
		 * not.
		 * @return bool             Possible values
		...
		/**
		 * @brief                   Configure the DMA for SPI exchange( transmit and receive)
		 * purpose.
		 *
		 * @param[in] tx_data       The address/pointer from which data will be picked for
		 * transmitting
		 *                          over an SPI
		 * @param[in] rx_data       The address/pointer at which data received over an is to be
		 * written.
		 * @param[in] length        The Number of data items/bytes to be transmitted/received over
		 * an SPI
		 * @note                    Implemented as an inline function.
		 * @n
		 * @n
		...
		// }
		// void			Disable_RX_DMA( void )					{ m_spi->SPI_PTCR =
		// AT91C_PDC_RXTDIS; }
		//
		// void			Set_TX_DMA( uint8_t* data, uint16_t length );
		// void			Enable_TX_DMA( void )					{ m_spi->SPI_PTCR = AT91C_PDC_TXTEN;
		// }
		// void			Disable_TX_DMA( void )					{ m_spi->SPI_PTCR =
		// AT91C_PDC_TXTDIS; }
		//
		// void			Enable_DMA( void )						{ m_spi->SPI_PTCR = AT91C_PDC_RXTEN
		// | AT91C_PDC_TXTEN; }
		// void			Disable_DMA( void )						{ m_spi->SPI_PTCR = AT91C_PDC_RXTDIS
		// | AT91C_PDC_TXTDIS; }
		//
	...
	//
	// void			Enable_Int( void )                      { uC_Interrupt::Enable_Int(
	// m_config->pid ); }
	// void			Disable_Int( void )                     { uC_Interrupt::Disable_Int(
	// m_config->pid ); }
	//
	...
	// }
	// inline BOOL		RX_Buff_End( void )						{ return ( ( m_spi->SPI_SR &
	// AT91C_SPI_ENDRX ) != 0 ); }
	//
	...
	// }
	// inline BOOL		TX_Buff_End( void )						{ return ( ( m_spi->SPI_SR &
	// AT91C_SPI_ENDTX ) != 0 ); }
	//
	// void			Enable_RX_Buff_Full_Int( void )         { m_spi->SPI_IER |=
	// AT91C_SPI_RXBUFF; }
	// void			Disable_RX_Buff_Full_Int( void )        { m_spi->SPI_IDR |=
	// AT91C_SPI_RXBUFF; }
	// inline BOOL		RX_Buff_Full( void )					{ return ( ( m_spi->SPI_SR &
	// AT91C_SPI_RXBUFF ) != 0 ); }
	//
	// void			Enable_TX_Buff_Empty_Int( void )        { m_spi->SPI_IER |=
	// AT91C_SPI_TXBUFE; }
	// void			Disable_TX_Buff_Empty_Int( void )       { m_spi->SPI_IDR |=
	// AT91C_SPI_TXBUFE; }
	// inline BOOL		TX_Buff_Empty( void )					{ return ( ( m_spi->SPI_SR &
	// AT91C_SPI_TXBUFE ) != 0 ); }
	//
	// void			Enable_TX_Shift_Empty_Int( void )       { m_spi->SPI_IER |=
	// AT91C_SPI_TXEMPTY; }
	// void			Disable_TX_Shift_Empty_Int( void )      { m_spi->SPI_IDR |=
	// AT91C_SPI_TXEMPTY; }
