	...
	m_port_io( reinterpret_cast<uC_PERIPH_IO_STRUCT const*>( nullptr ) )
{
	...
	if ( ( pwm_ctrl_block == static_cast<uint8_t>( uC_BASE_PWM_CTRL_1 ) ) ||
		 ( pwm_ctrl_block == static_cast<uint8_t>( uC_BASE_PWM_CTRL_15 ) ) ||
...
void uC_PWM::Turn_On( void )
{
	...
	{
		temp_val =
			static_cast<uint16_t>( *temp_ccmr &
...
void uC_PWM::Turn_Off( void )
{
	...
	else
	{
		temp_val =
			static_cast<uint16_t>( *temp_ccmr &
								   ( ~( static_cast<uint32_t>( TIM_CCMR1_OC1M ) <<
										MULT_BY_256 ) ) );
	}
 ...
 *****************************************************************************************
 */
bool uC_PWM::Is_On( void ) const
{
	uint32_t temp_ccmr;

	if ( m_pwm_ch < static_cast<uint8_t>( uC_BASE_TIMER_CAP_COMP_CH3 ) )
	{
		if ( m_pwm_ch == uC_BASE_TIMER_CAP_COMP_CH1 )
		{
			temp_ccmr = ( m_pwm_ctrl->reg_ctrl->CCMR1 & TIM_CCMR1_OC1M );
			return ( temp_ccmr == ( TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2 ) ? 1 : 0 );
		}
		else
		{
			temp_ccmr = ( m_pwm_ctrl->reg_ctrl->CCMR1 & TIM_CCMR1_OC2M );
			return ( temp_ccmr == ( TIM_CCMR1_OC2M_1 | TIM_CCMR1_OC2M_2 ) ? 1 : 0 );
		}
	}
	else
	{
		if ( m_pwm_ch == uC_BASE_TIMER_CAP_COMP_CH3 )
		{
			temp_ccmr = ( m_pwm_ctrl->reg_ctrl->CCMR2 & TIM_CCMR2_OC3M );
			return ( temp_ccmr == ( TIM_CCMR2_OC3M_1 | TIM_CCMR2_OC3M_2 ) ? 1 : 0 );
		}
		else
		{
			temp_ccmr = ( m_pwm_ctrl->reg_ctrl->CCMR2 & TIM_CCMR2_OC4M );
			return ( temp_ccmr == ( TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2 ) ? 1 : 0 );
		}
	}
}
						...
						uint32_t desired_freq, bool default_state )
{
	...
	if ( polarity == static_cast<uint8_t>( uC_PWM_DUTY_CYCLE_HIGH ) )
	{
		m_pwm_ctrl->reg_ctrl->CCER &=
			~( static_cast<uint16_t>( static_cast<uint32_t>( TIM_CCER_CC1P ) <<
									  ( static_cast<uint32_t>
										( m_pwm_ch ) << MULT_BY_8 ) ) );
	...
	{
		m_pwm_ctrl->reg_ctrl->CCER |=
			( static_cast<uint16_t>( TIM_CCER_CC1P ) <<
				( static_cast<uint32_t>( m_pwm_ch ) << MULT_BY_8 ) );
	...
	m_pwm_ctrl->reg_ctrl->CCER |=
		( static_cast<uint16_t>( TIM_CCER_CC1E ) << ( m_pwm_ch * 4U ) );
	...
	m_pwm_ctrl->reg_ctrl->CNT = static_cast<uint16_t>( count );
	m_pwm_ctrl->reg_ctrl->ARR = static_cast<uint16_t>( count );
	m_pwm_ctrl->reg_ctrl->PSC = static_cast<uint16_t>( prescaler );
...
void uC_PWM::Set_Duty_Cycle_Pct( uint16_t percent )
{
	...
	ticks_required =
		static_cast<uint16_t>( ( percent * m_percent_adj ) >> PWM_DUTY_CYCLE_PERCENT_SHIFT );				// PWM_DUTY_CYCLE_PERCENT_CALC;
