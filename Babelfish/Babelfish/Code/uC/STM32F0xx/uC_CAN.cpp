...
void uC_CAN::Set_CAN_Comm_Baud_Rate( CAN_BaudRate_Def CANBaudRate )
{
	...
	m_can_reg->BTR &=
		~( ( ( 0x03 ) << 24 ) | ( ( 0x07 ) << 20 ) | ( ( 0x0F ) << 16 ) | ( 0x1FF ) );

	/* Set the bit timing register */
	m_can_reg->BTR |=
		( UINT32 ) ( ( ( ( ( UINT32 )CANBitTimeParam[CANBaudRate].CAN_SJW - 1 ) & 0x03 )
					   << 24 )
					 | ( ( ( ( UINT32 )CANBitTimeParam[CANBaudRate].CAN_TSEG2 - 1 ) &
						   0x07 ) << 20 )
					 | ( ( ( ( UINT32 )CANBitTimeParam[CANBaudRate].CAN_TSEG1 - 1 ) &
						   0x0F ) << 16 )
					 | ( ( ( UINT32 )CANBitTimeParam[CANBaudRate].CAN_BPR - 1 ) &
						 0x1FF ) );
}
 ...
 *****************************************************************************************
 */
void uC_CAN::InitMailboxRegisters( UINT8 CAN_Filter_Index, UINT32 mesg_id, UINT8 format,
								   UINT8 isRTR )
{
	...
	m_can_reg->FA1R &= ~( UINT32 )filter_number_bit_pos;							// deactivate
																					// filter
																					// initialize
																					// filter
	// m_can_reg->FS1R &= ~(UINT32)filter_number_bit_pos;                         // set 16-bit scale
	// configuration
	m_can_reg->FS1R |= filter_number_bit_pos;						// set 32-bit scale configuration

	m_can_reg->FM1R |= ( UINT32 )filter_number_bit_pos;				// set 2 32-bit identifier list
																	// mode
	// m_can_reg->FM1R &= ~(UINT32)filter_number_bit_pos;                         // set 2 32-bit
	// identifier/mask mode

	/* 32-bit identifier or First 32-bit identifier */
	m_can_reg->sFilterRegister[CAN_Filter_Index].FR1 = ( UINT32 )CAN_Message_ID1;	// 32-bit
																					// identifier

	/* 32-bit mask or Second 32-bit identifier */
	m_can_reg->sFilterRegister[CAN_Filter_Index].FR2 = ( UINT32 )CAN_Message_ID1;	// CAN_Message_ID2
	////  32-bit
	// identifier

	m_can_reg->FFA1R &= ~( UINT32 )filter_number_bit_pos;					// assign filter to FIFO
																			// 0
	m_can_reg->FA1R |= filter_number_bit_pos;									// activate filter

	m_can_reg->FMR &= ~CAN_FMR_FINIT;					// reset Initialisation mode for filter banks
}
...
UINT8 uC_CAN::CAN_Data_Transmit( CAN_MessageBuf* TxMessage )
{
	...
	if ( transmit_mailbox != 0x04 )
	{
		...
		{
			m_can_reg->sTxMailBox[transmit_mailbox].TIR |= ( ( TxMessage->id << 21 )
															 | TxMessage->frameType );
		}
		else
		{
			m_can_reg->sTxMailBox[transmit_mailbox].TIR |= ( ( TxMessage->id << 3 )
															 | TxMessage->isIdExtended << 2U |
