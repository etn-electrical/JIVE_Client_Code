	...
	: m_cr_update_task( reinterpret_cast<CR_Tasker*>( NULL ) )
{
	...
	m_volatile_item_first.end -= 1U;	// start of system SRAM - 1 [Separate statements to avoid a
										// PC-lint warning (428).]
	...
	m_volatile_item_pointer.end = ( m_volatile_item_pointer.start + sizeof( p_RunTimeRamChk ) ) -
		1U;
...
uint8_t* uC_RAM::Check_Volatile( uint8_t* start, uint32_t length )
{
	 ...
	 *@n Justification: Generic #define is used here, recasting in that #define would have adverse
	 * effects throughout.
	 */
	/*lint -e{1960} # MISRA Deviation */
	return_value = reinterpret_cast<uint8_t*>( Mem_Aligned_Size_Word(
