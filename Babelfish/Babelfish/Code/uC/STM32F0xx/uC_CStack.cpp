...
void uC_CSTACK_Init( void )
{
	...
	target_ptr = reinterpret_cast<BF_Lib::MBASE*>( ( ( ( static_cast<UINT32>( __get_SP() )
														 - uC_CSTACK_FILL_HEAD_START ) ) -
													 ( sizeof( BF_Lib::MBASE ) - 1U ) )
												   & ~( sizeof( BF_Lib::MBASE ) - 1U ) );	// Align
																							// the
																							// number
																							// with
																							// 32bits.
	...
	cstack_begin = reinterpret_cast<BF_Lib::MBASE*>( ( reinterpret_cast<UINT32>( __section_begin(
																					 "CSTACK" ) ) +
													   ( sizeof( BF_Lib::MBASE ) - 1U ) ) &
													 ~( sizeof( BF_Lib::MBASE ) - 1U ) );
	cstack_end = reinterpret_cast<BF_Lib::MBASE*>( ( reinterpret_cast<UINT32>( __section_end(
																				   "CSTACK" ) ) -
													 ( sizeof( BF_Lib::MBASE ) - 1U ) ) &
												   ~( sizeof( BF_Lib::MBASE ) - 1U ) );
...
uint8_t uC_CSTACK_Used( void )
{
	...
	if ( present_top != old_top )
	{
		 ...
		 *@n MISRA-C++[2008] Required Rule 5-8-1, out of bounds value for right hand side of shift
		 * operator
		 *@n Justification: PC-lint false alarm.  Round_Div shifts the divisor right 2. PC-lint is
		 * seeing
