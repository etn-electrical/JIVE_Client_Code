...
uC_TIMERS_TICK_TD uC_Timers::Set_Timeout( uint32_t desired_frequency, bool repeat )
{
	...
	count = Round_Div( static_cast<uC_TIMERS_TICK_TD>( m_tclock_freq ),
					   static_cast<uC_TIMERS_TICK_TD>( prescaler + 1U ) ) / static_cast<
		uC_TIMERS_TICK_TD>( desired_frequency );
...
uC_TIMERS_TICK_TD uC_Timers::Get_Timer_Ticks( uint32_t desired_frequency ) const
{
	...
	{
		return_tick_count = return_tick_count / static_cast<uC_TIMERS_TICK_TD>
			( desired_frequency );
	}
