...
uC_Flash::uC_Flash( uC_FLASH_CHIP_CONFIG_TD const* chip_config, FLASH_TypeDef* flash_ctrl,
	...
	NV_Ctrl(),
	m_chip_cfg( reinterpret_cast<uC_FLASH_CHIP_CONFIG_TD const*>( nullptr ) ),
	m_flash_ctrl( reinterpret_cast<FLASH_TypeDef*>( nullptr ) ),
	m_erase_val( 0U ),
	...
	if ( Check_Range( address, length, use_protection ) )
	{
		...
		if ( ( good_data == true ) && ( use_protection == true ) )
		{
			...
			do
			{
				...
				good_data =
					Write_Now( data, address, recovery_address, length, use_protection );
				if ( good_data == true )
				{
						  ...
						  NV_CTRL_LENGTH_TD length, bool use_protection )
{
	...
	if ( ( dest_address & ~uC_FLASH_PAGE_ALIGNMENT_MASK ) != 0U )		// recovery_address !=
	...
	if ( ( ( dest_address + total_length ) & ~uC_FLASH_PAGE_ALIGNMENT_MASK ) != 0U )	// ( recovery_address
																						// + total_length
																						// ) !=
																						// end_page_address
																						// )
	{
		...
		{
			m_end_rec_buff.data =
				reinterpret_cast<uint8_t*>( recovery_address +
...
uC_FLASH_STATUS_EN uC_Flash::Get_Status( void ) const
{
	...
	else
	{
		...
		{
			if ( ( m_flash_ctrl->SR & FLASH_SR_WRPRTERR ) != 0U )
			{
