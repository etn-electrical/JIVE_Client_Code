...
#include "Includes.h"
#include "uC_DMA.h"
// #define DEBUG_RAM_TEST	// Define this to provide debug information to the host via
// Term_Print().
#include "uC_RAM.h"
 ...
 *@n Justification: The pattern of __ICFEDIT_region_RAM_start__ and __ICFEDIT_region_RAM_end__
 *@n is determined by the linker and cannot be changed.
 */
	...
	: m_cr_update_task( reinterpret_cast<CR_Tasker*>( NULL ) )
{
	...
	m_cr_update_task = new CR_Tasker( &Update_Static, reinterpret_cast<CR_TASKER_PARAM>( this ),
									  CR_TASKER_IDLE_PRIORITY, "uC_RAM Update" );

	// This volatile region represents the address space starting at the very
	// beginning and going up to but not including __ICFEDIT_region_RAM_start__.
	...
	m_volatile_item_last.start = reinterpret_cast<uint8_t*>( &__ICFEDIT_region_RAM_end__ + 1U );
