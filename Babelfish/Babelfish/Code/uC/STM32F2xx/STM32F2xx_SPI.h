 ...
 *****************************************************************************************
 */
#ifndef STM32F2xx_SPI_H
#define STM32F2xx_SPI_H
...
class STM32F2xx_SPI : public uC_Queued_SPI
{
	public:
		 ...
		 */
		STM32F2xx_SPI( uint8_t channel, uint8_t no_slaves, bool_t master,
					   uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl, uC_USER_IO_STRUCT const* const pio[] );

		/** @brief Destructor to delete an instance of STM32F2xx_SPI class */
		~STM32F2xx_SPI( void );
	...
	protected:
		...
		{
			static_cast<STM32F2xx_SPI*>( object )->DMA_ISR();
		}
		...
		{
			static_cast<STM32F2xx_SPI*>( object )->SPI_ISR();
		}
	...
	private:
		 ...
		 */
		STM32F2xx_SPI( void ) : m_spi( nullptr )
		{}
