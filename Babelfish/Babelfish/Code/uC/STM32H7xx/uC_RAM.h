...
class uC_RAM
{
	public:
		...
		/**
		 * @alignment size for ram diagnostics test
		 */
		static const uint32_t PROCESSOR_BYTE_SIZE = 1U;
	...
	private:
		 ...
		 */
		static inline uint32_t Mem_Aligned_Size_Byte( uint32_t mem_size )
		{
			return ( ( static_cast<UINT32>( mem_size ) + ( PROCESSOR_BYTE_SIZE - 1U ) )
					 & ( ~static_cast<UINT32>( PROCESSOR_BYTE_SIZE - 1U ) ) );
		}
		 ...
		 * @code
		   ...
		   m_volatile_item_first.end   = &__ICFEDIT_region_RAM_start__ - 1U;	// start of system SRAM - 1
		   m_volatile_item_first.next  = &m_volatile_item_last;	/// next (point it at the last item in the list)
		   m_volatile_item_first.prev  = &m_volatile_item_last;	/// prev (point it at the last item in the list)
		 * @endcode
		 */
		static volatile_item_t m_volatile_item_first;

		/**
		 * @details This is the SECOND region in the list of volatile regions of
		 * memory that should not be tested by the RAM test.  It's marked as
		 * volatile at startup and NEVER unmarked.  It isn't really a volatile
		 * region in memory, but its presence speeds up the handling of the truly
		 * volatile regions.  It's typically initialized as follows:
		 *
		 * @code
		   m_volatile_item_post_ccm.start = &__ICFEDIT_region_CCMRAM_end__ + 1U;	// start of CCM + 1
		   m_volatile_item_post_ccm.end   = &__ICFEDIT_region_RAM_start__ - 1U;	// start of system SRAM - 1
		   m_volatile_item_post_ccm.next  = &m_volatile_item_last;
		   m_volatile_item_post_ccm.prev  = &m_volatile_item_last;
		 * @endcode
		 */
		static volatile_item_t m_volatile_item_post_ccm;
		 ...
		 * @code
		   ...
		   m_volatile_item_last.next  = &m_volatile_item_first;	/// next (point it at the first item in the list)
		   m_volatile_item_last.prev  = &m_volatile_item_first;	/// prev (point it at the first item in the list)
		 * @endcode
		 */
