...
//it runs off of a 32kHz clock.
#define IWDG_BASE_CLOCK_FREQ        32000U  ///Range is 30to60kHz.
...
void uC_Watchdog_HW::Set_Timeout( uint32_t timeout_in_ms )
{
	...
	if ( timeout_in_ms != 0U )
	{
		...
		IWDG1->KR = IWDG_ENABLE_WDOG;
		IWDG1->KR = IWDG_WRITE_ACCESS_ENABLE;    //open for write access.
		IWDG1->PR = prescaler_index;
		IWDG1->RLR = test_count;
		while( IWDG1->SR != 0U )
		{
			/*Wait for the registers to be updated (IWDG_SR = 0x0000 0000).*/
		}

		IWDG1->KR = IWDG_COUNT_RELOAD;
