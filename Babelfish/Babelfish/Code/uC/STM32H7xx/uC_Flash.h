...
static const uint32_t FLASH_PGERR = ( FLASH_SR_RDSERR | FLASH_SR_RDPERR | \
									  FLASH_SR_OPERR | FLASH_SR_INCERR | \
									  FLASH_SR_STRBERR | FLASH_SR_PGSERR | \
									  FLASH_SR_WRPERR );
/*
 *****************************************************************************************
 *		Constants
...
class uC_Flash : public NV_Ctrl
{
	...
	private:
		BOOL Write_Now_Bank1( UINT8* data, UINT32 dest_address, NV_CTRL_LENGTH_TD length );

		BOOL Write_Now_Bank2( UINT8* data, UINT32 dest_address, NV_CTRL_LENGTH_TD length );
		...
		inline void Write_u8( uint32_t adr, uint8_t dat )
		{
			( *( ( volatile uint8_t* ) ( adr ) ) ) = dat;
			__DSB();
		}

		inline void Write_u16( uint32_t adr, uint16_t dat )
		{
			( *( ( volatile uint16_t* ) ( adr ) ) ) = dat;
			__DSB();
		}

		inline void Write_u32( uint32_t adr, uint32_t dat )
		{
		...
		static const uint16_t WATCHDOG_TIMEOUT_ERASE_SECTOR = 5000U;
};
}
#endif
