 ...
 *****************************************************************************************
 */
uC_USART_9BIT_Soft_DMA::uC_USART_9BIT_Soft_DMA( uint8_t hw_usart_num, uint8_t hw_timer_num,
												const uC_BASE_USART_IO_PORT_STRUCT* io_port ) :
	uC_USART_9BIT_HW( hw_usart_num, io_port, false ),
	...
	m_tx_buff_index( 0U ),
	m_cback_request( 0U ),
	m_cback_func( reinterpret_cast<uC_USART_9BIT_Buff::cback_func_t>( nullptr ) ),
	m_cback_param( reinterpret_cast<uC_USART_9BIT_Buff::cback_param_t>( nullptr ) ),
	m_tx_holdoff( false ),
	...
	m_char_size( uC_USART_9BIT_Buff::CHAR_SIZE_9BIT ),
	uC_USART_9BIT_Buff()
{
	Push_TGINT();
	...
	uC_USART_9BIT_Soft_DMA::Stop_TX();
	uC_USART_9BIT_Soft_DMA::Stop_RX();
 ...
 *****************************************************************************************
 */
uC_USART_9BIT_Soft_DMA::~uC_USART_9BIT_Soft_DMA( void )
{
	m_rx_buff = reinterpret_cast<uint8_t*>( nullptr );
	m_tx_buff = reinterpret_cast<uint8_t*>( nullptr );
	m_cback_func = static_cast<uC_USART_9BIT_Buff::cback_func_t>( 0U );
	m_cback_param = static_cast<uC_USART_9BIT_Buff::cback_param_t>( 0U );
	delete m_timer;
}
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::Config_Callback( uC_USART_9BIT_Buff::cback_func_t cback_func,
											  ...
											  uint32_t rx_byte_time, uint32_t tx_holdoff_time )
{
	...
	if ( ( tx_holdoff_time != 0U ) && ( BF_Lib::Bit::Test( m_cback_request, TXRX_HOLDOFF_COMPLETE ) ||
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::Start_RX( uint8_t* data, uint16_t length )
{
	...
	dump_data = static_cast<uint8_t>( m_usart->DR );

	m_rx_buff = data;
	m_rx_buff_index = 0U;
	m_rx_buff_len = length;

	m_usart->SR &= ~USART_SR_RXNE;
 ...
 *****************************************************************************************
 */
uC_USART_9BIT_Soft_DMA::rx_state_t uC_USART_9BIT_Soft_DMA::RX_State( void )
{
	return ( m_rx_state );
}
 ...
 *****************************************************************************************
 */
uint16_t uC_USART_9BIT_Soft_DMA::Get_RX_Length( void )
{
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::Stop_RX( void )
{
	Push_TGINT();
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::Start_TX( uint8_t* data, uint16_t length )
{
	Push_TGINT();
	...
	m_usart->SR &= ~USART_SR_TXE;	// ? Clearing the TXE bit is always performed by a write to the data register
 ...
 *****************************************************************************************
 */
uC_USART_9BIT_Soft_DMA::tx_state_t uC_USART_9BIT_Soft_DMA::TX_State( void )
{
	return ( m_tx_state );
}
 ...
 *****************************************************************************************
 */
bool_t uC_USART_9BIT_Soft_DMA::TX_Holdoff( void )
{
	return ( m_tx_holdoff );
}
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::Stop_TX( void )
{
	Push_TGINT();
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::USART_Int( void )
{
	uint32_t int_status;
	...
	int_status = m_usart->SR & static_cast<uint32_t>( m_int_enabled_mask );
	m_usart->SR = 0U;

	if ( ( ( int_status & USART_SR_RXNE ) != 0U ) )
	{
		if ( m_rx_buff_index < m_rx_buff_len )
		{
			if ( m_char_size != uC_USART_9BIT_Buff::CHAR_SIZE_9BIT )
			{
				m_rx_buff[m_rx_buff_index] = static_cast<uint8_t>( Get_RX_Data() );
				m_rx_buff_index++;
			}
			else
			{
				int_RxData = Get_RX_Data();
				m_rx_buff[m_rx_buff_index] = static_cast<uint8_t>( int_RxData );
				m_rx_buff_index++;
				m_rx_buff[m_rx_buff_index] = static_cast<uint8_t>( int_RxData >> 8 );
				m_rx_buff_index++;
			}
		}
	...
	if ( ( int_status & USART_SR_TXE ) != 0U )
	{
		if ( m_char_size != uC_USART_9BIT_Buff::CHAR_SIZE_9BIT )
		{
			m_usart->DR = m_tx_buff[m_tx_buff_index];
			m_tx_buff_index++;
		...
		{
			int_TxData = m_tx_buff[m_tx_buff_index + 1U];
			int_TxData = int_TxData << 8U;
			int_TxData |= m_tx_buff[m_tx_buff_index];
			m_usart->DR = int_TxData;
			m_tx_buff_index += 2U;
			if ( m_tx_buff_index >= m_tx_buff_len )
			{
				m_int_enabled_mask &= ~USART_CR1_TXEIE;
	...
	else if ( ( int_status & USART_SR_TC ) != 0U )
	{
		m_usart->CR1 &= ~USART_CR1_TCIE;
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::RX_Timer_Int( void )
{
	cback_status_t cback_status_bits = 0U;
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_Soft_DMA::TX_Timer_Int( void )
{
	cback_status_t cback_status_bits = 0U;
 ...
 *****************************************************************************************
 */
bool_t uC_USART_9BIT_Soft_DMA::Config_Port( uint32_t baud_rate, uC_USART_9BIT_Buff::char_size_t char_size,
											uC_USART_9BIT_Buff::parity_t parity,
											uC_USART_9BIT_Buff::stop_bit_t stop_bit_count )
{
	uC_USART_9BIT_HW::char_size_t hw_char_size;
	uC_USART_9BIT_HW::parity_t hw_parity;
	uC_USART_9BIT_HW::stop_bit_t hw_stop_bit_count;
	bool_t success = true;

	switch ( char_size )
	{
		case uC_USART_9BIT_Buff::CHAR_SIZE_5BIT:
			hw_char_size = uC_USART_9BIT_HW::CHAR_SIZE_5BIT;
			break;

		case uC_USART_9BIT_Buff::CHAR_SIZE_6BIT:
			hw_char_size = uC_USART_9BIT_HW::CHAR_SIZE_6BIT;
			break;

		case uC_USART_9BIT_Buff::CHAR_SIZE_7BIT:
			hw_char_size = uC_USART_9BIT_HW::CHAR_SIZE_7BIT;
			break;

		case uC_USART_9BIT_Buff::CHAR_SIZE_8BIT:
			hw_char_size = uC_USART_9BIT_HW::CHAR_SIZE_8BIT;
			break;

		case uC_USART_9BIT_Buff::CHAR_SIZE_9BIT:
			hw_char_size = uC_USART_9BIT_HW::CHAR_SIZE_9BIT;
			break;
	...
	switch ( parity )
	{
		case uC_USART_9BIT_Buff::PARITY_EVEN:
			hw_parity = uC_USART_9BIT_HW::PARITY_EVEN;
			break;

		case uC_USART_9BIT_Buff::PARITY_ODD:
			hw_parity = uC_USART_9BIT_HW::PARITY_ODD;
			break;

		case uC_USART_9BIT_Buff::PARITY_SPACE:
			hw_parity = uC_USART_9BIT_HW::PARITY_SPACE;
			break;

		case uC_USART_9BIT_Buff::PARITY_MARK:
			hw_parity = uC_USART_9BIT_HW::PARITY_MARK;
			break;

		case uC_USART_9BIT_Buff::PARITY_NONE:
			hw_parity = uC_USART_9BIT_HW::PARITY_NONE;
			break;
	...
	{
		case uC_USART_9BIT_Buff::STOP_BIT_1:
			hw_stop_bit_count = uC_USART_9BIT_HW::STOP_BIT_1;
			break;

		case uC_USART_9BIT_Buff::STOP_BIT_1p5:
			hw_stop_bit_count = uC_USART_9BIT_HW::STOP_BIT_1p5;
			break;

		case uC_USART_9BIT_Buff::STOP_BIT_2:
			hw_stop_bit_count = uC_USART_9BIT_HW::STOP_BIT_2;
			break;
