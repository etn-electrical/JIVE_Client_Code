...
/*
    External Battery supply voltage ( VBAT ) is connected to ADC3 channel 17
    Internal Temperature sensor ( VSENSE ) is connected to ADC3 channel 18
    Internal reference voltage ( VREFINT ) is connected to ADC3 channel 19
 */
...
#define ADC_MAX_CLOCK_FREQUENCY                  20000000UL
	...
	m_cback_func( nullptr )
{
	...
	Clr_Mask( m_adc->reg_ctrl->CR, ADC_CR_DEEPPWD );

	// Enable ADC instance internal voltage regulator.
	m_adc->reg_ctrl->CR |= ADC_CR_ADVREGEN;
	// ADC LDO startup time = 10 usec.
	uC_Delay( 10U );

	/* adc_sclk is equal to sys_ck when HPRE is set to 0, otherwise it corresponds to sys_ck/2 */
	/* The ADC clock can be a specific clock source, named "adc_ker_ck_input" which is
	    independent and asynchronous with the AHB clock.
	    HSI -> PER_CLK -> adc_ker_ck_input */

	m_adc->common_reg_ctrl->CCR |= ADC_CCR_CKMODE;

	if ( ( AHB1_PERIPH_CLOCK_FREQ / 2 ) > ADC_MAX_CLOCK_FREQUENCY )
	{
		if ( ( AHB1_PERIPH_CLOCK_FREQ / 4 ) > ADC_MAX_CLOCK_FREQUENCY )
		{
			if ( ( AHB1_PERIPH_CLOCK_FREQ / 6 ) > ADC_MAX_CLOCK_FREQUENCY )
			{
				ADCClkInit.ClockPrescaler = ADC_CLOCK_ASYNC_DIV8;
			}
			else
			{
				ADCClkInit.ClockPrescaler = ADC_CLOCK_ASYNC_DIV6;
			}
		}
		else
		{
			ADCClkInit.ClockPrescaler = ADC_CLOCK_ASYNC_DIV4;
		}
	}
	else
	{
		ADCClkInit.ClockPrescaler = ADC_CLOCK_ASYNC_DIV2;
	}
	/* Configure asyncronous clock and prescaler for ADC */
	Set_Mask( m_adc->common_reg_ctrl->CCR, ADC_CCR_PRESC, ADCClkInit.ClockPrescaler );

	/* Configure the BOOST Mode */
	if ( ADC_MAX_CLOCK_FREQUENCY > 20000000UL )
	{
		m_adc->reg_ctrl->CR |= ( 0x1UL << ADC_CR_BOOST_Pos );
	}

	/* Configure ADC resolution */
	m_resolution = static_cast<uint8_t>( resolution );

	if ( resolution == uC_ATOD_8BIT_RESOLUTION )
	{
		m_adc->reg_ctrl->CFGR |= ADC_RESOLUTION_8B;
	}
	else if ( resolution == uC_ATOD_10BIT_RESOLUTION )
	{
		m_adc->reg_ctrl->CFGR |= ADC_RESOLUTION_10B;
	}
	else if ( resolution == uC_ATOD_12BIT_RESOLUTION )
	{
		m_adc->reg_ctrl->CFGR |= ADC_RESOLUTION_12B;
	}
	else if ( resolution == uC_ATOD_14BIT_RESOLUTION )
	{
		m_adc->reg_ctrl->CFGR |= ADC_RESOLUTION_14B;
	}
	else
	{
		m_adc->reg_ctrl->CFGR |= ADC_RESOLUTION_16B;
	}

	/* Disable ADC oversampling scope on ADC group regular */
	Clr_Mask( m_adc->reg_ctrl->CFGR2, ADC_CFGR2_ROVSE );
	...
	while ( channel_counter < static_cast<uint8_t>( m_adc->max_channels ) )
	{
		if ( ( channel_mask & active_channels_mask ) != 0U )
		{
			...
			{
				m_adc->reg_ctrl->CR |= ADC_CCR_TSEN | ADC_CCR_VREFEN;
			}
	...
	// Update Regular channel sequence length such that ADC can understand
	// how many channels are being used.
	m_adc->reg_ctrl->SQR1 |= ( static_cast<uint32_t>( total_channels ) - 1U );

	m_adc->reg_ctrl->ISR |= ADC_ISR_ADRD;
	...
	if ( m_adc->dma_channel != static_cast<uint16_t>( uC_DMA_STREAM_DNE ) )
	{
		...
		m_adc->reg_ctrl->CFGR |= ADC_CFGR_DMNGT;	// DMA one shot mode selected
	...
	Dump_AtoD_Data();
	if ( time_reqd_to_scan_all_channels_us != 0U )
	{
...
void uC_AtoD::Set_Callback( INT_CALLBACK_FUNC* int_callback, uint8_t int_priority ) const
{
	...
	m_adc->reg_ctrl->IER |= ADC_IER_EOC;
	m_adc->reg_ctrl->IER |= ADC_IER_OVR;
...
{
	// Clear the Continuous conversion bit for single conversion
...
{
	return ( ( m_adc->reg_ctrl->ISR & ADC_ISR_EOC ) != 0U || m_adc->reg_ctrl->ISR & ADC_ISR_EOS );
}
