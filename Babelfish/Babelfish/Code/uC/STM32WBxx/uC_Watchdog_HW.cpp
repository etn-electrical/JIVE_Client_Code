	...
	if ( timeout_in_ms != 0U )
	{
		...
		do
		{
			test_count = timeout_in_ms *
				( static_cast<uint32_t>( IWDG_BASE_CLOCK_FREQ ) >> m_prescalers[prescaler_index] );
		...
		// Stop in debug. IWDG is frozen while CPU1 is in debug mode.
		DBGMCU->APB1FZR1 |= DBGMCU_APB1FZR1_DBG_IWDG_STOP;

		// Enable watchdog and set other registers
