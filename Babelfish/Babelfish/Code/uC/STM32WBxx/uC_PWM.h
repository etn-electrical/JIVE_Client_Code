 ...
 *                  over PWM channels on STM32WB55xx.
 * @details
...
static const uint8_t MAX_TIMER_COMPLIMENTARY_PINS = 8U;
static const uint16_t MAX_PWM_TICKS = 0xFFFF;

// This turns the period into a percentage of full range.
static const uint8_t PWM_DUTY_CYCLE_PERCENT_SHIFT = 16U;

// This is a minimum duty cycle setting.
static const uint16_t MIN_PWM_DUTY_CYCLE_TICKS = 1U;

static const uint8_t PWM_MODE_ENABLED = 0x3U;

uC_PERIPH_IO_STRUCT const* const timer_pio_n[MAX_TIMER_COMPLIMENTARY_PINS] =
{ &TIMER1_PIO1_PA7,
  &TIMER1_PIO2_PB8,
  &TIMER1_PIO3_PB9,
  &TIMER16_PIO1_PB6,
...
class uC_PWM
{
	public:
		 ...
		 * @n                                 STM32WB55xx features 4 PWM channels with each PWM control block.
		 ...
		 * @return
		 */
		void Set_Duty_Cycle_Pct( uint16_t count );

		/**
		 * @brief                   This function sets PWM duty cycle.
		 * @param[in] percent       0.0 to 100% Duty cycle command
		 * @return
		 */
		void Set_Duty_Cycle_Percentage( float32_t percent );
