...
#define TOTAL_SPI_CLOCK_PRESC       8
				...
				uint8_t total_chip_sel, uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl, bool enable_dma )
{
	...
	if ( master == true )
	{
		...
		m_chip_sel_cfg = ( CHIP_SELECT_CFG_STTD* )Ram::Allocate(
			( sizeof( CHIP_SELECT_CFG_STTD ) * total_chip_sel ) );
	...
	if ( ( m_config->tx_dma_channel != uC_DMA_CHANNEL_DNE ) &&
		 ( m_config->tx_dma_channel != uC_DMA_CHANNEL_DNE ) &&
		 ( enable_dma == true ) )
	{
		m_rx_dma = new uC_DMA( m_config->rx_dma_channel, uC_DMA_DIR_FROM_PERIPH, sizeof( uint8_t ),
							   uC_DMA_MEM_INC_TRUE, uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );

		m_tx_dma = new uC_DMA( m_config->tx_dma_channel, uC_DMA_DIR_FROM_MEMORY, sizeof( uint8_t ),
							   uC_DMA_MEM_INC_TRUE, uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	switch ( spi_mode )
	{
		...
		case uC_SPI_MODE_1:
			m_chip_sel_cfg[chip_select].cr1 |= SPI_CR1_CPHA;
			break;

		case uC_SPI_MODE_2:
			m_chip_sel_cfg[chip_select].cr1 |= SPI_CR1_CPOL;
			break;

		case uC_SPI_MODE_3:
			m_chip_sel_cfg[chip_select].cr1 |= ( SPI_CR1_CPOL | SPI_CR1_CPHA );
			break;
	...
	if ( bits_to_transfer == 16 )
	{
		m_chip_sel_cfg[chip_select].cr1 |= SPI_CR1_DFF;
	}
	else if ( bits_to_transfer != 8 )
	{
		BF_ASSERT( false );
	}
	...
	m_chip_sel_cfg[chip_select].cr1 |= ( SPI_CR1_BR & ( clock_div_counter << 3 ) );

	uC_IO_Config::Set_Pin_To_Output( m_chip_sel_ctrls[chip_select], uC_IO_OUT_INIT_HIGH );
}
...
void uC_SPI::Select_Chip( uint8_t chip_select )
{
	...
	temp_cr1 = m_spi->CR1 & SPI_CR1_SPE;	// Push the SPI enable register so we can choose to properly enable it.
 ...
 *****************************************************************************************
 */
void uC_SPI::De_Select_Chip( uint8_t chip_select )
{
