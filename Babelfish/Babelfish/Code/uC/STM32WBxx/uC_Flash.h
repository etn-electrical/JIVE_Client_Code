...
#define HSEM_PROCESS_1 1U
#define WRITE_RETRY_COUNT 3U
#define SYNC_WRITE_RETRY_DELAY 10U	// 10us delay for every retry
#define ASYNC_WRITE_RETRY_DELAY 22U	// 22ms delay for every retry
#define ERASE_RETRY_DELAY 10U	// 10us delay for every retry

namespace NV_Ctrls
{

static const uint32_t FLASH_PGERR = ( FLASH_SR_PGSERR | FLASH_SR_PROGERR | FLASH_SR_PGAERR
									  | FLASH_SR_WRPERR );
/*
 *****************************************************************************************
 *		Constants
 *****************************************************************************************
 */
static const uint8_t MIN_WRITE_CHUNK_LEN = 8U;
...
class uC_Flash : public NV_Ctrl
{
	...
	private:
		...
		/**
		 * @brief Interrupt handler for internal flash.
		   Currenlty only used in DEBUG mode to capture flash error events
		 */
		...
		CR_Tasker* m_flash_async_task_handle;

		uint32_t m_erase_start_time;

		/* Erase operation will be held for following milliseconds
		 * Note: When M4 updates BLE characteristics values to M0 using Ble_Char_Value_Update() api
		 * then immediately we should not perform any flash erase operation */
		const uint8_t ERASE_HOLDING_TIME_MILLI_SEC = 100U;
		...
		const uint32_t WRITE_CHUCK_SIZE = 112U;
};
}
#endif
