	...
	m_tx_dma( reinterpret_cast<uC_DMA*>( nullptr ) )
{
	...
	if ( m_config != reinterpret_cast<uC_BASE_SPI_IO_STRUCT const*>( nullptr ) )
	{
		...
		if ( master == true )
		{
			...
			m_chip_sel_cfg =
				reinterpret_cast<CHIP_SELECT_CFG_STTD*>( Ram::Allocate(
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	if ( chip_select < m_total_chip_sel )
	{
		...
		switch ( spi_mode )
		{
			...
			case uC_SPI_MODE_3:
				m_chip_sel_cfg[chip_select].cfg2 |= static_cast<uint16_t>( SPI_CFG2_CPOL | SPI_CFG2_CPHA );
				break;
		...
		if ( clock_div_counter > TOTAL_SPI_CLOCK_PRESC )
		{
			...
			BF_Lib::Exception::Stop_Here();	// We probably should not use a clock speed faster than the passed in value.
		}
