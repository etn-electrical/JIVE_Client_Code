...
{
	UINT32 CAN_Message_ID1 = 0U;
	UINT32 filter_number_bit_pos = 0U;
...
{
	uint32_t filter_number_bit_pos = 0U;
...
UINT8 uC_CAN::CAN_Data_Transmit( CAN_MessageBuf* TxMessage )
{
	...
	if ( transmit_mailbox != 0x04 )
	{
		...
		m_can_reg->sTxMailBox[transmit_mailbox].TDTR &= ( uint32_t )0xFFFFFFF0U;
...
void uC_CAN::CAN_Receive_Data( CAN_MessageBuf* RxMessage )
{
	...
	/* Release the FIFO 0 */
	m_can_reg->RF0R |= CAN_RF0R_RFOM0U;
...
{
	UINT8 isCANBusOffStateReached = 0U;

	/* Bus-Off bits */
	if ( ( m_can_reg->ESR & CAN_FLAG_BOF ) )
	{
		isCANBusOffStateReached = 1;
	}
	else
	{
		isCANBusOffStateReached = 0U;
	}
...
{
	UINT8 isCANErrCntrPassiveStateReached = 0U;

	/* Error cntr Passive state bits */
	if ( ( m_can_reg->ESR & CAN_FLAG_EPV ) )
	{
		isCANErrCntrPassiveStateReached = 1;
	}
	else
	{
		isCANErrCntrPassiveStateReached = 0U;
	}
...
{
	UINT8 isCANErrCntrWarnStateReached = 0U;

	/* Error cntr Warn state bits */
	if ( ( m_can_reg->ESR & CAN_FLAG_EWG ) )
	{
		isCANErrCntrWarnStateReached = 1;
	}
	else
	{
		isCANErrCntrWarnStateReached = 0U;
	}
