...
class uC_SPI
{
	public:
		...
		{
			volatile uint8_t dump1;			// Dan Hosko ADDED TO CLEAR RXNE FLAG

			while ( ( ( m_spi->SR & SPI_SR_RXNE ) != 0U ) || ( m_spi->SR & SPI_SR_BSY ) )
			{
				dump1 = *( ( __IO uint8_t* )&m_spi->DR );
			}
			*( ( __IO uint8_t* )&m_spi->DR ) = data;
		}
		...
		{
			volatile uint16_t dump1;		// Dan Hosko ADDED TO CLEAR RXNE FLAG
		...
		inline uint8_t RX_Byte( void )
		{
			...
			while ( ( m_spi->SR & SPI_SR_RXNE ) != 0U || ( m_spi->SR & SPI_SR_BSY ) )
			{
				dump = *( ( __IO uint8_t* )&m_spi->DR );
			}
			*( ( __IO uint8_t* )&m_spi->DR ) = 0U;
