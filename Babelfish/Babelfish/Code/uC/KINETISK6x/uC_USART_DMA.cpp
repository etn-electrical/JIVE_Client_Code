	...
	uC_USART_Buff()
{
	...
	m_tx_dma_ctrl->Attach_Callback( &TX_DMA_Int_Static,
									static_cast<uC_DMA::cback_param_t>( this ),
									static_cast<uint8_t>( uC_INT_HW_PRIORITY_12 ) );
	m_usart->C5 |= UART_C5_TDMAS_MASK;

	if ( hw_timer_num < uC_BASE_MAX_TIMER_CTRLS )
	{
		...
		m_timer = new uC_Multi_Timers( hw_timer_num, TOTAL_TIMERS, static_cast<uint8_t>( uC_INT_HW_PRIORITY_13 ) );
									...
									uint32_t rx_byte_time, uint32_t tx_holdoff_time )
{
	...
	if ( m_timer != NULL )
	{
		...
		if ( ( rx_byte_time != 0 ) && Test_Bit( m_cback_request, RX_BYTE_TIMEOUT ) )
		{
...
void uC_USART_DMA::Start_RX( uint8_t* data, uint16_t length )
{
	...
	m_buffer_head = data;
	dump_status = m_usart->S1;
	dump_data = m_usart->D;

	m_rx_dma_ctrl->Set_Buff( data, ( UINT8* )&m_usart->D, length );
	...
	m_usart->S1 &= ~UART_S1_RDRF_MASK;

	if ( BF_Lib::Bit::Test( m_cback_request, RX_BUS_IDLE ) ||
		 BF_Lib::Bit::Test( m_cback_request, RX_BYTE_TIMEOUT ) )
	{
		m_usart->C1 |= UART_C1_ILT_MASK;	// idle count starts after stop bit
		m_usart->C2 |= UART_C2_ILIE_MASK;	// enable idle line interrupts
		m_int_enabled_mask |= UART_C2_ILIE_MASK;
	}
	else
	{
		m_usart->C1 &= ~UART_C1_ILT_MASK;
		m_usart->C2 &= ~UART_C2_ILIE_MASK;
		m_int_enabled_mask &= ~UART_C2_ILIE_MASK;
	}
...
void uC_USART_DMA::Stop_RX( void )
{
	...
	m_usart->C2 &= ~UART_C2_ILIE_MASK;			// disable idle line interrupts
	m_int_enabled_mask &= ~UART_C2_ILIE_MASK;

	Disable_RX();
	Disable_RX_Ready_Int();
...
void uC_USART_DMA::USART_Int( void )
{
	...
	int_status = m_usart->S1 & ( static_cast<uint32_t>( m_int_enabled_mask ) );

	if ( ( int_status & UART_S1_IDLE_MASK ) != 0 )
	{
		dump_data = m_usart->D;		/* Clear IDLE Flag */
	...
	if ( ( int_status & UART_S1_TC_MASK ) != 0U )
	{
		Stop_TX();
...
bool uC_USART_DMA::Config_Port( uint32_t baud_rate, uC_USART_Buff::char_size_t char_size,
								uC_USART_Buff::parity_t parity, uC_USART_Buff::stop_bit_t stop_bit_count )
{
