...
void uC_PWM::Turn_On( void )
{
	...
	{
		temp_val = static_cast<uint16_t>( *temp_ccmr & ~( static_cast<uint32_t>( TIM_CCMR1_OC1M | TIM_CCMR1_OC1PE ) ) );
	...
	else
	{
		temp_val =
			static_cast<uint16_t>( *temp_ccmr &
								   ( ~( static_cast<uint32_t>( TIM_CCMR1_OC1M | TIM_CCMR1_OC1PE ) << MULT_BY_256 ) ) );
		temp_val = temp_val |
			static_cast<uint16_t>( ( ( static_cast<uint16_t>( TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2 |
															  TIM_CCMR1_OC1PE ) ) <<
			MULT_BY_256 ) );
...
void uC_PWM::Turn_Off( void )
{
	...
	if ( Is_Number_Even( m_pwm_ch ) )
	{
		temp_val = static_cast<uint16_t>( *temp_ccmr & ~static_cast<uint32_t>( TIM_CCMR1_OC1M ) );
	}
	else
	{
		temp_val =
			static_cast<uint16_t>( *temp_ccmr & ( ~( static_cast<uint32_t>( TIM_CCMR1_OC1M ) << MULT_BY_256 ) ) );
	}
...
bool uC_PWM::Is_On( void )
{
	...
	{
		temp_val = static_cast<uint16_t>( *temp_ccmr & ( static_cast<uint32_t>( TIM_CCMR1_OC1M ) << MULT_BY_256 ) );
						...
						uint32_t desired_freq, bool default_state )
{
	...
	m_pwm_ctrl->reg_ctrl->CR1 &= ~TIM_CR1_CEN;	// Have to turn off the timer to configure some of this stuff.
	...
	if ( polarity == static_cast<uint8_t>( uC_PWM_DUTY_CYCLE_HIGH ) )
	{
		m_pwm_ctrl->reg_ctrl->CCER &=
			~( static_cast<uint32_t>( TIM_CCER_CC1P ) << ( static_cast<uint32_t>( m_pwm_ch ) << MULT_BY_8 ) );
	...
	Turn_Off();		// This should get the states right for the bumpless switch-over from gen IO to periph.

	m_pwm_ctrl->reg_ctrl->CCER |= ( static_cast<uint32_t>( TIM_CCER_CC1E ) << ( m_pwm_ch * 4U ) );
...
void uC_PWM::Set_Duty_Cycle_Pct( uint16_t percent )
{
	...
	ticks_required = ( static_cast<uint32_t>( percent ) * m_percent_adj ) >> PWM_DUTY_CYCLE_PERCENT_SHIFT;	// PWM_DUTY_CYCLE_PERCENT_CALC;
