...
int8_t uC_USART_USB::Usb_Cdc_Control( uint8_t cmd, uint8_t* pbuf, uint16_t length )
{
	...
	switch ( cmd )
	{
		case CDC_SET_LINE_CODING:
			...
			{
				rx_byte_time = static_cast<uint32_t>( 1000000.0 * 11.0 * 1.5 ) / m_bitrate;		// in microseconds
				tx_holdoff_time = static_cast<uint32_t>( 1000000.0 * 11.0 * 3.5 ) / m_bitrate;	// in microseconds
			}
			Config_Timers( rx_byte_time, tx_holdoff_time );
			break;
...
int8_t uC_USART_USB::Usb_Cdc_Receive( uint8_t* Buf, uint32_t* Len )
{
	...
	if ( RX_IDLE != m_rx_state )
	{
		...
		else
		{
			...
			{
				m_rx_state = RX_IDLE;
				if ( BF_Lib::Bit::Test( m_cback_request, static_cast<cback_status_t>( RX_BUFF_FULL ) ) )
				{
					BF_Lib::Bit::Set( cback_status_bits, static_cast<cback_status_t>( RX_BUFF_FULL ) );
...
int8_t uC_USART_USB::Usb_Cdc_Transmit( uint8_t* Buf, uint32_t* Len )
{
	...
	if ( TX_RUNNING == m_tx_state )
	{
		...
		if ( BF_Lib::Bit::Test( m_cback_request, static_cast<cback_status_t>( TX_COMPLETE ) ) )
		{
