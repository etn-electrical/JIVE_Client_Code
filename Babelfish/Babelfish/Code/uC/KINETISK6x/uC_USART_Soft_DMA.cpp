...
void uC_USART_Soft_DMA::Start_RX( uint8_t* data, uint16_t length )
{
	...
	if ( BF_Lib::Bit::Test( m_cback_request, RX_BYTE_TIMEOUT ) ||
		 BF_Lib::Bit::Test( m_cback_request, TXRX_HOLDOFF_COMPLETE ) )
	{
		m_int_enabled_mask |= UART_C2_RIE_MASK;
		m_usart->C2 |= UART_C2_RIE_MASK;
	}
	else
	{
		m_int_enabled_mask &= ~UART_C2_RIE_MASK;
...
void uC_USART_Soft_DMA::USART_Int( void )
{
	...
	m_usart->S1 |= ( UART_S1_OR_MASK | UART_S1_IDLE_MASK | UART_S1_NF_MASK );

	if ( ( int_status & UART_S1_RDRF_MASK ) != 0U )
	{
	...
	if ( ( int_status & UART_S1_TDRE_MASK ) != 0U )
	{
		m_usart->D = m_tx_buff[m_tx_buff_index];
		m_tx_buff_index++;
	...
	else if ( ( int_status & UART_S1_TC_MASK ) != 0U )
	{
		Stop_TX();
