...
class uC_SPI
{
	public:
		...
		{
			Dump();
			*( __IO uint16_t* )& m_spi->DR = 0;
			while ( !RX_Data_Reg_Full() ){}
			return ( ( uint16_t )m_spi->DR );
		}
		...
		{
			volatile uint8_t dump_u8;

			while ( RX_Data_Reg_Full() )
			{
				dump_u8 = ( uint8_t )m_spi->DR;
			}
		}

		/**
		 * @brief                    Read the word data from SPI data register and dump it. Make the SPI.
		 *                           transmission buffer empty.
		 * @note                     Implemented as an inline function.
		 * @n
		 * @n
		 */
		inline void Dump_Word( void )
		{
			volatile uint16_t dump_u16;

			while ( RX_Data_Reg_Full() )
			{
				dump_u16 = ( uint16_t )m_spi->DR;
			}
		}
		...
		/**
		 * @brief                   Enables interrupt for DMA Rx channel
		 */
		void Set_RX_DMA_Interrupt( void );



		/**
		 * @brief                   Enables interrupt for DMA Tx channel
		 */
		void Set_TX_DMA_Interrupt( void );



		/**
		 * @brief                   Enables interrupt for DMA channels assigned for Rx and Tx
		 */
		void Enable_XChange_DMA_Interrupt( void );
		...
		/**
		 * @brief                    Sets the user callback for for Rx DMA interrupt.
		 *
		 * @return void
		 * @n
		 */
		void Set_Rx_DMA_Callback( cback_func_t func_callback,
								  cback_param_t func_param, uint8_t int_priority );



		/**
		 * @brief                    Sets the user callback for for Tx DMA interrupt.
		 *
		 * @return void
		 * @n
		 */
		void Set_Tx_DMA_Callback( cback_func_t func_callback,
								  cback_param_t func_param, uint8_t int_priority );
