 ...
 *****************************************************************************************
 */
uC_Timers_HW::uC_Timers_HW( uint8_t timer_num ) :
...
uC_Timers_HW::TICK_TD uC_Timers_HW::Set_Timer_Frequency( uint32_t desired_frequency )
{
	...
	count =
		Round_Div( static_cast<TICK_TD>( m_tclock_freq ),
				   ( prescaler + 1U ) ) / desired_frequency;
...
void uC_Timers_HW::Set_Tick_Frequency( uint32_t desired_frequency )
{
	...
	if ( m_tmr_ctrl->cnt_size_bytes == 2U )
	{
		prescaler_check = static_cast<TICK_TD>( temp_calc >> 16 );
	}
	else
	{
		prescaler_check = static_cast<TICK_TD>( temp_calc >> 32 );
	}
