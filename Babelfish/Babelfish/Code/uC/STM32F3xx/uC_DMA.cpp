							  ...
							  cback_param_t param, uint16_t priority )
{
	...
	{
		/**object to contain the vector behaviour and capabilities**/
		m_advanced_vect = new Advanced_Vect();

	}

	/**add the call-back function in the vector and the function arguments cback will have**/
	m_advanced_vect->Set_Vect( cback_func, param );

	/**binding of the vector with vector table; relates to NVIC and priority**/
	return ( uC_Interrupt::Set_Vector( m_advanced_vect->Get_Int_Func(), m_dma->irq_num, priority ) );
}
...
void uC_DMA::Enable_End_Buff_Int( void ) const
{
	...
	m_dma->dma_ctrl->IFCR = static_cast<uint32_t>
		( DMA_IFCR_CGIF1 ) << ( static_cast<uint32_t>( m_dma->dma_num ) << MULT_BY_4 );
...
uint16_t uC_DMA::Counts_Remaining( void ) const
{
	...
	// if ( ( m_dma->dma_channel_ctrl->CCR | DMA_CCR1_DIR ) == 0U )
	{
