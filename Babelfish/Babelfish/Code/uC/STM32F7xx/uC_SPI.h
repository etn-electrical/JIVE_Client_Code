...
class uC_SPI
{
	public:
		 ...
		 * @note                    Implemented as Inline function initially but observed the failure issue while
		 *				flashing the webui codepack image on the external flash due to function
		 *				inlining. In LTK -8763 the implementation is changed from inline function
		 *				to normal function.
		 */
		 ...
		 * @note                    Implemented as Inline function initially but observed the failure issue while
		 *				flashing the webui codepack image on the external flash due to function
		 *				inlining. In LTK -8543 the implementation is changed from inline function
		 *				to normal function.
		 */
		 ...
		 *				In LTK-8763, Used the seperate call to enable RX DMA and TX DMA.
		 *				As with earlier implementation when logging support is enabled there is
		 *				issue with webui firmware update on external flash. Resolved this issue
		 *              		by using the seperate call to enable RX DMA and TX DMA in function
		 *				Enable_Exchange_DMA().
		 *
		 *
		 * @note                    Implemented as Inline function initially but observed the failure issue while
