...
class uC_I2C
{
	public:
		 ...
		 * @param[in] synchronus_transfer - The user can use this option to slect if the transfer
		 ...
		 *                          4. Synchronous data transfer using DMA.
		 */
		uC_I2C( uint8_t channel, uint16_t device_address, i2c_mode_t speed_mode = I2C_STANDARD_MODE,
				address_type_t addr_mode = ADDRESS_7_BIT,
				bool_t synchronus_transfer = false, bool_t enable_dma = false,
	...
	private:
		 ...
		 * @param[in] none	        The i2c channel from which data will be picked for receiving/transmitting
		 *                          over an I2C
		 * @param[in] i2c_mode      The i2c mode of communication standard mode or fast mode.
		 */
		 ...
		 * @param[in] none
		 * @param[out] return_type  Returning the error suring start condition.
		 */
		 ...
		 * @param[in] none
		 * @param[out] return_type  Returning the error while sending data.
		 */
		return_type Send_Data( uint8_t data );

		/**
		 * @brief                   Function sets the stop condition over an I2C bus.
		 * @param[in] none
		 * @param[out] return_type  Returning the error while sending stop condition.
		 */
		return_type Set_Stop( void );

		/** @brief Receives data from an I2C slave */
		/**
		 * @brief                   Function Receives data from an I2C slave .
		 * @param[in] data 			Data received from the I2C slave deveice
		 * @param[out] return_type  Returning the error while receiving the data.
		 */
		 ...
		 * @param[in] flag_mask		Bit which need to masked during the timeout
		 * @param[in] reg 			Registor shall be monitored for timeout
		 * @param[in] val 			value of the register need to validated during timeout
		 * @param[out] return_type  Returning the error while waiting for the timeout been elapsed.
		 */
		return_type Wait_Flag_Timeout( uint32_t flag_mask, volatile uint32_t* reg, uint16_t val ) const;

		/**
		 * @brief                   Timeout function in case start frame of I2C protocol.
		 * @param[in] None
		 * @param[out] return_type  Returning the error while waiting for the timeout been elapsed.
		 */
		...
		/**
		 * @brief                   			Setting the state to the state machine.
		 * @param[in] error						Error if any occured because of timeout
		 * @param[in] current_state				current state of the I2C communication
		 * @param[in] next_state				Next state of the I2C communication
		 * @param[in] state_specific_error		I2C Communication state_specific_error which need to be returned in
		 * status
		 * @param[out] i2c_status_t				Return the status,if any error while setting the state machine
		 */
		...
		/**
		 * @brief                   Timeout check when I2C Bus is busy state.
		 * @param[in] None
		 * @param[out] return_type  Returning the error while waiting for the timeout been elapsed.
		 */
		...
		/**
		 * @brief                   RX DMA ISR
		 * @param[in] handle		DMA handle which need to be triggered on receive complete
		 */
		...
		/**
		 * @brief                   TX DMA ISR
		 * @param[in] handle		DMA handle which need to be triggered on transmit complete
		 */
		 ...
		 * @param[in] object		pointer to STM32F2xx object call back is itended for.
		 */
		static void I2C_ISR_Static( void* object )
		{
			static_cast<uC_I2C*>( object )->I2C_ISR();	// I2C_ISR( object );
		}
		...
		/** @brief Get the base address of I2C channel */
		I2C_TypeDef* m_i2c;
		...
		/** @brief Mode of transfer over I2C bus - Synchronous or Asynchronous*/
		bool_t m_synchronous_type;

		/** @brief callback ptr need to be called when receive or send complete interrupt*/
		cback_func_t m_call_back_ptr;

		/** @brief callback param need used when receive or send complete interrupt*/
		cback_param_t m_cback_param;

		/** @brief I2C IO configuration*/
		uC_BASE_I2C_IO_STRUCT const* m_config;

		/** @brief I2C interrupt event vector number */
		IRQn_Type m_i2c_vector_number;

		/** @brief I2C vector object for I2C events*/
		Advanced_Vect* m_i2c_vector_obj;

		/** @brief I2C IO control structure*/
		uC_BASE_I2C_IO_PORT_STRUCT const* m_io_ctrl;

		/** @brief I2C speed type standard mode/fast mode */
		i2c_mode_t m_i2c_speed_type;

		/** @brief I2C communication state machine */
		i2c_states_t m_state_machine;

		/** @brief Data pointer to transmit the data*/
		uint8_t* m_tx_data;

		/** @brief Data pointer to receive the data*/
		uint8_t* m_rx_data;

		/** @brief Length of the data to be transferred on I2C Bus*/
		uint16_t m_transfer_length;

		/** @brief Reload Length of the data in case if its more than 255 bytes*/
		uint8_t m_reload_length;

		/** @brief Reload mode of the data in case if its more than 255 bytes*/
		bool_t m_reload_mode;
		...
		static const uint32_t I2C_TIMEOUT = ( 25U );
		...
		/** @brief Maximum length that can be sent at a time in non DMA mode */
		static const uint8_t MAX_RELOAD_LENGTH = 255;

		/** @brief I2C Clock frequency in MHz used in I2C Timing Calculation */
		static const uint32_t I2C_CLOCK_FREQ_25_MHZ = ( 25000000U );

		static const uint32_t I2C_CLOCK_FREQ_90_MHZ = ( 90000000U );

		static const uint32_t I2C_CLOCK_FREQ_100_MHZ = ( 100000000U );

		static const uint32_t I2C_CLOCK_FREQ_120_MHZ = ( 120000000U );
