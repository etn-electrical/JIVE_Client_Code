 ...
 *@n Justification: The pattern of __ICFEDIT_region_RAM_start__, __ICFEDIT_region_RAM_end__,
 * etc.
...
extern uint8_t __ICFEDIT_region_ITCMRAM_start__;
extern uint8_t __ICFEDIT_region_ITCMRAM_end__;
}
	...
	: m_cr_update_task( reinterpret_cast<CR_Tasker*>( NULL ) )
{
	...
	m_cr_update_task = new CR_Tasker( &Update_Static,
									  ...
									  CR_TASKER_IDLE_PRIORITY, "uC_RAM update" );
	...
	m_volatile_item_first.start = reinterpret_cast<uint8_t*>( 0x00000000U );	// start of MCU
																				// address space
	m_volatile_item_first.end = &__ICFEDIT_region_ITCMRAM_start__;
	...
	// This volatile region represents the address space starting just after
	// the end of Core Connected Memory and going up to but not including
	// __ICFEDIT_region_RAM_start__.  It should NEVER be unmarked as volatile.
	m_volatile_item_post_ccm.start = &__ICFEDIT_region_ITCMRAM_end__;
	m_volatile_item_post_ccm.start += 1U;		// end of CCM +1
	m_volatile_item_post_ccm.end = &__ICFEDIT_region_RAM_start__;
	m_volatile_item_post_ccm.end -= 1U;		// start of system SRAM - 1 [Separate statements to avoid
											// a PC-lint warning (428).]
	m_volatile_item_post_ccm.next = reinterpret_cast<volatile_item_t*>( nullptr );
	m_volatile_item_post_ccm.prev = reinterpret_cast<volatile_item_t*>( nullptr );
	...
	m_volatile_item_pointer.end =
		( m_volatile_item_pointer.start + sizeof( p_RunTimeRamChk ) ) - 1U;
	...
	m_volatile_item_buffer.end = ( m_volatile_item_buffer.start + sizeof( RunTimeRamBuf ) ) -
		1U;
	...
	m_volatile_item_last.start = &__ICFEDIT_region_RAM_end__;
	m_volatile_item_last.start += 1U;		// end of system SRAM +1
	m_volatile_item_last.end = reinterpret_cast<uint8_t*>( 0xFFFFFFFFU );		// end of MCU address
																				// space
...
void uC_RAM::Init_Volatile_List( volatile_item_t* initial_item )
{
	...
	Mark_As_Volatile( &m_volatile_item_post_ccm );
...
TestStatus uC_RAM::Runtime_Test( void )
{
	...
	p_RunTimeRamChk =
		reinterpret_cast<uint32_t*>( Check_Volatile( reinterpret_cast<uint8_t*>( p_RunTimeRamChk ),
													 BLOCKSIZE ) );
