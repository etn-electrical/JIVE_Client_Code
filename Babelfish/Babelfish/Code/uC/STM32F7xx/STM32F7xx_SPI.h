/**
 *****************************************************************************************
 *	@file STM32F7xx_SPI.h
 *
 *	@brief Specialized Queued SPI class for the STM32F7xx architecture
 *
 *	@details
 *
 ...
 *****************************************************************************************
 */
#ifndef STM32F7xx_SPI_H
#define STM32F7xx_SPI_H
...
/**
 ****************************************************************************************
 * @brief STM32F7xx_SPI class
 *
 * @details Realizes Queued SPI interface for the STM32F7xx architecture
 *
 ****************************************************************************************
 */
class STM32F7xx_SPI : public uC_Queued_SPI
{
	public:

		/**
		 * @brief                  Constructs a STM32F7xx_SPI object.
		 ...
		 * @param[in] datatype_size The data widths of the source and destination data are programmable
		 *                         through the PSIZE and MSIZE bits in the DMA_SxCR register.
		 */
		STM32F7xx_SPI( uint8_t channel, uint8_t no_slaves, bool_t master,
					   uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl, uC_USER_IO_STRUCT const* const pio[],
					   uint8_t datatype_size = DATA_SIZE_8_BIT );

		/** @brief Destructor to delete an instance of STM32F7xx_SPI class */
		~STM32F7xx_SPI( void );
	...
	protected:
		 ...
		 * @param[in] object - pointer to STM32F7xx object call back is itended for.
		 */
		static void DMA_ISR_Static( void* object )
		{
			static_cast<STM32F7xx_SPI*>( object )->DMA_ISR();
		}
		...
		{
			static_cast<STM32F7xx_SPI*>( object )->SPI_ISR();
		}
	...
	private:
		 ...
		 */
		STM32F7xx_SPI( void ) : m_spi( nullptr )
		{}
		...
		uint8_t m_datatype_size;

		static const uint16_t SPI_CR1_MASK = SPI_CR1_CRCL |
			SPI_CR1_SSM |
			SPI_CR1_SSI |
			SPI_CR1_BR |
			SPI_CR1_CPOL |
		...
		/**
		 * @brief Data size selection used for dma control registers.
		 */
		static const uint8_t DATA_SIZE_8_BIT = 1U;
		static const uint8_t DATA_SIZE_16_BIT = 2U;
		static const uint8_t DATA_SIZE_32_BIT = 4U;
