...
class STM32F4xx_SPI : public uC_Queued_SPI
{
	public:
		 ...
		 * @param[in] datatype_size	The data widths of the source and destination data are
		 *							programmable through the PSIZE and MSIZE bits in the
		 *							DMA_SxCR register
		 */
		STM32F4xx_SPI( uint8_t channel, uint8_t no_slaves, bool_t master,
					   uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl,
					   uC_USER_IO_STRUCT const* const pio[], uint8_t datatype_size = DATA_SIZE_8_BIT );
