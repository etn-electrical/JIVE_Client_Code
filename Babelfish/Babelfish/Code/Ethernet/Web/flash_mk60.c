...
static uint32_t ftfe_flash_command_sequence( volatile FTFE_FLASH_INTERNAL_STRUCT_PTR, uint8_t *,
	uint8_t, void *, uint32_t );
...
const FLASHX_BLOCK_INFO_STRUCT _flashx_kinetisN_block_map[] =
	{
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 4,
				(uint32_t)BSP_INTERNAL_FLASH_BASE, BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 4,
				(uint32_t)BSP_INTERNAL_FLASH_BASE + 1 * BSP_INTERNAL_FLASH_SIZE / 4,
				BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 4,
				(uint32_t)BSP_INTERNAL_FLASH_BASE + 2 * BSP_INTERNAL_FLASH_SIZE / 4,
				BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 4,
				(uint32_t)BSP_INTERNAL_FLASH_BASE + 3 * BSP_INTERNAL_FLASH_SIZE / 4,
				BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ 0, 0, 0 }
	};
...
const FLASHX_BLOCK_INFO_STRUCT _flashx_kinetisX_block_map[] =
	{
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 2,
				(uint32_t)BSP_INTERNAL_FLASH_BASE, BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ BSP_INTERNAL_FLASH_SIZE / BSP_INTERNAL_FLASH_SECTOR_SIZE / 2,
				(uint32_t)BSP_INTERNAL_FLASH_BASE + 1 * BSP_INTERNAL_FLASH_SIZE / 2,
				BSP_INTERNAL_FLASH_SECTOR_SIZE },
			{ 0, 0, 0 }
	};
	...
	)
{
	...
	{
		case 12:
			ftfe_ptr->FCCOBB = command_array[--count];
		case 11:
			ftfe_ptr->FCCOBA = command_array[--count];
		case 10:
			ftfe_ptr->FCCOB9 = command_array[--count];
		case 9:
			ftfe_ptr->FCCOB8 = command_array[--count];
		case 8:
			ftfe_ptr->FCCOB7 = command_array[--count];
		case 7:
			ftfe_ptr->FCCOB6 = command_array[--count];
		case 6:
			ftfe_ptr->FCCOB5 = command_array[--count];
		case 5:
			ftfe_ptr->FCCOB4 = command_array[--count];
		case 4:
			ftfe_ptr->FCCOB3 = command_array[--count];
		case 3:
			ftfe_ptr->FCCOB2 = command_array[--count];
		case 2:
			ftfe_ptr->FCCOB1 = command_array[--count];
		case 1:
			ftfe_ptr->FCCOB0 = command_array[--count];
		default:
			break;
	}
	...
	)
{
	...
	dev_spec_ptr->flash_execute_code_ptr = ftfe_init_ram_function( (char *)ftfe_ram_function,
		(char *)ftfe_ram_function_end );
#if PSP_MQX_CPU_IS_KINETIS
...
{
	FTFE_FLASH_INTERNAL_STRUCT_PTR dev_spec_ptr = (FTFE_FLASH_INTERNAL_STRUCT_PTR)dev_ptr
	...
	)
{
	...
	_mem_copy( (char *)ftfe_ram_function_start, ram_code_ptr,
		(char *)function_end - (char *)ftfe_ram_function_start );
	...
	)
{
	...
	if ( FTFE_OK
		!= ftfe_flash_command_sequence( dev_spec_ptr, command_array, 4, (void*)write_addr, size ) )
	{
		return FALSE;
	}
	...
	)
{
	...
	while ( size )
	{
		...
		/* call flash command sequence function to execute the command */
		if ( FTFE_OK
			!= ftfe_flash_command_sequence( dev_spec_ptr, command_array, 4 + FTFE_PHRASE_SIZE,
