...
#include "User_Management_Json.h"
...
static const uint16_t CHANGE_PWD_MASK =
	( ( 1U <<
	  ...
	  ( 1U << CUR_PWD_TAG_STATE ) | ( 1U << PWD_TAG_STATE ) | ( 1U << PWD_SET_EPOCH_TIME_TAG_STATE ) );
...
/**
 * @brief This Callback function is to parse the user account.
 * @details This function to parse the user account when /rs/users/accounts is requested.
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Parse_Accounts_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

/**
 * @brief This Callback function is to parse the user roles.
 * @details This function to parse the user account when /rs/users/roles is requested.
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Parse_Roles_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

/**
 * @brief This Callback function is to parse the user session.
 * @details This function to parse the user session when /rs/users/session is requested.
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Parse_Session_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

/**
 * @brief This function is to logout the user session.
 * @details This function to logout the user session.
 * @param[in] parser : Pointer to the hams parser.
 * @return true.
 */
bool User_Logout_Json( hams_parser* parser );

static uint32_t Users_Response_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Users_Accounts_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Users_Session_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Users_Roles_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Userid_Info_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Put_Post_User_Details_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static bool Is_UserName_Valid( uint8_t* srcptr, uint8_t length );

static uint32_t User_Delete_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t User_Role_Details_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

/**
 * @brief
 * @details This function is called when /rs/users/accounts OPTIONS is requested
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Users_Accounts_Options_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Parse_Users_Help_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint16_t pwd_complexity_check_true( pwd_complexity_pass* pwd_structure );

/**
 * @brief This Callback function is to indicate method not allowed error.
 * @details This function is to indicate method not allowed error.
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Method_Not_Allowed_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

/**
 * @brief
 * @details This function is called when /rs/users/accounts/curruser OPTIONS is requested
 * @param[in] parser : Pointer to the hams parser.
 * @param[in] data : Pointer to the data
 * @param[in] length : length of the data to be parsed
 * @return None.
 */
static uint32_t Users_Accounts_curruser_Options_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Curr_User_Put_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static uint32_t Init_Curr_User_Put_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );

static bool Validate_Date_Time_And_Convert_To_Numeric( char_t* str_time );
 ...
 * @return error code
 */
uint16_t Reset_Default_User_Json( uint16_t bitmap );
 ...
 * @return error code
 */
uint16_t Factory_Reset_Json( void );
 ...
 * @return None
 */
static uint32_t Options_User_Reset_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );
 ...
 * @return None
 */
static uint32_t Post_User_Reset_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length );
 ...
 * @return error code
 */
uint16_t Create_Default_User_Json( uint16_t bitmap, const uint8_t def_admin_userid );
		...
		if ( 0U != hams_check_headers_ended( data, length ) )
		{
			if ( ( HTTP_METHOD_GET == parser->method ) || ( HTTP_METHOD_HEAD == parser->method ) )
			{
				if ( 0U != rs->auth_level )
				{
					...
					parser->user_data_cb = static_cast<hams_data_cb>( Users_Response_Cb_Json );
				}
				else
				{
			...
			{
				Validate_Options_Cors_Headers( parser, rs, Users_Response_Cb_Json );
			}
			else
			{
				hams_respond_error( parser, ERROR_METHOD_NOT_ALLOWED );	/* Method not allowed */
			}
		}
		else
		{
			if ( ( length == 9U ) &&
				 ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/accounts", length ) ==
				   0 ) )
			{
				parser->user_data_cb = static_cast<hams_data_cb>( Parse_Accounts_Cb_Json );
			}
			else if ( ( length == 6U ) &&
					  ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/roles",
								 length ) == 0 ) )
			{
				parser->user_data_cb = static_cast<hams_data_cb>( Parse_Roles_Cb_Json );
			}
			else if ( ( length == 5U ) &&
					  ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/help",
								 length ) == 0 ) )
			{
				parser->user_data_cb = static_cast<hams_data_cb>( Parse_Users_Help_Cb_Json );
			}
			else if ( ( length == 8U ) &&
					  ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/session",
								 length ) == 0 ) )
			{
				parser->user_data_cb = static_cast<hams_data_cb>( Parse_Session_Cb_Json );
			}
			else
			{
...
static uint32_t Users_Response_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length )
{
	if ( HTTP_METHOD_OPTIONS == parser->method )
	{
		...
		hams_response_header( parser, COUNT_HTTP_HDRS, reinterpret_cast<const void*>( nullptr ), 0U );
	}
	else
	{
		http_server_req_state* rs = Http_Server_Get_State( parser );
		if ( 0U != hams_can_respond( data, length ) )
		{
			...
			if ( json_buffer != nullptr )
			{
				...
				Add_String_Member( json_buffer, "Users", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users", true );
				Add_String_Member( json_buffer, "Accounts", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users/accounts", false );
				Close_Json_Object( json_buffer, true );
				Add_String_Member( json_buffer, "Roles", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users/roles", false );
				Close_Json_Object( json_buffer, true );
				Add_String_Member( json_buffer, "Session", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users/session", false );
...
static uint32_t Parse_Session_Cb_Json( hams_parser* parser, const uint8_t* data, uint32_t length )
{
	if ( length != 1U )
	{
		if ( 0U != hams_check_headers_ended( data, length ) )
		{
			...
			{
				User_Logout_Json( parser );
				parser->response_code = 202U;
			}
			else if ( HTTP_METHOD_OPTIONS == parser->method )
			{
				Validate_Options_Cors_Headers( parser, rs, Users_Session_Cb_Json );
			}
			else
			{
	...
	if ( length != 1U )
	{
		...
		if ( 0U != hams_check_headers_ended( data, length ) )
		{
			if ( ( HTTP_METHOD_GET == parser->method ) || ( HTTP_METHOD_HEAD == parser->method ) )
			{
				...
				{
					parser->response_code = RESPONSE_OK;
					parser->user_data_cb = static_cast<hams_data_cb>( Users_Accounts_Cb_Json );
				}
				else
				{
					hams_respond_error( parser, ERROR_UNAUTHORIZED );
			...
			{
				Validate_Options_Cors_Headers( parser, rs, Users_Accounts_Cb_Json );
			}
						...
						( rs->user_id != 0U ) ) )
			{
				if ( parser->content_length != 0U )
				{
					parser->user_data_cb = static_cast<hams_data_cb>
						( Put_Post_User_Details_Cb_Json );
					rs->p.usermgmt.yjson = Http_Server_Alloc_Json_Parser( parser, 32U );
					...
					user_mgmt_data_t* meta_data =
						reinterpret_cast<user_mgmt_data_t*>( Http_Server_Alloc_Scratch( parser, num_blocks ) );
		...
		else
		{
				 ...
				 INT_ASCII_Conversion::str_to_uint8( ( data ), 1U, length - 1U, &user_id ) )
			{
				...
				else
				{
					...
					if ( HTTP_METHOD_DELETE == parser->method )
					{
						parser->user_data_cb = static_cast<hams_data_cb>( User_Delete_Cb_Json );
					}
					else if ( HTTP_METHOD_OPTIONS == parser->method )
					{
						parser->user_data_cb = static_cast<hams_data_cb>
							( Users_Accounts_Options_Json );
					}
					else if ( ( HTTP_METHOD_GET == parser->method ) ||
							  ( HTTP_METHOD_HEAD == parser->method ) )
					{
						parser->user_data_cb = static_cast<hams_data_cb>( Userid_Info_Cb_Json );
					}
					else if ( HTTP_METHOD_PUT != parser->method )
					{
						parser->user_data_cb = static_cast<hams_data_cb>
							( Method_Not_Allowed_Cb_Json );
					}
					else
					{	/* Misra Suppress */
					}
					  ...
					  ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/curruser", length ) == 0U ) )
			{
				...
				{
					parser->user_data_cb = static_cast<hams_data_cb>( Init_Curr_User_Put_Cb_Json );
				}
				else if ( HTTP_METHOD_OPTIONS == parser->method )
				{
					parser->user_data_cb =
						static_cast<hams_data_cb>( Users_Accounts_curruser_Options_Json );
				}
				else
				{
					parser->user_data_cb = static_cast<hams_data_cb>( Method_Not_Allowed_Cb_Json );
				}
			}
			else if ( ( length == 6U ) &&
					  ( strncmp( reinterpret_cast<const char_t*>( &data[0] ), "/reset", length ) == 0U ) )
			{
				...
				{
					parser->user_data_cb = static_cast<hams_data_cb>( Post_User_Reset_Cb_Json );
				}
				else if ( HTTP_METHOD_OPTIONS == parser->method )
				{
					parser->user_data_cb =
						static_cast<hams_data_cb>( Options_User_Reset_Cb_Json );
				}
				else
				{
					parser->user_data_cb = static_cast<hams_data_cb>( Method_Not_Allowed_Cb_Json );
	...
	else
	{
		if ( 0U != hams_can_respond( data, length ) )
		{
			...
			if ( json_buffer != nullptr )
			{
				...
				memset( json_buffer->json_buff, 0U, JSON_MAXBUFF_LEN );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, "UserAccountList", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users", true );
				Add_Integer_Member( json_buffer, "TotalUsers", total_users, true );
				Add_Integer_Member( json_buffer, "MaxAllowedUsers", MAX_NUM_OF_AUTH_USERS, true );
				Add_String_Member( json_buffer, "User", nullptr, false );
			...
			if ( USER_ROLE_ADMIN == rs->auth_level )
			{
				if ( 0U != user_count )
				{
					for ( user_loc = 0U; ( user_loc < MAX_NUM_OF_AUTH_USERS ); user_loc++ )
					{
						...
						{
							len = 0U;
							if ( m_user_account_handler->Get_User_Name( user_loc, j_dest_data,
																		( static_cast<uint8_t>( len ) ) ) )
							{
								if ( json_buffer != nullptr )
								{
									...
									Open_Json_Object( json_buffer );
									Add_String_Member( json_buffer, "user_name", ( char const* )j_dest_data, true );
									snprintf( formatted_buff, BUFFER_LEN, "/rs/users/accounts/%d", user_loc + 1U );
			...
			{
				len = 0U;
				if ( m_user_account_handler->Get_User_Name( rs->user_id - 1, j_dest_data,
															( static_cast<uint8_t>( len ) ) ) )
				{
					...
					if ( json_buffer != nullptr )
					{
						Open_Json_Object( json_buffer );
						Add_String_Member( json_buffer, "user_name", ( char const* )j_dest_data, true );
						snprintf( formatted_buff, BUFFER_LEN, "/rs/users/accounts/%d", rs->user_id );
						Add_String_Member( json_buffer, HREF, formatted_buff, false );
						Close_Json_Object( json_buffer, false );
					}
				}
			...
			if ( json_buffer != nullptr )
			{
				...
				Add_Integer_Member( json_buffer, "UserNameSizeLimit", User_Account::USER_NAME_LIMIT, true );
				Add_Integer_Member( json_buffer, "FullNameSizeLimit", FULL_NAME_LIMIT, false );
...
static uint32_t Put_Post_User_Details_Cb_Json( hams_parser* parser, const uint8_t* data,
											   uint32_t length )
{
	http_server_req_state* rs = Http_Server_Get_State( parser );
	struct yjson_put_json_state* ypx = reinterpret_cast<struct yjson_put_json_state*>( rs->p.usermgmt.yjson );
	uint16_t error_code = RESPONSE_OK;
	...
	{
		user_mgmt_data_t* usermgmt_p_data = reinterpret_cast<user_mgmt_data_t*>( rs->p.usermgmt.put_request );

		if ( parser->content_type != HTTP_CONTENT_TYPE_JSON )
		{
		...
		else
		{
			...
			result = jsmn_parse( &j_parser, ( char_t* ) p_msg_buffer, msgLen, tokens,
			...
			if ( result < 0 )
			{
				...
				{
					/* Failed to parse user account paramter */
			...
			for ( index = 1; index < result; index++ )
			{
				...
				if ( usermgmt_p_data->user_tag_rx.field.user_account_rx != 1U )
				{
					if ( jsoneq( ( char_t* )p_msg_buffer, &tokens[index],
								 ( reinterpret_cast<const char_t*>( User_info_Tags[USERACCOUNT_TAG_STATE] ) ) ) == 0 )
					{
				...
				else
				{
					...
					for ( uint32_t i = 1U; i < MAX_TAG_STATE; i++ )
					{
						if ( jsoneq( ( char_t* )p_msg_buffer, &tokens[index],  User_info_Tags[i] ) == 0 )
						{
							...
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->full_name[usermgmt_p_data->counter] = json_value[k];
									k++;
									usermgmt_p_data->counter++;
								}
							}
							else if ( USERNAME_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->user_name[usermgmt_p_data->counter] = json_value[k];
									k++;
									usermgmt_p_data->counter++;
								}
								error_code = RESPONSE_OK;
							...
							else if ( PWD_COMPLEXITY_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									if ( IS_DIGIT( json_value[k] ) )
									{
										...
										pwd_complexity *= 10U;
										pwd_complexity += ( json_value[k] - 0x30U );
							...
							else if ( PWD_TIMEOUT_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									if ( IS_DIGIT( json_value[k] ) )
									{
										...
										pwd_timeout *= 10U;
										pwd_timeout += ( json_value[k] - 0x30U );
							...
							else if ( CUR_PWD_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->cur_password[usermgmt_p_data->counter] = json_value[k];
									k++;
									usermgmt_p_data->counter++;
								}
							}
							else if ( PWD_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->password[usermgmt_p_data->counter] = json_value[k];
									usermgmt_p_data->counter++;
									k++;
								}
							}
							else if ( ROLE_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->role[usermgmt_p_data->counter] = json_value[k];
									usermgmt_p_data->counter++;
									k++;
								}
							}
							else if ( PWD_SET_EPOCH_TIME_TAG_STATE == ypx->put_state )
							{
								while ( json_value[k] != nullptr )
								{
									usermgmt_p_data->pwd_set_epoch_time[usermgmt_p_data->counter] = json_value[k];
									usermgmt_p_data->counter++;
									k++;
								}
							}
				...
				if ( ( index == result - 1 ) || ( error_code == ERROR_BAD_REQUEST ) )
				{
			...
			if ( MAX_TAG_STATE == ypx->put_state )
			{
				...
				if ( bitmap != User_Account::UNKNOWN_ERROR )
				{
					...
					{
						if ( ( bitmap && ( 1U << rs->p.usermgmt.req_user_id - 1U ) ) == 0U )
						{
								   ...
								   1U ) )
							{
								if ( error_code == RESPONSE_OK )
								{
																	...
																	cur_pwd_rx == 1U ) )
									{
										...
										m_user_account_handler->Get_Password( ( rs->user_id - 1U ), cur_pwd,
																			  static_cast<uint8_t>( data_len ) );
										uint16_t string_length = BF_Lib::Get_String_Length(
											reinterpret_cast<uint8_t*>( usermgmt_p_data->cur_password ),
											( PWD_BASE64_SIZE_LIMIT + 1U ) );

										if ( BASE64_DECODING_ERR_OK !=
											 Base64::Decode( ( uint8_t* ) usermgmt_p_data->cur_password,
															 string_length, j_dest_data,
															 &decoded_length, 0U ) )
										{
					...
					if ( error_code == RESPONSE_OK )
					{
						...
						{
							uint16_t str_len =
								BF_Lib::Get_String_Length( reinterpret_cast<uint8_t*>( usermgmt_p_data->full_name ),
							...
							else
							{
								...
								{
									if ( IS_ALPHA( j_dest_data[0] ) )
									{
										memcpy( usermgmt_p_data->full_name, j_dest_data,
												decoded_length );
										usermgmt_p_data->full_name[decoded_length] = 0U;		/* null character  */
									}
									else
									{
										error_code = ERROR_BAD_REQUEST;
							 ...
							 ( usermgmt_p_data->user_tag_rx.field.user_name_rx == 1U ) )
						{
							if ( BASE64_DECODING_ERR_OK !=
								 Base64::Decode(
									 ...
									 BF_Lib::Get_String_Length( reinterpret_cast<uint8_t*>( usermgmt_p_data->
																							user_name ),
																( USERNAME_BASE64_SIZE_LIMIT + 1U ) ), j_dest_data,
									 &decoded_length, 0U ) )
							{
								error_code = E422_DECRYPTION_FAILED;
							}
							else
							{
								...
								{
									if ( Is_UserName_Valid( j_dest_data, decoded_length ) )
									{
										memcpy( usermgmt_p_data->user_name, j_dest_data, decoded_length );
										usermgmt_p_data->user_name[decoded_length] = 0U;		/* null  character */
									}
									else
									{
										error_code = ERROR_BAD_REQUEST;
							 ...
							 ( usermgmt_p_data->user_tag_rx.field.pwd_rx == 1U ) )
						{
							...
							else
							{
								...
								{
									memcpy( usermgmt_p_data->password, j_dest_data,
											decoded_length );
									usermgmt_p_data->password[decoded_length] = 0U;			/* null character */
								}
								else
								{
									error_code = ERROR_BAD_REQUEST;
							 ...
							 ( usermgmt_p_data->user_tag_rx.field.role_rx == 1U ) )
						{
							...
							for ( uint32_t temp_counter = 0U;
								  ( ( temp_counter < NUM_OF_AUTH_LEVELS ) &&
									( error_code == ERROR_BAD_REQUEST ) ); temp_counter++ )
							{
						...
						if ( error_code == RESPONSE_OK )
						{
							if ( usermgmt_p_data->user_tag_rx.field.user_name_rx == 1U )
							{
								for ( uint8_t user_id = 0U;
									  ...
									  ( RESPONSE_OK == error_code ); user_id++ )
								{
							...
							if ( error_code == RESPONSE_OK )
							{
								...
								{										/* Perform password complexity check first */
									...
									complexity_structure.full_name =
										( uint8_t* )usermgmt_p_data->full_name;
									if ( ( error_code == RESPONSE_OK ) &&
										 ( 0U == usermgmt_p_data->user_tag_rx.field.user_name_rx ) )
									{
										data_len = 0U;
										if ( m_user_account_handler->Get_User_Name(
												 user_loc,
									...
									if ( parser->method == HTTP_METHOD_POST )
									{
										complexity_structure.cur_pwd = nullptr;
									}
									else
									{
										data_len = 0U;
								...
								if ( RESPONSE_OK == error_code )
								{
										   ...
										   1U ) )
									{
										...
										if ( m_user_account_handler->Set_Password_Aging(
												 user_loc,
												 reinterpret_cast<uint8_t*>( &usermgmt_p_data->pwd_timeout ),
												 ( static_cast<uint8_t>( data_len ) ) ) )
										{
											error_code = RESPONSE_OK;
										}
										 ...
										 ( usermgmt_p_data->user_tag_rx.field.pwd_rx == 1U ) )
									{
											 ...
											 ( complexity_structure.cur_pwd != nullptr ) )
										{
											...
											/* check that logged user is admin or some other role based user */
											...
											/*
											   log events based on whether admin changes pwd of role based user or role
											      based user does it for itself
											 */
											if ( rs->user_id - 1U == ( ADMIN_USER_INDEX ) )
											{
												if ( m_user_log_audit_capture_cb !=
									...
									if ( error_code == RESPONSE_OK )
									{
										...
										if ( parser->method == HTTP_METHOD_POST )
										{
											...
											ip_address_setting.port =
												parser->remote_port;
											/* resetting the last login date, time and pwd set date value to default
											   value for newly created user */
											uint32_t time = 0U;
		...
		if ( ( error_code != RESPONSE_OK ) || ( MAX_TAG_STATE == ypx->put_state ) )
		{
			msg_cnt = 1;
		...
		if ( length == 0U )
		{
			...
			{	/* magic number for curr_user to be verified and update req_user_id */
					   ...
					   ( rs->p.usermgmt.req_user_id == rs->user_id ) ) && ( rs->user_id != 0U ) )
				{
						...
						if ( bitmap != User_Account::UNKNOWN_ERROR )
						{
								 ...
								 0U )
							{
								...
								if ( json_buffer != nullptr )
								{
									...
									Open_Json_Object( json_buffer );
									Add_String_Member( json_buffer, "UserAccount", nullptr, false );
									Open_Json_Object( json_buffer );
									Add_Integer_Member( json_buffer, "user_id", rs->p.usermgmt.req_user_id, true );
									snprintf( formatted_buff, BUFFER_LEN, "/rs/users/accounts/%d",
											  rs->p.usermgmt.req_user_id );
									...
									{
										error_code = RESPONSE_OK;
										Add_String_Member( json_buffer, "FullName", ( char const* )j_dest_data, true );
									}
									else
									{
										error_code = ERROR_BAD_REQUEST;
									...
									{
										error_code = RESPONSE_OK;
										Add_String_Member( json_buffer, "UserName", ( char const* )j_dest_data, true );
									...
									buffer_length = 0U;
									if ( ( error_code == RESPONSE_OK ) &&
										 m_user_account_handler->Get_Password_Complexity( rs->p.usermgmt.req_user_id -
																						  1U,
																						  &pwd_complexity,
																						  ( static_cast<uint8_t>(
																								buffer_length ) ) ) )
									{
										...
										Add_Integer_Member( json_buffer, "PwdComplexity", pwd_complexity, true );
									...
									buffer_length = 0U;
									if ( ( error_code == RESPONSE_OK ) &&
										 m_user_account_handler->Get_Password_Aging( rs->p.usermgmt.req_user_id - 1U,
																					 reinterpret_cast<uint8_t*>( &
																												 pwd_aging ),
																					 ( static_cast<
																						   uint8_t>
									...
									{
										error_code = RESPONSE_OK;
										Add_Integer_Member( json_buffer, "PwdTimeoutDays", pwd_aging, true );
									}
									else
									{
										error_code = ERROR_BAD_REQUEST;
									...
									if ( error_code == RESPONSE_OK )
									{
										...
										if ( role_id != User_Account::UNKNOWN_ERROR )
										{
												  ...
												  counter++ )
											{
												...
												{
													Add_String_Member( json_buffer, "Role", nullptr, false );
													Open_Json_Object( json_buffer );
													snprintf( formatted_buff, BUFFER_LEN, "/rs/users/roles/%d",
															  counter + 1 );
													Add_String_Member( json_buffer, HREF, formatted_buff, true );
													Add_String_Member( json_buffer, "text",
																	   auth_struct[counter].role_name, false );
													Close_Json_Object( json_buffer, true );
										 ...
										 m_user_account_handler->Get_Password_Set_Epoch_Time( rs->p.
																							  ...
																							  (
																								  ...
																								  >(
																									  buffer_length ) ) ) )
									{
										Add_Integer_Member( json_buffer, "PwdSetEpochTime", date_format.date_time,
															true );
									}
									else
									{
										 ...
										 m_user_account_handler->Get_Last_Login_Epoch_Time( rs->p.
																							...
																							(
																								...
																								>(
																									buffer_length ) ) ) )
									{
										Add_Integer_Member( json_buffer, "LastLoginTime", date_format.date_time, true );

										uint8_t failedattemptCnt = m_user_session_handler->Get_Failed_Attempt_Count(
											rs->p.usermgmt.req_user_id - 1U );

										Add_Integer_Member( json_buffer, "FailedAttemptCount", failedattemptCnt, true );
										...
										Add_Integer_Member( json_buffer, "ChangePwd", trigger_pwd_change, false );
										Multiple_Close_Object( json_buffer, 2 );
									}
									Http_Server_Print_Dynamic( parser, json_buffer->json_buff );
									Ram::De_Allocate( json_buffer );
					...
					{
						error_code = ERROR_REQUEST_ENTITY_TOO_LARGE;	/* Request Entity is too large */
					}
					if ( error_code != RESPONSE_OK )
					{
						hams_respond_error( parser, error_code );
				...
				{
					hams_respond_error( parser, ERROR_UNAUTHORIZED );	/* Authorization required */
				}
			}
			else
			{
	...
	if ( length != 1U )
	{
		if ( 0U != hams_check_headers_ended( data, length ) )
		{
			if ( ( HTTP_METHOD_GET == parser->method ) || ( HTTP_METHOD_HEAD == parser->method ) )
			{
				...
				{
					parser->response_code = RESPONSE_OK;
					parser->user_data_cb = static_cast<hams_data_cb>( Users_Roles_Cb_Json );
				}
				else
				{
					hams_respond_error( parser, ERROR_UNAUTHORIZED );
			...
			{
				Validate_Options_Cors_Headers( parser, rs, Users_Roles_Cb_Json );
			}
			else
			{
				hams_respond_error( parser, ERROR_METHOD_NOT_ALLOWED );	/* Method not allowed */
		...
		else
		{
				 ...
				 INT_ASCII_Conversion::str_to_uint8( ( data ), 1U, length - 1U, &user_role ) )
			{
				...
				else
				{
					...
					parser->user_data_cb = static_cast<hams_data_cb>( User_Role_Details_Cb_Json );
				}
			}
			else
			{
	...
	else
	{
		...
		if ( 0U != hams_can_respond( data, length ) )
		{
			...
			if ( json_buffer != nullptr )
			{
				...
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, "UserRoles", nullptr, false );
				Open_Json_Object( json_buffer );
				Add_String_Member( json_buffer, HREF, "/rs/users/roles", true );

				Add_String_Member( json_buffer, "Role", nullptr, false );
				Open_Json_Array( json_buffer );
				for ( uint8_t role_id = 0U; role_id < NUM_OF_AUTH_LEVELS; role_id++ )
				{
					snprintf( formatted_buff, BUFFER_LEN, "/rs/users/roles/%d", role_id + 1U );
					Open_Json_Object( json_buffer );
					Add_String_Member( json_buffer, "role_name", auth_struct[role_id].role_name, true );
					Add_String_Member( json_buffer, HREF, formatted_buff, false );

					if ( role_id < NUM_OF_AUTH_LEVELS - 1 )
					{
						Close_Json_Object( json_buffer, true );
					}
					else
			...
			if ( ( HTTP_METHOD_GET == parser->method ) || ( HTTP_METHOD_HEAD == parser->method ) )
			{
				if ( 0U != rs->auth_level )
				{
					...
					if ( json_buffer != nullptr )
					{
						...
						Open_Json_Object( json_buffer );
						Add_String_Member( json_buffer, "UserRole", nullptr, false );
						Open_Json_Object( json_buffer );
						Add_Integer_Member( json_buffer, "role_id", rs->p.usermgmt.req_user_role, true );
						snprintf( formatted_buff, BUFFER_LEN, "/rs/users/roles/%d", rs->p.usermgmt.req_user_role );
						Add_String_Member( json_buffer, HREF, formatted_buff, true );
						Add_String_Member( json_buffer, "Name", auth_struct[rs->p.usermgmt.req_user_role - 1].role_name,
										   true );
						Add_Integer_Member( json_buffer, "RoleLevel",
											auth_struct[rs->p.usermgmt.req_user_role - 1].role_level, true );

						Add_String_Member( json_buffer, "Description",
										   auth_struct[rs->p.usermgmt.req_user_role - 1].role_desc, false );
...
/**
 * @brief This function validate the username
 * @details This function validate the username.
 * @param[in] srcptr : Pointer to the username.
					 ...
					 ( HTTP_METHOD_HEAD == parser->method ) )
				{
					if ( 0U != rs->auth_level )
					{
						...
						if ( json_buffer != nullptr )
						{
							...
							memset( json_buffer->json_buff, 0U, JSON_MAXBUFF_LEN );
							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "UsersHelp", nullptr, false );
							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, HREF, "/rs/users/help", true );
							Add_String_Member( json_buffer, "FullNameLimit", "32", true );
							Add_String_Member( json_buffer, "UserNameLimit", "20", true );
							Add_String_Member( json_buffer, "PwdLimit", "20", true );
							Add_String_Member( json_buffer, "PwdComplexityLevels", nullptr, false );
							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "Complexity", nullptr, false );
							Open_Json_Array( json_buffer );

							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "Level", "0", true );
							Add_String_Member( json_buffer, "MinLen", "6", true );
							Add_String_Member( json_buffer, "Alpha", "0", true );
							Add_String_Member( json_buffer, "UpperCase", "0", true );
							Add_String_Member( json_buffer, "LowerCase", "0", true );
							Add_String_Member( json_buffer, "Num", "0", true );
							Add_String_Member( json_buffer, "SplChar", "0", false );
							Close_Json_Object( json_buffer, true );

							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "Level", "1", true );
							Add_String_Member( json_buffer, "MinLen", "8", true );
							Add_String_Member( json_buffer, "Alpha", "1", true );
							Add_String_Member( json_buffer, "UpperCase", "0", true );
							Add_String_Member( json_buffer, "LowerCase", "0", true );
							Add_String_Member( json_buffer, "Num", "1", true );
							Add_String_Member( json_buffer, "SplChar", "0", false );
							Close_Json_Object( json_buffer, true );

							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "Level", "2", true );
							Add_String_Member( json_buffer, "MinLen", "12", true );
							Add_String_Member( json_buffer, "Alpha", "1", true );
							Add_String_Member( json_buffer, "UpperCase", "1", true );
							Add_String_Member( json_buffer, "LowerCase", "0", true );
							Add_String_Member( json_buffer, "Num", "1", true );
							Add_String_Member( json_buffer, "SplChar", "1", false );
							Close_Json_Object( json_buffer, true );

							Open_Json_Object( json_buffer );
							Add_String_Member( json_buffer, "Level", "3", true );
							Add_String_Member( json_buffer, "MinLen", "16", true );
							Add_String_Member( json_buffer, "Alpha", "2", true );
							Add_String_Member( json_buffer, "UpperCase", "1", true );
							Add_String_Member( json_buffer, "LowerCase", "1", true );
							Add_String_Member( json_buffer, "Num", "1", true );
							Add_String_Member( json_buffer, "SplChar", "2", false );
							Close_Json_Object( json_buffer, false );
							Close_Json_Array( json_buffer, false );
							Multiple_Close_Object( json_buffer, 3 );
	...
	if ( length == 0U )
	{
		...
		if ( rs->user_id != 0U )
		{
			...
			if ( parser->content_type == HTTP_CONTENT_TYPE_JSON )
			{
				if ( parser->content_length != 0U )
				{
					parser->user_data_cb = static_cast<hams_data_cb>( Curr_User_Put_Cb_Json );
					rs->p.usermgmt.yjson = Http_Server_Alloc_Json_Parser( parser, 32U );

					/* Allocate ourselves a block for JSON parsing. */
					uint8_t num_blocks = ( ( sizeof( login_date_time_t ) ) + ( HTTP_POOL_BLOCK_SIZE - 1U ) ) /
						HTTP_POOL_BLOCK_SIZE;
					login_date_time_t* meta_data =
						reinterpret_cast<login_date_time_t*>( Http_Server_Alloc_Scratch( parser, num_blocks ) );
					if ( meta_data != nullptr )
					{
						rs->p.usermgmt.put_request = ( void* )meta_data;
					}
				...
				{
					error_code = ERROR_LENGTH_REQUIRED;
				}
			}
			else
			{
...
{
	http_server_req_state* rs = Http_Server_Get_State( parser );
	struct yjson_put_json_state* ypx = reinterpret_cast<struct yjson_put_json_state*>( rs->p.usermgmt
																					   .yjson );
	uint16_t error_code = RESPONSE_OK;
	jsmn_parser j_parser;
	int16_t result = 0;
	...
	if ( 0U != hams_has_uri_part( data, length ) )
	{
		login_date_time_t* usermgmt_p_data = reinterpret_cast<login_date_time_t*>( rs->p.usermgmt.put_request );
		if ( msg_cnt == 1 )
		{
			p_msg_buffer = reinterpret_cast<char_t*>( Ram::Allocate( JSON_MAXBUFF_LEN ) );
			if ( p_msg_buffer != nullptr )
		...
		if ( result < 0 )
		{
			...
			{
				/* Failed to parse current user paramter */
				error_code = E400_JSON_SYNTAX_ERROR;
			}
		}

		usermgmt_p_data->counter = 0U;
		uint16_t token_size = tokens[index].end - tokens[index].start;
		while ( ypx->put_state < PUT_CURR_USER_COMPLETE_TAG_STATE )
		{
			for (index = 1; index < result; index++)
			{
				if ( jsoneq( ( char_t* )p_msg_buffer, &tokens[index],
							 reinterpret_cast<const char_t*>( user_date_time_Tags[ypx->put_state] ) ) == 0 )
				{
					if ( PUT_CURR_USER_TIME_TAG_STATE == ( ypx->put_state ) )
					{
						error_code = RESPONSE_OK;
						token_size = tokens[index + 1U].end - tokens[index + 1U].start;
						memset( json_value, 0U, JSON_VALUE_LEN );
						strncpy( json_value, p_msg_buffer + tokens[index + 1U].start, token_size );
						while ( json_value[k] != nullptr )
						{
							usermgmt_p_data->login_time[usermgmt_p_data->counter] = json_value[k];
							k++;
							usermgmt_p_data->counter++;
						}
						if ( usermgmt_p_data->counter > time_tag_content_len[ypx->put_state] )
						{
							error_code = ERROR_BAD_REQUEST;
						}
					}
		...
		if ( PUT_CURR_USER_COMPLETE_TAG_STATE == ypx->put_state )
		{
			...
			if ( bitmap != User_Account::UNKNOWN_ERROR )
			{
				if ( ( bitmap && ( 1 << rs->p.usermgmt.req_user_id - 1 ) ) ==
					 0U )
				{
					error_code = STATUS_NOT_FOUND;
				}
		...
		if ( ( error_code != RESPONSE_OK ) || ( PUT_CURR_USER_MAX_TAG_STATE == ypx->put_state ) )
		{
			msg_cnt = 1;
			Ram::De_Allocate( p_msg_buffer );
			Http_Server_Free( rs->p.usermgmt.put_request );
 ...
 *****************************************************************************************
 */
bool Validate_Date_Time_And_Convert_To_Numeric( char_t* str_time )
{
								  ...
								  ( char const* )pwd_structure->user_name ) ) ) ) )
		{
					...
					( IS_SPACE( pwd_structure->new_pwd[new_pwd_len - 1U] ) ) ) )
			{
				...
				switch ( pwd_structure->pwd_complexity_level )
				{
					...
					default:
						/* The complexity level is invalid and is not implemented */
						return_val = E422_PWD_COMPLEXITY_VIOLATION;
						break;
				}
			}
		}
...
static uint16_t Validate_Role_Change_or_Delete_Condition( uint8_t req_user_index, uint16_t bitmap )
{
			 ...
			 &len, 0U ) )
	{
		...
		{
			for ( uint32_t i = 0U; ( i < MAX_NUM_OF_AUTH_USERS ) && ( error_code == RESPONSE_OK ); i++ )
			{
	...
	if ( ( error_code == RESPONSE_OK ) && ( num_admin_user < 2U ) )
	{
...
/*
 *****************************************************************************************
 * See function declaration for function definition.
 *****************************************************************************************
 */
static uint32_t Verify_Password( uint8_t user_loc, user_mgmt_data_t* usermgmt_p_data, uint8_t data_len )
{
	...
	uint16_t error_code = ERROR_BAD_REQUEST;

	// Get Default user name for this user
	ret_status = m_user_account_handler->Get_Default_User_Name( user_loc, j_dest_data, 0U );
	if ( true == ret_status )
	{
		...
		ret_status = BF_Lib::String_Compare( j_dest_data, reinterpret_cast<const uint8_t*>( "" ), 1U );
		if ( false == ret_status )
		{
	...
	if ( RESPONSE_OK == error_code )
	{
		...
		ret_status =
			m_user_account_handler->Set_Password( user_loc, reinterpret_cast<uint8_t*>( usermgmt_p_data->password ),
												  static_cast<uint8_t>( data_len ) );
		...
		if ( true == ret_status )
		{
			...
			ret_status =
				m_user_account_handler->Get_Password_Complexity( user_loc,
			...
			if ( true == ret_status )
			{
				...
				ret_status =
					m_user_account_handler->Set_Password_Complexity( user_loc,
																	 reinterpret_cast<uint8_t*>( &pwd_complexity ),
																	 static_cast<uint8_t>( data_len ) );
		...
		else
		{
			...
			if ( length >= 2U )
			{
					 ...
					 INT_ASCII_Conversion::str_to_uint8( ( data ), 1U, length - 1U, &recv_reset_val ) )
				{
					...
					if ( ( recv_reset_val < RESET_DEFAULT_USER_ACC ) || ( recv_reset_val > RESET_ALL_USERS_ACC ) )
					{
						hams_respond_error( parser, ERROR_BAD_REQUEST );
					}
				}
	...
	if ( 0U != hams_has_uri_part( data, length ) )
	{
		...
		else
		{
			...
			if ( result < 0 )
			{
				...
				{
					/* Failed to parse user reset parameter  */
					error_code = E400_JSON_SYNTAX_ERROR;
				}
			}
			...
			for ( uint32_t i = 0U; ( ( i <= MAX_RESET_USER_TAG ) ); i++ )
			{
				for (index = 1; ( ( index < result ) ); index++)
				{
					if ( jsoneq( ( char_t* )p_msg_buffer, &tokens[index],
								 ( reinterpret_cast<const char_t*>( RESET_USERS_TAGS[i] ) ) ) == 0 )
					{
						if ( ( usermgmt_p_data->user_tag_rx.all & ( 1U << i ) ) == 0U )
						{
							ypx->put_state = i;
							...
							if ( LOGGEDIN_PWD_TAG_DEF == ypx->put_state )
							{
								...
								if ( ++usermgmt_p_data->counter > tag_content_len[ypx->put_state] )
								{
									error_code = ERROR_BAD_REQUEST;						// needs to be moved to
																						// json_value[k]
								}
							}
						}
			...
			if ( MAX_RESET_USER_TAG == ypx->put_state )
			{
				...
				if ( bitmap != User_Account::UNKNOWN_ERROR )
				{
									...
									else
									{
										if ( ( decoded_length !=
											 ...
											 ( 0U != strncmp( reinterpret_cast<char_t*>( cur_pwd ),
															  reinterpret_cast<char_t*>( j_dest_data ),
															  BF_Lib::Get_String_Length(
																  reinterpret_cast<uint8_t*>( cur_pwd ),
																  (
																	  CUR_PWD_BASE64_SIZE_LIMIT ) ) ) ) )
										{
											error_code = 403U;
						 ...
						 ( RESET_DEFAULT_USER_ACC == rs->p.usermgmt.reserved ) )
					{
						...
						error_code = Reset_Default_User_Json( bitmap );
						if ( RESPONSE_OK == error_code )
						{
							// Set response code
							rs->flags |= REQ_FLAG_NO_DYN_PARSER_SPACE;
					...
					// Check for reset value received in URI
					else if ( ( error_code == RESPONSE_OK ) &&
							  ( RESET_ALL_USERS_ACC == rs->p.usermgmt.reserved ) )
					{
						// Function call to reset default user password to default password
						error_code = Factory_Reset_Json();
						if ( RESPONSE_OK == error_code )
						{
							// Set response code
							rs->flags |= REQ_FLAG_NO_DYN_PARSER_SPACE;
					...
					if ( error_code == RESPONSE_OK )
					{
						...
						ip_address_setting.ip_address = parser->general_use_reg;
						ip_address_setting.port = parser->remote_port;
						log_param[User_Account::USER_NAME_LIMIT + 1U] = ip_address_setting.ip_address &
							0xffU;
						log_param[User_Account::USER_NAME_LIMIT +
		...
		if ( ( error_code != RESPONSE_OK ) || ( MAX_RESET_USER_TAG == ypx->put_state ) )
		{
			msg_cnt = 1;
			Ram::De_Allocate( p_msg_buffer );
			Http_Server_Free( rs->p.usermgmt.put_request );
...
uint16_t Reset_Default_User_Json( uint16_t bitmap )
{
	...
	// If default admin is found, error code will be changed to RESPONSE_OK OK
	uint16_t error_code = ERROR_INTERNAL_SERVER;
	...
	for ( cur_user_id = 0U; cur_user_id < MAX_NUM_OF_AUTH_USERS; cur_user_id++ )
	{
		if ( ( found_def_user_id == false ) &&
			 ( m_user_account_handler->Get_Default_User_Name( cur_user_id, j_dest_data, 0U ) ) )
		{
			// Check if user name exists for this user
			if ( false == BF_Lib::String_Compare( j_dest_data, reinterpret_cast<const uint8_t*>( "" ), 1U ) )
			{
			 ...
			 ( nullptr != m_user_account_handler->Get_User_Name( cur_user_id, cur_user_name, 0U ) ) )
		{
			...
			if ( true ==
				 BF_Lib::String_Compare( cur_user_name, j_dest_data,
	...
	{
		error_code = Create_Default_User_Json( bitmap, def_admin_userid );
	}
...
uint16_t Create_Default_User_Json( uint16_t bitmap, const uint8_t def_admin_userid )
{
	...
	if ( true == empty_found )
	{
		...
		if ( true == m_user_account_handler->Get_Default_User_Fullname( def_admin_userid, j_dest_data, 0U ) )
		{
			string_len = BF_Lib::Get_String_Length( j_dest_data, ( FULL_NAME_LIMIT + 1U ) );
			ret_status = m_user_account_handler->Set_User_Fullname( user_loc, j_dest_data, string_len );
			if ( true == ret_status )
			{
				ret_status = m_user_account_handler->Get_Default_User_Name( def_admin_userid, j_dest_data, 0U );
				if ( true == ret_status )
				{
					string_len = BF_Lib::Get_String_Length( j_dest_data, ( USERNAME_BASE64_SIZE_LIMIT + 1U ) );
					ret_status = m_user_account_handler->Set_User_Name( user_loc, j_dest_data, string_len );
					if ( true == ret_status )
					{
...
uint16_t Factory_Reset_Json( void )
{
	...
	error_code = Reset_Default_User_Json( 0U );
	return ( error_code );
}
