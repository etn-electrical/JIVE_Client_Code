...
class OS_FIFO
{
	public:
		...
		OS_FIFO( BF_Lib::MBASE queue_length, BF_Lib::MBASE item_size, bool is_mutex = false )
		{
			...
			{
				m_buffer_start = reinterpret_cast<int8_t*>( Ram::Allocate( item_size * queue_length,
																		   false, 0 ) );
			}
			else
			{
		 ...
		 * @param p_data: Pointer to buffer to receive the item.
		 ...
		 */
		inline void Get( int8_t* p_data )
		{
			if ( m_num_items_in_queue > 0 )
			{
			...
			else
			{
				/* Can not use BF_ASSERT() call here because loop is getting form
				   because of file inclusion. BF_ASSERT() uses OS_Task file and OS_Task
				   uses OS_FIFO file.
				 */
#ifndef ESP32_SETUP
				__asm( "BKPT #0\n" );
		 ...
		 * an item of the size specified at construction.
		 */
		inline void Peek( int8_t* p_data )
		{
#if CONFIG_USE_MUTEXES == 1
			if ( !m_is_mutex )
			{
		 ...
		 */
		inline void Set( const int8_t* p_data, bool copy_position = SEND_TO_BACK )
		{
			if ( m_num_items_in_queue < m_queue_length )
			{
				m_num_items_in_queue++;
#if CONFIG_USE_MUTEXES == 1
				if ( !m_is_mutex )
		...
		{
			m_pMutexHolder = reinterpret_cast<int8_t*>( task );
		}
		...
		{
			return ( reinterpret_cast<OS_Task*>( m_pMutexHolder ) );
		}
