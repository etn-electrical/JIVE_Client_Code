...
#ifndef ESP32_SETUP
		// TODO_ESP32_REWORK: LTK-8498 This conditional compilation needs to be removed once Watchdog support is
		// available in ESP32
 ...
 */
#ifdef ESP32_SETUP
void IRAM_ATTR CR_Tasker::Tick_Event( void )
{
	/* We are removing disable/enable global interrupts as it confilct with in built ESP32 SPI-FLASH Read/Write
	   Operations. */
...
{
	// Flag to check weather we need to execute the task or not
	...
	m_current_coroutine = reinterpret_cast<CR_Tasker*>( m_ready_cr_lists[m_cr_top_ready_priority]->Get_Next() );

	// Check for nullptr
	if ( m_current_coroutine != nullptr )
	{
		// Check if suspend bit is set for this task
		if ( BF_Lib::Bit::Test( m_current_coroutine->m_priority, m_current_coroutine->CR_TASKER_SUSPEND_BIT ) )
		{
			Push_TGINT();
			// Check if suspend bit is set for this task
			if ( BF_Lib::Bit::Test( m_current_coroutine->m_priority, m_current_coroutine->CR_TASKER_SUSPEND_BIT ) )
			{
				Remove_From_Any_List( m_current_coroutine->m_generic_list_item );
				Remove_From_Any_List( m_current_coroutine->m_event_list_item );
				BF_Lib::Bit::Set( m_current_coroutine->m_priority, CR_TASKER_SUSPENDED_PRI_BIT );
				BF_Lib::Bit::Clr( m_current_coroutine->m_priority, CR_TASKER_SUSPEND_BIT );
			}
			else
			{
				execute_task = true;
	...
	// Ensure that execute_task flag is set to true only after checking m_current_coroutine is not NULL
	if ( true == execute_task )
	{
...
#ifndef ESP32_SETUP
	// TODO_ESP32_REWORK: LTK-8498 :This conditional compilation needs to be removed once Watchdog support is available
	// in ESP32
