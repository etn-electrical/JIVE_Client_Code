...
void OSCR_Mon::Checker_Task( CR_Tasker* cr_task, CR_TASKER_PARAM param )
{
...
#ifdef USE_FULL_RTOS
	uint32_t os_priority_run_times[OS_TASK_PRIORITY_MAX] = {0};
#endif
	...
	while ( true )
	{
		...
		OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "\n\n***** Update - CR_Tasker *****\n" );
		for ( uint8_t priority = CR_TASKER_PRIORITY_0;
			  priority < CR_TASKER_MAX_PRIORITIES; priority++ )
		{
			OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "Priority %d Time (us): %d\n", priority,
							cr_priority_run_times[priority] );
		}
		OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "Total Time (us): %d\n", total_time );
		...
		{
#ifdef OSCR_MON_PRINT_ADVANCED_TASK_DETAILS
			OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "0x%.8X, 0x%.8X, %d, %d, %d, %d, %d, %d, %s\n",
			...
			CR_Tasker_Delay( cr_task, REPORT_INTERPRINT_TIME_DELAY );
#else
			OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "%d, %d, %d, %d, %s\n",
							cr_task_report.max_run_time, cr_task_report.filtered_run_time,
		...
		{
#ifdef OSCR_MON_PRINT_ADVANCED_TASK_DETAILS
			OS_DEBUG_PRINT( BF_DBG_MSG_DEBUG, "0x%.8X, %14d, %7d, %7d, %7d, %7d, %18d, %8d, %s\n",
							os_task_report.top_of_stack, os_task_report.max_stack_usage,
							os_task_report.times_run,
