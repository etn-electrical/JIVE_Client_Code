...
class CR_Tasker
{
	public:
		...
		void Suspend_And_Reset( void )
		{
			...
			{
				m_priority = ( m_priority | ( 1U << CR_TASKER_SUSPENDED_PRI_BIT ) );
	...
	private:
		...
		{
			if ( cr_task->m_priority > m_cr_top_ready_priority )
			{
				m_cr_top_ready_priority = cr_task->m_priority;
			}
			m_ready_cr_lists[cr_task->m_priority]->Insert( cr_task->m_generic_list_item );
		}
