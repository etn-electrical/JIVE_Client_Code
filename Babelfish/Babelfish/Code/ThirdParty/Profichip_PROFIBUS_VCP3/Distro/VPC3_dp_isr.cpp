...
static unsigned int count_DXOUT = 0; //Added SSN
unsigned char count_FF=0;
/*---------------------------------------------------------------------------*/
...
volatile UBYTE bResult;
  ...
  //SSN
     if (pDpSystem->wInterruptEvent == 0xFF)
       count_FF++;
   /*------------------------------------------------------------------------*/
   /* IND_NEW_PRM_DATA                                                       */
   /*------------------------------------------------------------------------*/
      ...
      if( VPC3_GET_IND_NEW_PRM_DATA() )
      {
        pDpSystem->wEvent |= VPC3_EV_NEW_PRM_DATA;          
         profi_activity->Post_From_ISR( FALSE ); 
       }//if( VPC3_GET_IND_NEW_PRM_DATA() )
   ...
   /*------------------------------------------------------------------------*/
      ...
      if( VPC3_GET_IND_DX_OUT() )
      {
         ...
         #endif//#if DP_MSAC_C1
          count_DXOUT++; //SSN added
         UserIsrDxOut();
        //SSN: This function is Empty thus commenting it since it is not required.
        //This same activity is been done at the end of this routine by function CopyToVpc3_ which takes care for all the other events. 
        // VPC3_CON_IND_DX_OUT(); 
      }//if( VPC3_GET_IND_DX_OUT() )
      ...
      if( pDpSystem->wPollInterruptEvent > 0 )
      {
         ...
         if( VPC3_POLL_IND_NEW_PRM_DATA() )
         {
            ...
            do
            {
               ...
               if( UserChkNewPrmData( (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK )
               {
                  #if DP_MSAC_C1
               ...
               {
                  bResult = VPC3_SET_PRM_DATA_NOT_OK();
               }//else of if( UserChkNewPrmData( (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK )
            }
            while( bResult == VPC3_PRM_CONFLICT );
         }//if( VPC3_POLL_IND_NEW_PRM_DATA() )
         ...
         if( VPC3_POLL_IND_NEW_CFG_DATA() )
         {
            ...
            do
            {
               ...
               switch( UserChkNewCfgData( (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bCfgLength ) )
               {
                  case VPC3_CFG_OK:
                  {
                     #if DP_MSAC_C1
