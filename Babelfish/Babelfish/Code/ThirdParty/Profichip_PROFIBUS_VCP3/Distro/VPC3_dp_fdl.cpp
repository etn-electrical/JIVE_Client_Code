...
#if DP_FDL //compile only, if fdl-application is selected --> dp_cfg.h
...
UBYTE              i;
   ...
   for( i = 0; i < FDL_SAP_MAX; i++ )
   {
      ...
      if( ( pDpSystem->wVpc3UsedDpBufferMemory + wFdlUsedMemory ) > ASIC_USER_RAM )
      {
         memset( &pDpSystem->sFdl.abSearchSap[0], 0xFF, sizeof( SIZE_OF_SEARCH_KEY ) );
         //on error: empty sap-list - first byte of sap-list becomes end-criterion
         VPC3_SET_EMPTY_SAP_LIST();
   ...
   VPC3_SET_SAP_LIST_END( 7 * FDL_SAP_MAX );

   pDpSystem->wVpc3UsedDpBufferMemory += wFdlUsedMemory;

   return DP_OK;
}//DP_ERROR_CODE FDL_Init( void )
   ...
   for( i = 0; i < FDL_SAP_MAX; i++ )
   {
          ...
          && ( psFdlHeader->eFdlState == ENABLED ) )
      {
         if( psFdlHeader->eSapType == FDL_RM_SAP )
         {
               ...
               #else
                  psFdlHeaderRm = FDL_GetSapHeaderPtr((UBYTE) pDpSystem->sFdl.psRmRespBuffer->bSapNr);
                  // allow access to the sap from the detected master and it's source-sap
...
/*---------------------------------------------------------------------------*/
/*                                                                           */
/* FDL_SAP_HEADER_PTR FDL_GetSapHeaderPtr( UBYTE bSap )                   */
/*                                                                           */
...
UBYTE bIndex;
   ...
   bIndex = pDpSystem->sFdl.abSearchSap[ bSap ];
   if( ( bSap >= sizeof( pDpSystem->sFdl.abSearchSap ) ) || ( bIndex == 0xFF ) )
   {
      // error function
      sVpc3Error.bErrorCode = bIndex;
      sVpc3Error.bCnId      = bSap;
      // *** no further action of dpv1/c2 ***
      FatalError( _DP_FDL, __LINE__, &sVpc3Error );
   }//if( ( bSap >= sizeof( pDpSystem->sFdl.abSearchSap ) ) || ( bIndex == 0xFF ) )

   return ( &pDpSystem->sFdl.asSapHeaderList[ bIndex ] );
}//FDL_SAP_HEADER_PTR   FDL_GetSapHeaderPtr( UBYTE sap )
...
#endif//#if DP_FDL // compile only, if fdl-application is selected --> dp_cfg.h
