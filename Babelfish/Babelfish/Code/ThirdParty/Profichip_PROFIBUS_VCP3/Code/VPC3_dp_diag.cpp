 ...
 * @return FALSE - the static diagnostic is not resettet yet, try again
 */
...
uint8_t bLoop;
   ...
   {
      bDiagRetValue = DpDiag_Alarm( USER_TYPE_PRM_OK, 0, (ALARM_STATUS_PDU_PTR) VPC3_NULL_PTR, FALSE );
 ...
 * @return FALSE - diagnostic is not resettet yet, try again
 */
...
uint8_t bLoop;
   ...
   while( bLoop++ < MAX_DIAG_RETRY )
   {
      if( DpDiag_Alarm( USER_TYPE_PRM_NOK, 0, (ALARM_STATUS_PDU_PTR) VPC3_NULL_PTR, FALSE ) == DP_OK )
      {
         bRetValue = ePrmError;
         break;
      }
...
uint8_t bLoop;
   ...
   while( bLoop++ < MAX_DIAG_RETRY )
   {
      if( DpDiag_Alarm( USER_TYPE_CFG_OK, 0x22, (ALARM_STATUS_PDU_PTR) VPC3_NULL_PTR, FALSE ) == DP_OK )
      {
         eRetValue = eCfgError;
         break;
      }
 ...
 * @return FALSE - the static diagnostic bit is not resettet yet, try again
 */
uint8_t DpDiag_ResetStatDiag( void )
{
   if( DpDiag_Alarm( USER_TYPE_APPL_RDY, 0, (ALARM_STATUS_PDU_PTR) VPC3_NULL_PTR, FALSE ) == DP_OK )
   {
      return TRUE;
   }
   return FALSE;
...
void DpDiag_ResetDiagnosticBuffer( void )
{
   ...
   DpDiag_Alarm( USER_TYPE_RESET_DIAG, USER_TYPE_RESET_DIAG, (ALARM_STATUS_PDU_PTR) VPC3_NULL_PTR, FALSE );
}//void DpDiag_ResetDiagnosticBuffer( void )
