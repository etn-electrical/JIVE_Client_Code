...
static void netQEmpty( BufQueue*queue )
{
	...
	while ( queue->tail != queue->head )
	{
		//Get the buffer from the queue.
		queue->tail = ( queue->tail + 1 ) & PBUF_QUEUE_MASK;
		pbuf_free( ( struct pbuf* )queue->pbuf[queue->tail] );
	}
...
{
	ip_addr_t multicastAaddr;         //modified to resolve incomplete type error
...
bool netInit( NetPath*netPath, PtpClock*ptpClock )
{
	...
	ip_addr_t interfaceAddr;            //modified to resolve incomplete type error
...
static ssize_t netSend( const octet_t*buf, int16_t length, TimeInternal*time, const int32_t* addr,
						struct udp_pcb* pcb )
{
	err_t result;
	struct pbuf* p;
	...
	/* send the buffer. */
	result = udp_sendto( pcb, p, ( const ip_addr_t* )addr, pcb->local_port );
	if ( ERR_OK != result )
	{
