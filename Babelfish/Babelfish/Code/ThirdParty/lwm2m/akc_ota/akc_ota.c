...
static void usage()
{
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Usage: akc_ota [options]\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-u <server URI> : LWM2M server URI\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-d <device ID> : AKC device ID\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-t <device token> : AKC device token\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-c <path device certificate> : Device certificate\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-k <path device private key> : Device private key\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-s <path server certificate> : Server certificate\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-r <path root ca> : Root CA\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-n : don't verify SSL certificate\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-p <port> : local source port to connect from\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-v <version> : initial version string\r\n");
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"\t-h : display help\r\n");
}
...
static bool fill_buffer_from_file(const char *file, char **pbuffer)
{
    ...
    if (access(file, F_OK) != 0) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"cannot access '%s': %s\n", file, strerror(errno));
        return false;
    }

    stream = fopen(file, "r");
    if (!stream) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"cannot open '%s': %s\n", file, strerror(errno));
        goto error;
    }

    if (fseek(stream, 0, SEEK_END) != 0) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"cannot seek '%s': %s\n", file, strerror(errno));
        goto error;
    }

    size = ftell(stream);
    if (size < 0) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"cannot tell '%s': %s\n", file, strerror(errno));
        goto error;
    }
    ...
    if (!buffer) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"cannot allocate %ld bytes\n", size);
        goto error;
    }
...
static bool extract_ota(char *filename, bool do_extract)
{
    ...
    if (archive_read_open_filename(a, filename, 10240)) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Error opening archive %s : %s", filename, archive_error_string(a));
        goto exit;
    }
...
static void * start_updating_ota(void *user_data) {
    ...
    if (!extract_ota(ota_update->file, true)) {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Extract %s failed\n", ota_update->file);
        ota_update->state = LWM2M_FIRMWARE_UPD_RES_DEFAULT;
        goto exit;
    }
    ...
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Launch %s\n", script);
    ...
    if (WIFEXITED(status)) {
        if (WEXITSTATUS(status) == 0) {
            ...
            if (!fp) {
                LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Failed to open %s\n", version_file);
            } else {
...
static void on_firmware_update(void *param, void *extra)
{
    ...
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Update in progress...\n");
...
static void on_resource_changed(void *param, void *extra)
{
    ...
    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Resource Changed: %s\r\n", params->uri);
    ...
    if (!strncmp(params->uri, LWM2M_URI_FIRMWARE_PACKAGE_URI, LWM2M_MAX_URI_LEN))
    {
        ...
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Downloading update to %s\n", file);
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "r:c:s:k:u:d:t:np:v:h")) != -1) {
            ...
            case 'd':
                ...
                {
                    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Wrong device ID parameter\r\n");
                    usage();
                    return -1;
                }
            ...
            case 't':
                if (strlen(optarg) != AKC_UUID_LEN)
                {
                    LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Wrong device token parameter\r\n");
                    usage();
                    return -1;
                }
    ...
    {
        LWM2M_DEBUG_PRINT(DBG_MSG_DEBUG,"Failed to start client\n");
        return -1;
    }
