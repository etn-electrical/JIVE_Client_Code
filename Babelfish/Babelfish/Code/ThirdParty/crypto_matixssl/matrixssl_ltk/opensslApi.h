...
/** The context for our SSL/TLS activities.
 * Holds the keys and socket address. */
typedef struct
{
	sslKeys_t*keys;
...
typedef struct
{
	...
	uint32 idle_state_timeout;	/* in milliseconds*/
	int32 fd;
	int32 outBufOffset;
	int32 appRecLen;
...
typedef SSL SSL_SESSION;	// aka ssl_t, the structure Matrix SSL uses
...
int SSL_CTX_load_rsa_key_material( SSL_CTX*ctx, const unsigned char*certBuf,
								   int32 certLen, const unsigned char*privBuf, int32 privLen,
								   const unsigned char*CAbuf, int32 CAlen ); /* not openssl */
...
int SSL_connect_Dtls( SSL*cp );

int SSL_do_handshake_Dtls( SSL*cp );

int SSL_read_Dtls( SSL*cp, void*userBuf, int userBufLen );

int SSL_write_Dtls( SSL*cp, const void*inbuf, int32 inlen );

/**
 * @brief Get active ssl protocol version.
