...
#ifdef USE_CERT_GEN
/**
    Write a pstm format integer to base-128 Big Endian raw binary format.

    @param[in] pool Memory pool to use.
    @param[in] out Output buffer where to put the encoding.
    @param[in,out] out_buf_len In: length of the output buffer.
    Out: number of bytes written to out.

    @return < 0 on failure, PS_SUCCESS on success, PS_OUTPUT_LENGTH when
    the output buffer is too small.
*/
int32_t pstm_to_base128_be_unsigned_bin(psPool_t *pool, const pstm_int *in,
        unsigned char *out, size_t *out_len)
{
    ...
    if ((res = pstm_init_copy(pool, &t, in, 0)) != PSTM_OKAY)
    {
        return res;
    }

    /*
      For the base-10 to base-128 conversion, use method 1a
      from Knuth's TAOCP, section 4.4.A. All operations are base-10.
      U_0 = M mod 128
      U_1 = floor(M/128) mod 128
      U_2 = floor(floor(M/128)/128) mod 128
          ...   ...
      Stop when the quotient in the mod 128 remainder calculation
      becomes 0.

      This method will give out the octets in Little Endian order:
      U_0,U_1,...,U_n. We'll convert to Big Endian below.
    */
    while (pstm_iszero(&t) == 0)
    {
        if (i >= *out_len)
        ...
        /* out[x] = t mod 128. */
        out[i] = (unsigned char) (t.dp[0] & 127);
        i++;

        /* t = quotient(t/128). */
        if ((res = pstm_div_2d(pool, &t, 7, &t, NULL)) != PSTM_OKAY)
        {
            pstm_clear(&t);
            return res;
        }
...
/*
    These APIs deal with big number primes and are only needed when
    generating RSA keys
 */

static const int32 lnz[16] = {
 ...
 */
int32_t pstm_cnt_lsb(const pstm_int *a)
{
    int32 x;
    pstm_digit q, qq;
...
/**
    ...
    c = gcd(a, b)
 */
int32_t pstm_gcd(psPool_t *pool, const pstm_int *a, const pstm_int *b, pstm_int *c)
{
    pstm_int u, v, r;
    int32 res;

    /* either zero than gcd is the largest */
    if (pstm_iszero(a) == 1 && pstm_iszero(b) == 0)
    {
        return pstm_abs(b, c);
    }
    if (pstm_iszero(a) == 0 && pstm_iszero(b) == 1)
    {
        return pstm_abs(a, c);
    }
    ...
    if (pstm_cmp_mag(a, b) != PSTM_LT)
    {
        if ((res = pstm_init_copy(pool, &u, a, 0)) != PSTM_OKAY)
        {
            return res;
        }
        if ((res = pstm_init_copy(pool, &v, b, 0)) != PSTM_OKAY)
        {
            goto LBL_U;
        }
    }
    ...
    {
        if ((res = pstm_init_copy(pool, &u, b, 0)) != PSTM_OKAY)
        {
            return res;
        }
        if ((res = pstm_init_copy(pool, &v, a, 0)) != PSTM_OKAY)
        {
            goto LBL_U;
        }
    }

    if ((res = pstm_init_size(pool, &r, v.alloc)) != PSTM_OKAY)
    {
        goto LBL_V;
    }
    pstm_zero(&r);
    while (pstm_iszero(&v) == PSTM_NO)
    {
        if ((res = pstm_mod(pool, &u, &v, &r)) != PSTM_OKAY)
        {
            goto LBL_R;
        }
        if ((res = pstm_copy(&v, &u)) != PSTM_OKAY)
        {
            goto LBL_R;
        }
        if ((res = pstm_copy(&r, &v)) != PSTM_OKAY)
        {
            goto LBL_R;
        }
    }
    if ((res = pstm_copy(&u, c)) != PSTM_OKAY)
    {
        goto LBL_R;
    }
    res = PSTM_OKAY;
...
/**
    ...
    c = lcm(a, b)
 */
int32_t pstm_lcm(psPool_t *pool, const pstm_int *a, const pstm_int *b, pstm_int *c)
{
    pstm_int t1, t2;
    int32 res;

    if (pstm_init(pool, &t1) != PSTM_OKAY)
    {
        return PS_MEM_FAIL;
    }
    if (pstm_init(pool, &t2) != PSTM_OKAY)
    {
        pstm_clear(&t1);
        return PS_MEM_FAIL;
    }
    ...
    if ((res = pstm_gcd(pool, a, b, &t1)) != PSTM_OKAY)
    {
        goto LBL_CLR;
    }

    /* divide the smallest by the GCD */
    if (pstm_cmp_mag(a, b) == PSTM_GT)
    {
        /* store quotient in t2 such that t2 * b is the LCM */
        if ((res = pstm_div(pool, a, &t1, &t2, NULL)) != PSTM_OKAY)
        {
            goto LBL_CLR;
        }
        if ((res = pstm_mul_comba(pool, b, &t2, c, NULL, 0)) != PSTM_OKAY)
        {
            goto LBL_CLR;
        }
    }
    else
    {
        ...
        if ((res = pstm_div(pool, b, &t1, &t2, NULL)) != PSTM_OKAY)
        {
            goto LBL_CLR;
        }
        if ((res = pstm_mul_comba(pool, a, &t2, c, NULL, 0)) != PSTM_OKAY)
        {
            goto LBL_CLR;
        }
    }
...
static int32 s_is_power_of_two(pstm_digit b, int32_t *p)
{
    int32 x;
...
/**
    a/b => cb + d == a.
 */
static int32_t pstm_div_d(psPool_t *pool, const pstm_int *a, pstm_digit b,
    pstm_int *c, pstm_digit *d)
{
    pstm_int q;
    pstm_word w;
    pstm_digit t;
    ...
    if (s_is_power_of_two(b, &ix) == 1)
    {
        if (d != NULL)
        {
            *d = a->dp[0] & ((((pstm_digit) 1) << ix) - 1);
        }
        if (c != NULL)
        {
            if ((res = pstm_div_2d(pool, a, ix, c, NULL)) != PSTM_OKAY)
            {
                return res;
            }
        }
    ...
    {
        w = (w << ((pstm_word) DIGIT_BIT)) | ((pstm_word) a->dp[ix]);

        if (w >= b)
        {
            t = (pstm_digit) (w / b);
            w -= ((pstm_word) t) * ((pstm_word) b);
        }
        else
        {
            t = 0;
        }
        q.dp[ix] = (pstm_digit) t;
    }
...
/**
    c = a * a (mod b).
 */
static int32_t pstm_sqrmod(psPool_t *pool, const pstm_int *a, const pstm_int *b,
    pstm_int *c)
{
    pstm_int tmp;
    int32 res;

    if (pstm_init_size(pool, &tmp, (a->used * 2) + 1) != PSTM_OKAY)
    {
        return PS_MEM_FAIL;
    }
    pstm_zero(&tmp);

    if ((res = pstm_sqr_comba(pool, a, &tmp, NULL, 0)) != PSTM_OKAY)
    {
        pstm_clear(&tmp);
        return res;
    }
    res = pstm_mod(pool, &tmp, b, c);
    pstm_clear(&tmp);
    return res;
}
...
/**
    ...
    HAC p.139 Algorithm 4.24.

    Sets result to 0 if definitely composite or 1 if probably prime.
    Randomly the chance of error is no more than 1/4 and often
    very much lower.
 */
static int32_t pstm_prime_miller_rabin(psPool_t *pool, const pstm_int *a,
    const pstm_int *b, int32_t *result)
{
    pstm_int n1, y, r;
    int32 s, j, res;
    ...
    *result = PSTM_NO;
    ...
    /* ensure b > 1 */
    if (pstm_cmp_d(b, 1) != PSTM_GT)
    {
        return PS_LIMIT_FAIL;
    }

    /* get n1 = a - 1 */
    if ((res = pstm_init_copy(pool, &n1, a, 0)) != PSTM_OKAY)
    {
        return res;
    }
    if ((res = pstm_sub_d(pool, &n1, 1, &n1)) != PSTM_OKAY)
    {
        goto LBL_N;
    }

    /* set 2**s * r = n1 */
    if ((res = pstm_init_copy(pool, &r, &n1, 0)) != PSTM_OKAY)
    {
        goto LBL_N;
    }
    ...
    if ((res = pstm_div_2d(pool, &r, s, &r, NULL)) != PSTM_OKAY)
    {
        goto LBL_R;
    }
    ...
    if ((res = pstm_exptmod(pool, b, &r, a, &y)) != PSTM_OKAY)
    {
        goto LBL_Y;
    }

    /* if y != 1 and y != n1 do */
    if (pstm_cmp_d(&y, 1) != PSTM_EQ && pstm_cmp(&y, &n1) != PSTM_EQ)
    {
        j = 1;
        /* while j <= s-1 and y != n1 */
        while ((j <= (s - 1)) && pstm_cmp(&y, &n1) != PSTM_EQ)
        {
            if ((res = pstm_sqrmod(pool, &y, a, &y)) != PSTM_OKAY)
            {
                goto LBL_Y;
            }

            /* if y == 1 then composite */
            if (pstm_cmp_d(&y, 1) == PSTM_EQ)
            {
                res = PSTM_OKAY;
                goto LBL_Y;
            }
...
/**
    c = a mod b, 0 <= c < b.
 */
static int32_t pstm_mod_d(psPool_t *pool, const pstm_int *a, pstm_digit b,
    pstm_digit *c)
{
    return pstm_div_d(pool, a, b, NULL, c);
}

/* a few primes */
static const pstm_digit primes[256] = {
...
/**
    ...
    256 trial divisions and 8 rounds of Miller-Rabin
 */
int32_t pstm_isprime(psPool_t *pool, const pstm_int *a, int32_t *result)
{
    pstm_int b;
    pstm_digit d;
    int32 r, err, res;
    ...
    *result = PSTM_NO;
    ...
    for (r = 0; r < 256; r++)
    {
        if ((err = pstm_mod_d(pool, a, primes[r], &d)) != PSTM_OKAY)
        {
            return err;
        }
        if (d == 0)
    ...
    if (pstm_init_size(pool, &b, sizeof(pstm_digit)) != PSTM_OKAY)
    {
        return PS_MEM_FAIL;
    }
    for (r = 0; r < 8; r++)
    {
        pstm_set(&b, primes[r]);
        if ((err = pstm_prime_miller_rabin(pool, a, &b, &res)) != PSTM_OKAY)
        {
            pstm_clear(&b);
            return err;
        }
...
# endif /* USE_CERT_GEN && USE_MATRIX_RSA */
