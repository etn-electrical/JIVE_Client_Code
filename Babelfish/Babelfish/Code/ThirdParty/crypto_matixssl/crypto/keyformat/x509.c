        ...
        char out[MAX_OID_PRINTED_LEN])
{
    ...
    #ifdef HAVE_ASN_FORMAT_OID		// Instead of #ifndef NO_ASN_FORMAT_OID, which didn't help
    mem = asnFormatOid(
    ...
    void *hwCtx, void *poolUserPtr)
{
    ...
    */
    while (ic)
    {
        ...
        */
        if ((ic->version > 2) && (ic->extensions.bc.cA != CA_TRUE))
        {
            if (sc != ic)
            {
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        if (psEstimatePkiPoolSize(issuer->publicKey.keysize, PS_RSA,
                &pubSize, &privSize) < 0)
        {
            return PS_FAILURE;
        }
        if ((pkiPool = psOpenPool("TMP_PKI", pubSize, 0, NULL, NULL)) == NULL)
        {
            return PS_MEM_FAIL;
        }
...
#  ifdef USE_ECC
    if (sigType == PS_ECC)
    {
        ...
        /* ECDSA signature */
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
        if (psEstimatePkiPoolSize(issuer->publicKey.keysize, PS_ECC,
                &pubSize, &privSize) < 0)
        {
            return PS_FAILURE;
        }
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
    /* Only failure cases have closed pool */
    psClosePool(pkiPool);
#  endif
