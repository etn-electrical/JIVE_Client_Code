...
int32_t getStreamingAsnAlgorithmIdentifier(const unsigned char **pp, psSizeL_t size,
    int32_t *oi, psSize_t *paramLen)
{
    ...
    if (size < 1 || (rc = getAsnSequence32(&p, size, &len32, 1)) < 0)
    {
        return rc;
    }
    ...
    if (size < ((uint32_t) (p - *pp) + len32))
    {
        return PS_LIMIT_FAIL;
    }
    rc = getAsnOID(&p, len32, oi, 1, paramLen);
    *pp = p;
    return rc;
}
#endif
 ...
 */
int32_t getAsnConstructedOctetString32(psPool_t *pool,
		       const unsigned char **pp, psSizeL_t size,
		       unsigned char **outString, uint32_t *outStringLen)
{
    const unsigned char *p = *pp, *end, *tmp;
    unsigned char *constructStart;
    uint32_t llen, totalStrLen, index;
    int32_t rc;

    end = p + size;

    if ((rc = getAsnLength32(&p, end - p, &llen, 1)) < 0)
    {
        return rc;
    }
    if (end - p < 1)
    ...
    while (*p == ASN_OCTET_STRING)
    {
        ...
        psAssert(rc != ASN_UNKNOWN_LEN);
        if ((uint32_t) (end - p) < llen)
        {
            return PS_LIMIT_FAIL;
        }
        totalStrLen += llen;
    ...
    {
        p++;
        if (getAsnLength32(&p, end - p, &llen, 1) < 0 || size < llen)
        {
