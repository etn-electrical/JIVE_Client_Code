...
# endif
                    ...
                    else if (*c == SSL_RECORD_TYPE_ALERT &&
                             ssl->hsState == SSL_HS_DONE &&
                             (c != (origbuf + *len)))
                    {
                        psTraceErrr("Unexpected record type\n");
                        *error = PS_PROTOCOL_FAIL; // LTK-6559, patch provided by matrixssl
                        return MATRIXSSL_ERROR;
                    }         
                    else
                    {
...
/*
                    There is no telling what specific handshake message could
                    be sent in fragmented form.  We do want to store this
                    message in the handshake pool though.  Just need to confirm
                    it exists.
 */
                    if (ssl->hsPool == NULL)
                    {
