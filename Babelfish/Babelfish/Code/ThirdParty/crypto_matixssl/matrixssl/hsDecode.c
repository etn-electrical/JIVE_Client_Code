...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
            /* This is the first time we need to access a handshake pool. */
            if (ssl->hsPool == NULL)
            {
                ssl->hsPool = matrixSslGetHandshakePool(ssl);
                if (ssl->hsPool == NULL)
                {
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
    unsigned short ckepubSize, ckeprivSize;
#  endif
...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    /*  As a server, we may have allocated a pool during the Certificate
        step of client auth (or during generation of ServerKeyExchange message
        if using DH).  If not, allocate the pool here */
    if (ssl->hsPool == NULL)
    {
        ssl->hsPool = matrixSslGetHandshakePool(ssl);
        if (ssl->hsPool == NULL)
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
    if ((rc = psEstimatePkiPoolSize(ssl->->chosenIdentity->privKey.keysize, PS_RSA,
             &ckepubSize, &ckeprivSize)) < 0)
    {
        return rc;
    }
    if ((ckepkiPool = psOpenPool("TMP_PKI", ckeprivSize, 0, NULL,
             ssl->memAllocPtr)) == NULL)
    {
        return PS_MEM_FAIL;
    }
#  endif
...
#  endif
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
    psClosePool(ckepkiPool); ckepkiPool = NULL;
#  endif
...
#  endif /* USE_DTLS */
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        matrixSslCloseHandshakePool(ssl->hsPool);
#  endif /* USE_MATRIX_MEMORY_MANAGEMENT        */
        ssl->hsPool = NULL;
    }
# else /* CLIENT_AUTH */
...
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
    unsigned short skepubSize, skeprivSize;
#   endif
#  endif /* USE_ONLY_PSK_CIPHER_SUITE */
#  ifdef USE_ECC_CIPHER_SUITE
    const psEccCurve_t *curve;
...
#   ifdef USE_ANON_DH_CIPHER_SUITE
        if (ssl->flags & SSL_FLAGS_ANON_CIPHER)
        {
            /*  This case means there was no CERTIFICATE message, which means
                there is no handshake pool yet. */
            if (ssl->hsPool == NULL)
            {
                ssl->hsPool = matrixSslGetHandshakePool(ssl);
                if (ssl->hsPool == NULL)
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
/*
                No CERTIFICATE message, which means no handshake pool yet.
 */
            if (ssl->hsPool == NULL)
            {
                ssl->hsPool = matrixSslGetHandshakePool(ssl);
...
#  if defined(USE_DHE_CIPHER_SUITE) && defined(USE_MATRIX_MEMORY_MANAGEMENT)
    unsigned short dpubSize, dprivSize;
#  endif
...
#    endif
#    ifdef USE_MATRIX_MEMORY_MANAGEMENT
            rc = psEstimatePkiPoolSize(ssl->sec.eccKeyPub->curve->size * 2,
                                       PS_ECC, &dpubSize, &dprivSize);
            if (rc < 0)
            {
                return rc;
            }
            /*  Life of this pool is a bit longer than TMP_PKI and we need
                a bit of extra storage so adding in the pubSize for that
                overhead.  This pool is closed on session delete on failures
                or manually when lifecycle is over on success */
            if ((ssl->sec.eccDhKeyPool = psOpenPool("ECC_PKI_POOL",
                     dprivSize + dpubSize, 0, NULL, ssl->memAllocPtr)) == NULL)
            {
                return PS_MEM_FAIL;
            }
#    endif
...
#    ifdef USE_MATRIX_MEMORY_MANAGEMENT
        rc = psEstimatePkiPoolSize(ssl->sec.dhKeyPub->size, PS_RSA,
                                   &dpubSize, &dprivSize);
        if (rc < 0)
        {
            return rc;
...
/*
            Life of this pool is a bit longer than TMP_PKI and we need
            a bit of extra storage so adding in the pubSize for that overhead
 */
        if ((ssl->sec.dhKeyPool = psOpenPool("DH_PKI_POOL",
                 dprivSize + dpubSize, 0, NULL, ssl->memAllocPtr)) == NULL)
        {
            return PS_MEM_FAIL;
        }
...
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
    /* Potential first time the handshake pool is needed for this path */
    if (ssl->hsPool == NULL)
    {
        ssl->hsPool = matrixSslGetHandshakePool(ssl);
        if (ssl->hsPool == NULL)
