...
#    ifdef USE_MATRIX_MEMORY_MANAGEMENT
                    /*  Done with the DH_PKI pool that was opened at parse of
                        SERVER_HELLO_DONE.  Fine to remove pool for all
                        current hardware crypto support.  It isn't used */
                    psClosePool(ssl->sec.eccDhKeyPool);
                    ssl->sec.eccDhKeyPool = NULL;
#    endif
                    if (rc != PS_PENDING)
                    {
...
/*
    ...
    the receipt of the finished message.  Both cases are attempted
...
void clearPkaAfter(ssl_t *ssl)
{
    ...
    {
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
        /* The pool was saved aside */
        psClosePool(ssl->pkaAfter[0].pool); ssl->pkaAfter[0].pool = NULL;
# endif /* USE_MATRIX_MEMORY_MANAGEMENT */
    }
...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    if (ssl->flightPool == NULL)
    {
        /* Create if needed */
        ssl->flightPool = psOpenPool("FLIGHT POOL", 1024 + sizeof(psPool_t),
            0, NULL, ssl->memAllocPtr);
        if (ssl->flightPool == NULL)
        {
            return PS_MEM_FAIL;
        }
...
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
            /* First time handshake pool would be needed */
            if (ssl->hsPool == NULL)
            {
...
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
/*
        If the server did not send a ServerKeyExchange message there
        has not been a handshake pool created yet.  Time to find out.
 */
        if (ssl->hsPool == NULL)
        {
            ssl->hsPool = matrixSslGetHandshakePool(ssl);
...
#   ifdef USE_ECC_CIPHER_SUITE
    ...
    {
#    ifdef USE_MATRIX_MEMORY_MANAGEMENT
        if ((rc = psEstimatePkiPoolSize(ssl->sec.cert->publicKey.keysize,
                 PS_ECC, &pubSize, &privSize)) < 0)
        {
            return rc;
        }
        if ((pkiPool = psOpenPool("TMP_PKI", pubSize, 0, NULL,
...
#     ifdef USE_MATRIX_MEMORY_MANAGEMENT
    if ((rc = psEstimatePkiPoolSize(chosen->privKey.keysize,
             PS_RSA, &pubSize, &privSize)) < 0)
    {
        return rc;
    }
    if ((pkiPool = psOpenPool("TMP_PKI", privSize, 0, NULL,
             ssl->memAllocPtr)) == NULL)
    {
        return PS_MEM_FAIL;
    }
