...
static void ProcessPublishMessage(MQTT_CLIENT* mqtt_client, uint8_t* initialPos, size_t packetLength, int flags)
{
    ...
    else
    {
        ...
        if (qosValue != DELIVER_AT_MOST_ONCE)
        {
            ...
            if (is_trace_enabled(mqtt_client))
            {
                STRING_sprintf(trace_log, " | PACKET_ID: %u", packetId);
            }
#endif
        }
        ...
        if ((iterator != NULL && packetLength > 0) || packet == PINGRESP_TYPE)
        {
            switch (packet)
            {
                ...
                case PUBCOMP_TYPE:
                {
                    ...
                    if (is_trace_enabled(mqtt_client))
                    {
                        STRING_HANDLE trace_log = STRING_construct_sprintf("%s | PACKET_ID: %u", packet == PUBACK_TYPE ? "PUBACK" : (packet == PUBREC_TYPE) ? "PUBREC" : (packet == PUBREL_TYPE) ? "PUBREL" : "PUBCOMP",
                ...
                case SUBACK_TYPE:
                {
                    ...
                    if (is_trace_enabled(mqtt_client))
                    {
                        trace_log = STRING_construct_sprintf("SUBACK | PACKET_ID: %u", suback.packetId);
                    }
#endif
                    ...
                    if (suback.qosReturn != NULL)
                    {
                        while (remainLen > 0)
                        {
                            ...
                            if (is_trace_enabled(mqtt_client))
                            {
                                STRING_sprintf(trace_log, " | RETURN_CODE: %u", suback.qosReturn[suback.qosCount-1]);
                            }
#endif
                        }
                ...
                case UNSUBACK_TYPE:
                {
                    ...
                    if (is_trace_enabled(mqtt_client))
                    {
                        STRING_HANDLE trace_log = STRING_construct_sprintf("UNSUBACK | PACKET_ID: %u", unsuback.packetId);
                        log_incoming_trace(mqtt_client, trace_log);
                        STRING_delete(trace_log);
                    }
#endif
