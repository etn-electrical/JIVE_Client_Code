 ...
 *****************************************************************************************
 */
Cert_Conf_ECC_Matrix::Cert_Conf_ECC_Matrix( Cert_Conf_Lib::cert_config_data_t* cert_conf,
 ...
 *****************************************************************************************
 */
int32_t Cert_Conf_ECC_Matrix::Write_DN_Attribute( psPool_t* pool, psCertConfig_t* conf,
												  char_t* device_cn,
 ...
 *****************************************************************************************
 */
PKI_Common::pki_status_t Cert_Conf_ECC_Matrix::Generate( void )
{
	...
	if ( m_pscert_conf != nullptr )
	{
		if ( m_cert_conf != nullptr )
		{
			...
			ret = psX509SetSerialNum( pool, m_pscert_conf, ( uint8_t* )m_cert_conf->serial_num,
									  BF_Lib::Get_String_Length( reinterpret_cast<uint8_t*>(
																	 const_cast<char_t*>(
																		 m_cert_conf->serial_num ) ),
																 PKI_Common::MAX_STRING_LENGTH ) );
			if ( ret == PS_SUCCESS )
			{
				ret = psX509SetBasicConstraintsCA( pool, m_pscert_conf,
												   m_pki_conf->bsc_constraint_ca );
					...
					if ( ret == PS_SUCCESS )
					{
						...
						if ( ret == PS_SUCCESS )
						{
							...
							if ( ret == PS_SUCCESS )
							{
								...
								{
									ret =
										psX509AddKeyUsageBit( pool, m_pscert_conf, KEY_CERT_SIGN );
								}
								if ( ( ret == PS_SUCCESS ) &&
									 ( m_pki_conf->key_usage & PKI_Common::KEY_USG_KEY_AGREEMENT ) )
								{
									ret =
										psX509AddKeyUsageBit( pool, m_pscert_conf, KEY_AGREEMENT );
								}
								if ( ( ret == PS_SUCCESS ) &&
									 ( m_pki_conf->key_usage &
									   PKI_Common::KEY_USG_DATA_ENCIPHERMENT ) )
								{
									ret = psX509AddKeyUsageBit( pool, m_pscert_conf,
																DATA_ENCIPHERMENT );
								}
								if ( ( ret == PS_SUCCESS ) &&
									 ( m_pki_conf->key_usage &
									   PKI_Common::KEY_USG_KEY_ENCIPHERMENT ) )
								{
									ret = psX509AddKeyUsageBit( pool, m_pscert_conf,
																KEY_ENCIPHERMENT );
								}
								if ( ( ret == PS_SUCCESS ) &&
									 ( m_pki_conf->key_usage &
									   PKI_Common::KEY_USG_NON_REPUDIATION ) )
								{
									ret =
										psX509AddKeyUsageBit( pool, m_pscert_conf,
															  NON_REPUDIATION );
								}
								if ( ( ret == PS_SUCCESS ) &&
									 ( m_pki_conf->key_usage &
									   PKI_Common::KEY_USG_DIGITAL_SIGNATURE ) )
								{
									ret = psX509AddKeyUsageBit( pool, m_pscert_conf,
																DIGITAL_SIGNATURE );
								}
										...
										if ( m_cert_conf->common_name != nullptr )
										{
											...
											if ( ret == PS_SUCCESS )
											{
													 ...
													 ( m_cert_conf->san_ip_address != nullptr ) )
												{
													...
													BF_Lib::Copy_Val_To_String( reinterpret_cast<
																					uint8_t*>( &
																							   sanEntry ), 0U,
																				sizeof(
													...
													if ( m_cert_conf->san_dns_name != nullptr )
													{
														...
														sanEntry.dNSNameLen = BF_Lib::Get_String_Length(
															reinterpret_cast<uint8_t*>( m_cert_conf->san_dns_name ),
															PKI_Common::MAX_STRING_LENGTH );
													}
													sanEntry.iPAddress =
														m_cert_conf->san_ip_address;
													sanEntry.iPAddressLen =
														BF_Lib::Get_String_Length( reinterpret_cast<
																					   uint8_t*>(
																					   m_cert_conf->
																					   san_ip_address ),
													...
													ret = psX509AddSubjectAltName( pool,
																				   m_pscert_conf,
																				   &sanEntry );
												}
											}
 ...
 *****************************************************************************************
 */
PKI_Common::pki_status_t Cert_Conf_ECC_Matrix::Validate( void )
{
 ...
 *****************************************************************************************
 */
PKI_Common::pki_status_t Cert_Conf_ECC_Matrix::Get_Cert_Config( void** cert_conf )
{
 ...
 *****************************************************************************************
 */
Cert_Conf_ECC_Matrix::~Cert_Conf_ECC_Matrix( void )
{
	if ( m_pscert_conf != nullptr )
	{
