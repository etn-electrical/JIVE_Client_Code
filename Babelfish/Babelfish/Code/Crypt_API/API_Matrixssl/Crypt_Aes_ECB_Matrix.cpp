 ...
 */
#include "Includes.h"
#include "Crypt_Aes_ECB_Matrix.h"
#include "Ram.h"
#include "Stdlib_MV.h"
#include "Babelfish_Assert.h"

const uint8_t Crypt_Aes_ECB_Matrix::BLANK_ENC_STRING[BLANK_ENC_STRING_LEN] =
 ...
 *****************************************************************************************
 */
Crypt_Aes_ECB_Matrix::Crypt_Aes_ECB_Matrix( uint8_t const* key, aes_key_size_t key_size ) :
	Crypt(),
	m_key( key ),
	m_key_size( key_size ),
	m_ctx( nullptr )
{
	BF_ASSERT ( ( key_size == AES_128_KEY ) || ( key_size == AES_256_KEY ) );
	m_ctx = reinterpret_cast<psAesKey_t*>( Ram::Allocate(
											   static_cast<size_t>( sizeof( psAesKey_t ) ) ) );
 ...
 *****************************************************************************************
 */
Crypt::crypt_status_t Crypt_Aes_ECB_Matrix::Encrypt( uint8_t* input, uint32_t input_length,
													 uint8_t* output )
{
	Crypt::crypt_status_t status = DATA_ERROR;

	uint32_t data_len = 0U;

	if ( ( input_length > 0U ) && ( ( input_length & CRYPT_AES_ECB_BLOCK_LENGTH_MASK ) == 0U ) )
	{
		psAesInitBlockKey( m_ctx, m_key, m_key_size, PS_AES_ENCRYPT );
		do
		{
			psAesEncryptBlock( m_ctx, reinterpret_cast<uint8_t*>( input + data_len ),
							   reinterpret_cast<uint8_t*>( output + data_len ) );

			data_len += CRYPT_AES_ECB_BLOCK_LENGTH;
		} while ( data_len < input_length );
		status = SUCCESS;
	}
 ...
 *****************************************************************************************
 */
Crypt::crypt_status_t Crypt_Aes_ECB_Matrix::Decrypt( uint8_t* input, uint32_t input_length,
													 uint8_t* output )
{
	Crypt::crypt_status_t status = DATA_ERROR;
	uint32_t data_len = 0U;

	if ( ( input_length > 0 ) && ( ( input_length & CRYPT_AES_ECB_BLOCK_LENGTH_MASK ) == 0U ) )
	{
		psAesInitBlockKey( m_ctx, m_key, m_key_size, PS_AES_DECRYPT );
		do
		{
			psAesDecryptBlock( m_ctx, reinterpret_cast<uint8_t*>( input + data_len ),
							   reinterpret_cast<uint8_t*>( output + data_len ) );
													 ...
													 uint32_t* output_length )
{
	...
	uint32_t min_output_len = MIN_U8_ENCODED_AES_ECB_STRING_SIZE( input_length );
