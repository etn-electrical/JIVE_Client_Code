 ...
 *	@file Mram.h
 *
 *	@brief  MRAM is non-volatile random-access memory that offer the
...
#define MRAM_SEMAPHORE_TIMEOUT          2000U
#define MRAM_SPI_SEMAPHORE_TIMEOUT      2000U

typedef struct MRAM_CHIP_CONFIG_TD
{
...
/**
 ****************************************************************************************
 * @brief The MRAM class derived from the NV_Ctrl class contains implementations required for the access of the MRAM
 *memory.
 *
 * @details It provides functionalities to read, write, erase the MRAM memory.
 *
 ****************************************************************************************
 */
class MRAM : public NV_Ctrl
{
	public:
		 ...
		 * @param[in] chip_config: The chip configuration for MRAM.
		 * @return None
		 */
		MRAM( uC_SPI* spi_ctrl, uint8_t chip_select, MRAM_CHIP_CONFIG_TD const* chip_config );

		/**
		 *  @brief Destructor
		 *  @details It will get called when object of MRAM goes out of scope or deleted.
		 *  @return None
		 */
		~MRAM( void );
		...
		/**
		 * @brief This writes the MRAM Status Register content.
	...
	private:
		...
		void Send_Data_Command( uint8_t mram_command, NV_CTRL_ADDRESS_TD address );
		...
		MRAM_CHIP_CONFIG_TD const* m_chip_cfg;
		#ifndef MRAM_MIRROR_SUPPORT_DISABLE
			#define MRAM_MIRROR_SUPPORT true
		#else
			#define MRAM_MIRROR_SUPPORT false
		#endif
		/* Enable or disable the MIRROR support for FRAM */
		static const bool MIRROR_SUPPORT = MRAM_MIRROR_SUPPORT;
};
