 ...
 *****************************************************************************************
 */
#define MRAM_WRITE_STATUS_REGISTER_OPCODE       0x01U
#define MRAM_WRITE_MEMORY_ADD_DATA_OPCODE       0x02U
#define MRAM_READ_MEMORY_ADD_DATA_OPCODE        0x03U
#define MRAM_WRITE_DISABLE_OPCODE               0x04U
#define MRAM_READ_STATUS_REGISTER_OPCODE        0x05U
#define MRAM_SET_WRITE_ENABLE_LATCH_OPCODE      0x06U
#define MRAM_ENTER_SLEEP_MODE_OPCODE            0xB9U
#define MRAM_EXIT_SLEEP_MODE_OPCODE             0xABU

// #define MRAM_STATUS_BLOCK_PROTECT_0_TO_7FF		0x0CU
// #define MRAM_STATUS_BLOCK_PROTECT_400_TO_7FF	0x08U
// #define MRAM_STATUS_BLOCK_PROTECT_600_TO_7FF	0x04U
// #define MRAM_STATUS_BLOCK_PROTECT_600_TO_7FF	0x00U
#define MRAM_STATUS_WRITE_PROTECT_ENABLE_BIT    7U
#define MRAM_STATUS_BLOCK0_PROTECT_ENABLE_BIT   3U
#define MRAM_STATUS_BLOCK1_PROTECT_ENABLE_BIT   2U
#define MRAM_STATUS_WRITES_ENABLED_LATCH_BIT    1U

#define MRAM_STATUS_WRITE_PROTECT_ENABLE_BITMASK    ( 1U << MRAM_STATUS_WRITE_PROTECT_ENABLE_BIT )
#define MRAM_STATUS_BLOCK0_PROTECT_ENABLE_BITMASK   ( 1U << MRAM_STATUS_BLOCK0_PROTECT_ENABLE_BIT )
#define MRAM_STATUS_BLOCK1_PROTECT_ENABLE_BITMASK   ( 1U << MRAM_STATUS_BLOCK1_PROTECT_ENABLE_BIT )
#define MRAM_STATUS_WRITES_ENABLED_LATCH_BITMASK    ( 1U << MRAM_STATUS_WRITES_ENABLED_LATCH_BIT )
...
#define MAX_NUMBER_OF_MRAM_WRITES       2U
#define MAX_NUMBER_OF_MRAM_READS        2U

#define Delay_Chip_Select_Change()      uC_Delay( 1U )

#define MRAM_NV_CHECKSUM_LEN            2U
 ...
 *****************************************************************************************
 */
MRAM::MRAM( uC_SPI* spi_ctrl, uint8_t chip_select, MRAM_CHIP_CONFIG_TD const* chip_config ) :
	NV_Ctrl(),
	m_spi_ctrl( spi_ctrl ),
 ...
 *****************************************************************************************
 */
void MRAM::Execute_Op_Code( uint8_t op_code, uint8_t* rx_data, uint16_t rx_data_len )
{
...
MRAM::~MRAM( void )
{
	...
	m_chip_cfg = reinterpret_cast<MRAM_CHIP_CONFIG_TD const*>( nullptr );
 ...
 *****************************************************************************************
 */
NV_Ctrl::nv_status_t MRAM::Read( uint8_t* data, uint32_t address, uint32_t length,
								 bool use_protection )
{
	...
	{
		if ( m_spi_ctrl->Get_Semaphore( MRAM_SPI_SEMAPHORE_TIMEOUT ) )
		{
			read_count = 0U;
			do
			{
			...
			}while ( ( !good_data ) && ( read_count < MAX_NUMBER_OF_MRAM_READS ) );
 ...
 *****************************************************************************************
 */
NV_Ctrl::nv_status_t MRAM::Read_Checksum( uint16_t* checksum_dest, NV_CTRL_ADDRESS_TD address,
										  NV_CTRL_LENGTH_TD length )
{
	return ( Read( reinterpret_cast<uint8_t*>( checksum_dest ), address + length,
				   MRAM_NV_CHECKSUM_LEN, false ) );
}
 ...
 *****************************************************************************************
 */
NV_Ctrl::nv_status_t MRAM::Write( uint8_t* data, uint32_t address, uint32_t length,
								  bool use_protection )
{
	NV_Ctrl::nv_status_t return_status = NV_Ctrl::BUSY_ERROR;
	bool good_data;
	...
	if ( Check_Range( address, length, use_protection ) && ( length != 0U ) )
	{
		if ( m_spi_ctrl->Get_Semaphore( MRAM_SPI_SEMAPHORE_TIMEOUT ) )
		{
			...
			}while ( ( !good_data ) && ( write_count < MAX_NUMBER_OF_MRAM_WRITES ) );
			...
			else
			{
				return_status = NV_Ctrl::DATA_ERROR;
				NV_CTRL_PRINT( DBG_MSG_ERROR,
							   "Failed to write from Address: %X, Length: %u, UseProtection: %d, ErrorStatus: %d",
							   address, length, use_protection, return_status );
			}
 ...
 *****************************************************************************************
 */
NV_Ctrl::nv_status_t MRAM::Erase( uint32_t address, uint32_t length, uint8_t erase_data,
								  bool protected_data )
{
	NV_Ctrl::nv_status_t return_status = NV_Ctrl::BUSY_ERROR;
	...
	if ( Check_Range( address, length, protected_data ) )
	{
		if ( m_spi_ctrl->Get_Semaphore( MRAM_SPI_SEMAPHORE_TIMEOUT ) )
		{
			temp_address.u16 = static_cast<uint16_t>( address );// MRAM_FLUSH_START_ADDRESS_NVADD;

			block_count = 0U;
			do
			{
				m_spi_ctrl->Select_Chip( m_chip_select );	// SPI_PERIPH_Enable_CS( MRAM_DEVICE_ID );

				m_spi_ctrl->TX_Byte( MRAM_SET_WRITE_ENABLE_LATCH_OPCODE );
				while ( !m_spi_ctrl->TX_Shift_Empty() )
				{}

				m_spi_ctrl->De_Select_Chip( m_chip_select );// SPI_PERIPH_Disable_CS( MRAM_DEVICE_ID );

				Delay_Chip_Select_Change();

				m_spi_ctrl->Select_Chip( m_chip_select );	// SPI_PERIPH_Enable_CS( MRAM_DEVICE_ID );

				Send_Data_Command( MRAM_WRITE_MEMORY_ADD_DATA_OPCODE, temp_address.u16 );

				if ( true == protected_data )
				{
					length += MRAM_NV_CHECKSUM_LEN;
				}
 ...
 *****************************************************************************************
 */
NV_Ctrl::nv_status_t MRAM::Erase_All( void )
{
 ...
 *****************************************************************************************
 */
bool MRAM::Read_Now( uint8_t* data, uint32_t address, uint32_t length, bool use_protection )
{
	...
	Send_Data_Command( MRAM_READ_MEMORY_ADD_DATA_OPCODE, address );
	while ( !m_spi_ctrl->TX_Shift_Empty() )
	{}
 ...
 *****************************************************************************************
 */
bool MRAM::Write_Now( uint8_t* data, uint32_t address, uint32_t length, bool use_protection )
{
	...
	Send_Data_Command( MRAM_WRITE_MEMORY_ADD_DATA_OPCODE, address );
 ...
 *****************************************************************************************
 */
bool MRAM::Check_Data( uint8_t* check_data, uint32_t address, uint32_t length )
{
	bool good_data = true;
	uint8_t read_data;
 ...
 *****************************************************************************************
 */
void MRAM::Send_Data_Command( uint8_t mram_command, uint32_t address )
{
	...
	switch ( m_chip_cfg->address_length )
	{
		...
		default:
			m_spi_ctrl->TX_Byte( mram_command );
			while ( !m_spi_ctrl->TX_Data_Reg_Empty() )
			{}
 ...
 *****************************************************************************************
 */
bool MRAM::Read_Status( uint8_t* status )
{
	bool success = false;
	...
	if ( m_spi_ctrl->Get_Semaphore( MRAM_SPI_SEMAPHORE_TIMEOUT ) )
	{
		...
		m_spi_ctrl->TX_Byte( MRAM_READ_STATUS_REGISTER_OPCODE );
		while ( !m_spi_ctrl->TX_Shift_Empty() )
		{}
 ...
 *****************************************************************************************
 */
bool MRAM::Write_Status( uint8_t status )
{
	bool success = false;
	...
	if ( m_spi_ctrl->Get_Semaphore( MRAM_SPI_SEMAPHORE_TIMEOUT ) )
	{
		...
		m_spi_ctrl->TX_Byte( MRAM_WRITE_STATUS_REGISTER_OPCODE );
		while ( !m_spi_ctrl->TX_Data_Reg_Empty() )
		{}
 ...
 *****************************************************************************************
 */
bool MRAM::Check_Range( NV_CTRL_ADDRESS_TD address, NV_CTRL_LENGTH_TD length,
