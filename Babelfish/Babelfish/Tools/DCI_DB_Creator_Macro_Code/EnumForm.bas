...
Private Sub EnumNumber_TextBox_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger)
    ...
    'Allow keys 0 to 9, minus(-) and decimal point (.) for floating point numbers
    Cur_Data_Type = ActiveSheet.Cells(ActiveCell.row, DCI_DATATYPE_COL).value
    
    'If Data type is unsigned then we should not allow "Minus" key i.e. Negative numbers
    If (Cur_Data_Type = "DCI_DTYPE_UINT8") Or (Cur_Data_Type = "DCI_DTYPE_UINT16") Or (Cur_Data_Type = "DCI_DTYPE_UINT32") Or (Cur_Data_Type = "DCI_DTYPE_UINT64") Or (Cur_Data_Type = "DCI_DTYPE_BOOL") Then
        Is_Number_Unsigned = True
    End If
    If (KeyAscii >= 48 And KeyAscii <= 57) Or (KeyAscii = 45) Or (KeyAscii = 46) Then
        If (val(EnumNumber_TextBox.text & Chr(KeyAscii)) > MaxEnumVal) Then
            ...
            KeyAscii = 0
            Exit Sub
        ElseIf ((KeyAscii = 45) And (Is_Number_Unsigned = True)) Then
            MsgBox "Negative enum value is not allowed since Data type is unsigned"
            KeyAscii = 0
            Exit Sub
        End If
    Else
...
Private Sub NewUpdate_Button_Click()
    ...
    If IsNumeric(EnumNumber_TextBox.text) = False Then
        MsgBox "Enum# Invalid Value", vbCritical, "Invalid Enum Value"
        Exit Sub
    End If
    ...
    If NewUpdate_Button.caption = "Add" Then
        ...
        index = index + 1
		Items_ListView.ListItems.Add index, , PadLeft(EnumNumber_TextBox.text, 10, " ")
        ...
        '****** Disabled sorting because it sorts alphabetically, not numerically(fails for negative enum values)
        'Enable sorting
    ...
    Else
        ...
        For index = 1 To UBound(ListItems) Step 1
            If "" <> Trim(ListItems(index)) Then
				...
				Items_ListView.ListItems.Add index, , PadLeft(Trim(ParsedItems(0)), 10, " ") 
