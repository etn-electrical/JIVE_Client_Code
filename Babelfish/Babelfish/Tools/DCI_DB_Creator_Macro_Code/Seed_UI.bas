...
'Lower case vowels a,e,i are removed from the ENCODE_STRING to avoid formation of Java-Script keywords.
Private Const ENCODE_STRING = "bcdfghjklmpqrstuvwxyzABCDEFGHIJKLMPQRSTUVWXYZ"

Public removeExportFunct As Boolean
Public removeImportFunct As Boolean
...
'*********************
    ...
    Worksheets("Seed UI").Range("A:J").Font.name = "Arial"
    Worksheets("Seed UI").Range("A:J").Font.Size = 10
    
'****************************************************************************************************************************
'****** Check if the parameter mentioned in the seedUI sheet is present in the REST sheet.
'****************************************************************************************************************************
    Dim web_firstRow
    Dim PARAM As String
    Set web_dci = webSheet.Range("A:A")
    PARAM = "6 - Param"
    'Find the first row, which is the header
    Set web_dRow = web_dci.Find(what:="*", lookat:=xlWhole)
    'Save the header row so we know when we loop around
    web_firstRow = web_dRow.row
    'Loop through remaining until none left
    Set web_dRow = web_dci.Find(what:="*", After:=web_dRow, lookat:=xlWhole)
    'Clear the cell color
    Do While Not web_dRow.row = web_firstRow
        If web_dRow.Cells(, WEB_TYPE_COL).value = PARAM Then
            web_dRow.Cells(, WEB_NAME_COL).Interior.ColorIndex = 0
        End If
        Set web_dRow = web_dci.Find(what:="*", After:=web_dRow, lookat:=xlWhole)
    Loop
    ...
    Do While Not web_dRow.row = web_firstRow
        If web_dRow.Cells(, WEB_TYPE_COL).value = PARAM Then
            'If using DCI parameter as array then extracting only name of the parameter
            row_value = Split(web_dRow.Cells(, WEB_NAME_COL).value, "[")
            If getRow("REST", CStr(row_value(0))) Is Nothing Then
                web_dRow.Cells(, WEB_NAME_COL).Interior.ColorIndex = DCI_DESCRIPTORS_ERROR_CHECK_FAULT_COLOR
...
'Create a list of all DCI objects found on the REST page, with all their information
    ...
    counter = START_ROW
    count = 0
    removeExportFunct = True
    removeImportFunct = True
    ...
    While (restSheet.Cells(counter, REST_DATA_DEFINE_COL).text <> Empty) And (restSheet.Cells(counter, REST_DATA_DEFINE_COL).text <> BEGIN_IGNORED_DATA)
        ...
        Dim exportVal As String
        Dim importVal As String
        ...
        exportVal = restSheet.Cells(counter, REST_EXPORT_COL).value
        importVal = restSheet.Cells(counter, REST_IMPORT_COL).value
        
        If exportVal = "x" Then
            p.p_export = True
            removeExportFunct = False
        Else
            p.p_export = False
        End If
        ...
        p.format = escapeString(dRow.Cells(, DCI_FORMAT_COL).text)
        ...
        If (removeExportFunct = False) Then
            Call addLit(p.p_export, literals)
        End If
        If (removeImportFunct = False) Then
            Call addLit(p.p_import, literals)
        End If
    ...
    Do While Not dRow Is Nothing
        If IsNumeric(dRow.value) Then
            ...
            Do
                ...
                asms(count).params(pCount) = getRow("REST", dRow.Cells(pCount, 5)).Cells(, REST_PARAM_ID_COL).value
                Call addLit(asms(count).params(pCount), literals)
            Loop While (dRow.Cells(pCount + 1, 1) = Empty And dRow.Cells(pCount + 1, 5) <> Empty)
...
'****************************************************************************************************************************
    ...
    REST_DCI_JS.WriteLine ("var True = 1, False = 0;")
    ...
    REST_DCI_JS.WriteLine ("var _ = function (a, menum, modbus, mbitfield){")
    If (removeExportFunct = False) And (removeImportFunct = False) Then
        REST_DCI_JS.WriteLine ("    var obj = {define:a[0],pid:a[1],r:a[2],w:a[3],format:a[4],datatype:a[5],count:a[6],name:a[7],desc:a[8],units:a[9],export:a[10],import:a[11]};")
    ElseIf removeExportFunct Then
        REST_DCI_JS.WriteLine ("    var obj = {define:a[0],pid:a[1],r:a[2],w:a[3],format:a[4],datatype:a[5],count:a[6],name:a[7],desc:a[8],units:a[9],import:a[10]};")
    ElseIf removeImportFunct Then
        REST_DCI_JS.WriteLine ("    var obj = {define:a[0],pid:a[1],r:a[2],w:a[3],format:a[4],datatype:a[5],count:a[6],name:a[7],desc:a[8],units:a[9],export:a[10]};")
    End If
    ...
    REST_DCI_JS.WriteLine ("        }")
    REST_DCI_JS.WriteLine ("    }")
    REST_DCI_JS.WriteLine ("    if (mbitfield) {")
    REST_DCI_JS.WriteLine ("        if (Array.isArray(mbitfield[0])) {")
    REST_DCI_JS.WriteLine ("            obj.bitfield=mbitfield[0];")
    REST_DCI_JS.WriteLine ("            obj.bitfield_desc=mbitfield[1];")
    REST_DCI_JS.WriteLine ("        } else {")
    REST_DCI_JS.WriteLine ("            obj.min=mbitfield[0];")
    REST_DCI_JS.WriteLine ("            obj.max=mbitfield[1];")
    REST_DCI_JS.WriteLine ("        }")
    REST_DCI_JS.WriteLine ("    }")
    ...
    For i = 0 To UBound(params)
        ...
        Dim mbArg As String
        Dim mbArr() As String
        Dim mEnumDetailsFormat As String
        Dim mFlagIndexLast As Boolean
        Dim mTemp As String
        Dim symLocation As Integer
        Dim mbitfield As String
        Dim mBitfieldDetailsFormat As String

        Dim ws As Excel.Worksheet
        Dim FoundCell As Excel.Range
        
        'Flag, detecting the Enums having Enum Details not filled
        mFlagIndexLast = False
        mFlagHasEnumDetails = False
        'Flag, detecting the Bitfields having Bitfield Details not filled
        mFlagHasBitfieldDetails = False
        ...
        Set FoundCell = ws.Range("B:B").Find(what:=p.define, lookat:=xlWhole)
        
        RawString = Trim(Worksheets(DCI_DESCRIPTORS_SHEET).Cells(FoundCell.row, ENUMS_COL))
        ...
        If RawString <> "" Then
            ...
            For index = 1 To UBound(DCI_Enum_Details) Step 1
                If "" <> Trim(DCI_Enum_Details(index)) Then
                    ...
                    'Fill the DCI_Enum_Details string array here
                    '
                    DCI_Enum_Details(index) = ParsedItems(2)
                    mFlagHasEnumDetails = True
                End If
            Next index
        Else
            mFlagHasEnumDetails = False
        ...
        'Bitfield data extraction
        BitfieldRawString = Trim(Worksheets(DCI_DESCRIPTORS_SHEET).Cells(FoundCell.row, BITFIELDS_COL))
        
        If BitfieldRawString <> "" Then
            DCI_Bitfield_Descriptions = Split(BitfieldRawString, ITEM_DELIMITER)
            For index = 1 To UBound(DCI_Bitfield_Descriptions) Step 1
                If "" <> Trim(DCI_Bitfield_Descriptions(index)) Then
                    ParsedItems = Split(DCI_Bitfield_Descriptions(index), SUB_ITEM_DELIMITER)
                    'Remove line feeds
                    ParsedItems(0) = Replace(ParsedItems(0), vbLf, "")
                    ParsedItems(1) = Replace(ParsedItems(1), vbLf, "")
                    ParsedItems(2) = Replace(ParsedItems(2), vbLf, "")
                    
                    'Fill the DCI_Bitfield_Descriptions string array here
                    '
                    DCI_Bitfield_Descriptions(index) = ParsedItems(2)
                End If
            Next index

            DCI_Bitfield_Values = Split(BitfieldRawString, ITEM_DELIMITER)
            For index = 1 To UBound(DCI_Bitfield_Values) Step 1
                If "" <> Trim(DCI_Bitfield_Values(index)) Then
                    ParsedItems = Split(DCI_Bitfield_Values(index), SUB_ITEM_DELIMITER)
                    'Remove line feeds
                    ParsedItems(0) = Replace(ParsedItems(0), vbLf, "")
                    ParsedItems(1) = Replace(ParsedItems(1), vbLf, "")
                    ParsedItems(2) = Replace(ParsedItems(2), vbLf, "")
                    
                    'Fill the DCI_Bitfield_Values string array here
                    '
                    DCI_Bitfield_Values(index) = ParsedItems(0)
                End If
            Next index
            mFlagHasBitfieldDetails = True
        Else
            mFlagHasBitfieldDetails = False
        End If
        ...
        If (removeExportFunct = False) And (removeImportFunct = False) Then
            a = "[" & Join(Array(convertToRefs(p.define, literals), _
                       ...
                       convertToRefs(p.units, literals), _
                       convertToRefs(p.p_export, literals), _
                       convertToRefs(p.p_import, literals)), ",") & "]"
        ElseIf removeExportFunct Then
            a = "[" & Join(Array(convertToRefs(p.define, literals), _
                       convertToRefs(p.pid, literals), _
                       convertToRefs(p.r, literals), _
        ...
        ElseIf removeImportFunct Then
            a = "[" & Join(Array(convertToRefs(p.define, literals), _
                       ...
                       convertToRefs(p.desc, literals), _
                       convertToRefs(p.units, literals), _
                       convertToRefs(p.p_export, literals)), ",") & "]"
        End If
        ...
        ElseIf Not IsEmpty(p.enums) Then
            ...
            'Check for the Time and Date Enum Details that needs to be appended into the
            'dci.js file that will be generated.
            ElseIf (mFlagHasEnumDetails = True) Then
                mEnumDetailsFormat = "["
                For j = 1 To CInt(DCI_Enum_Details(0))
                    symLocation = InStr(DCI_Enum_Details(j), "'")
                    If symLocation <> 0 Then
                        DCI_Enum_Details(j) = Replace(DCI_Enum_Details(j), "'", "\'")
                    End If
                    mEnumDetailsFormat = mEnumDetailsFormat & "'" & DCI_Enum_Details(j) & "'"
                    If j <> (DCI_Enum_Details(0)) Then
                        mEnumDetailsFormat = mEnumDetailsFormat & ","
                    End If
                Next j
                mEnumDetailsFormat = mEnumDetailsFormat & "]"
                menum = "[[" & Join(p.enums, ",") & "]" & "," & mEnumDetailsFormat & "]"
            Else
                menum = "[[" & Join(p.enums, ",") & "]]"
            End If
        ...
        'Add Bitfield details to dci.js ----------------------------------------
        If (mFlagHasBitfieldDetails = True) Then
                mBitfieldDetailsFormat = "["
                For j = 1 To CInt(DCI_Bitfield_Descriptions(0))
                    symLocation = InStr(DCI_Bitfield_Descriptions(j), "'")
                    If symLocation <> 0 Then
                        DCI_Bitfield_Descriptions(j) = Replace(DCI_Bitfield_Descriptions(j), "'", "\'")
                    End If
                    mBitfieldDetailsFormat = mBitfieldDetailsFormat & "'" & DCI_Bitfield_Descriptions(j) & "'"
                    If j <> (DCI_Bitfield_Descriptions(0)) Then
                        mBitfieldDetailsFormat = mBitfieldDetailsFormat & ","
                    End If
                Next j
                mBitfieldDetailsFormat = mBitfieldDetailsFormat & "]"
                
                Dim arrTest() As Variant
                counter = DCI_Bitfield_Values(0)      
                ReDim Preserve arrTest(1 To counter)
                
                For k = 1 To CInt(DCI_Bitfield_Values(0))
                    arrTest(k) = DCI_Bitfield_Values(k)
                Next k
                mbitfield = "[[" & Join(arrTest, ",") & "]" & "," & mBitfieldDetailsFormat & "]"
        End If
        '-----------------------------------------------------------------------
        ...
        If Len(mbArg) = 0 Then
            If Len(menum) = 0 Then
				If Len(mbitfield) = 0 Then
					REST_DCI_JS.WriteLine ("_(" & a & "),")
				Else
					REST_DCI_JS.WriteLine ("_(" & a & ",0,0," & mbitfield & "),")
				End If
            Else
				If Len(mbitfield) = 0 Then
					REST_DCI_JS.WriteLine ("_(" & a & "," & menum & "),")
				Else
					REST_DCI_JS.WriteLine ("_(" & a & "," & menum & ",0," & mbitfield & "),")
				End If
            End If
        Else
            If Len(menum) = 0 Then
				...
				Else
				    REST_DCI_JS.WriteLine ("_(" & a & ",0," & mbArg & "," & mbitfield & "),")
				End If
            Else
				If Len(mbitfield) = 0 Then
					REST_DCI_JS.WriteLine ("_(" & a & "," & menum & "," & mbArg & "),")
				Else
					REST_DCI_JS.WriteLine ("_(" & a & "," & menum & "," & mbArg & "," & mbitfield & "),")
				End If
            End If
        End If
    Next i
    
    'If the Flag has detected (gets true) the Enum Details not filled for any of the rows
    'having Enum defined for it, then notify about the same.
    If (mFlagIndexLast = True) And (mFlagHasEnumDetails = False) Then
        MsgBox "DCI defines with enum declarations detected in the REST sheet! Please check if the <Enum Details> column also needs to be filled for the respective enums.", , "DCI Defines with Enums"
        mFlagIndexLast = False
    End If
...
'    Next i
    ...
    'Closes the TextStream of REST_DCI_JS
    REST_DCI_JS.Close
...
'Move favicon file to the specific folder
Sub Move_File()
    Const FAVICON_ROW = 1
    Const FAVICON_COL = 7
    Dim FAVICON_DEST_PATH As String
    FAVICON_DEST_PATH = "\..\Babelfish\Code\Ethernet\Web\WebUI\LTK_Seed\app\images\"
    Sheets("Seed UI").Select
    If (Cells(FAVICON_ROW, FAVICON_COL).text <> Empty) Then
        Set file_system = CreateObject("Scripting.FileSystemObject")
        source_file = ActiveWorkbook.Path & "\" & Cells(FAVICON_ROW, FAVICON_COL).value
        destination_path = ActiveWorkbook.Path & FAVICON_DEST_PATH
        If file_system.FileExists(source_file) Then
            file_system.CopyFile (source_file), destination_path
        End If
    End If
End Sub

'Converts the encoding of dci.js (ANSI to UTF-8)
'There is no easy way to convert the ANSI directly to UTF-8 without BOM(byte order mark), hence first converting ANSI to ISO, then ISO to UTF-8 removing the BOM
Sub ANSI_TO_UTF8()
Dim SourcePath, DestinationPath, FilePath, CurrentDirectory, UTFStream, ANSIStream, BinaryStream
...
FilePath = Replace(CurrentDirectory, "Tools", "Babelfish")
SourcePath = FilePath & "\Code\Ethernet\Web\WebUI\LTK_Seed\app\scripts\lib\dci.js"
DestinationPath = FilePath & "\Code\Ethernet\Web\WebUI\LTK_Seed\app\scripts\lib\dci.js"
	...
	'Conversion of ANSI to ISO
    ANSIStream.Type = adTypeText
    ANSIStream.Charset = "iso-8859-1"
    ANSIStream.Open
    ANSIStream.LoadFromFile SourcePath  'ANSI File
	...
	'Removing BOM to avoid issue with uglification while generating the codepack
    ...
    BinaryStream.SaveToFile DestinationPath, adSaveCreateOverWrite
    BinaryStream.Flush
    BinaryStream.Close
