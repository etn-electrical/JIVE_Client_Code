Attribute VB_Name = "XXX_Delta_Com_Virtual"
'Global constants for some reason
Dim hex_string As String
Dim bit_list_word As Double

Sub XXX_Create_Virtual_Device_OLD()
...
'****************************************************************************************************************************
    missing_row_counter = PARAMxDCI_VIRT_DATA_START_ROW
    While (Cells(missing_row_counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty)
        missing_row_counter = missing_row_counter + 1
    Wend
    Range(Cells(missing_row_counter, 1), Cells(PARAMxDCI_VIRT_MAX_ROW, PARAMxDCI_VIRT_MAX_COL)).Clear
...
'****************************************************************************************************************************
    ...
    While Worksheets("DCI Descriptors").Range("A1").Cells(dci_defines_row_ctr, DCI_DEFINES_COL).value <> Empty
        ...
        If C Is Nothing Then '
            Cells(missing_row_counter, PARAMxDCI_VIRT_DCI_DESCRIPT_COL).value = Worksheets("DCI Descriptors").Range("A1").Cells(dci_defines_row_ctr, DCI_DESCRIPTORS_COL).value
            Cells(missing_row_counter, PARAMxDCI_VIRT_DCI_DEFINE_COL).value = Worksheets("DCI Descriptors").Range("A1").Cells(dci_defines_row_ctr, DCI_DEFINES_COL).value
            missing_row_counter = missing_row_counter + 1
        End If
...
'****************************************************************************************************************************
    num_ids = 0
    client_truth_subscription_list_items = 0
    server_truth_subscription_list_items = 0
    publication_list_items = 0
    
    error = False
    warning = False
    Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_ERROR_COL).Interior.ColorIndex = xlNone
    Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_ERROR_COL).value = "Error?"
    Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_WARN_COL).Interior.ColorIndex = xlNone
    Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_WARN_COL).value = "Warning?"

    counter = PARAMxDCI_VIRT_DATA_START_ROW
    Dim dci_row As Integer
    Dim param_row As Integer
    While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And _
                (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
        Cells(counter, PARAMxDCI_VIRT_ERROR_COL).Interior.ColorIndex = xlNone
        Cells(counter, PARAMxDCI_VIRT_ERROR_COL).value = ""
        Cells(counter, PARAMxDCI_VIRT_WARN_COL).Interior.ColorIndex = xlNone
        Cells(counter, PARAMxDCI_VIRT_WARN_COL).value = ""

        dci_row = Get_DCI_Row(Cells(counter, PARAMxDCI_VIRT_DCI_DEFINE_COL).value)
        If (dci_row <> 0) Then
            Cells(counter, PARAMxDCI_VIRT_DCID_COL).value = dci_row - DCI_DEFINES_START_ROW
            dci_length = Worksheets("DCI Descriptors").Cells(dci_row, DCI_LENGTH_COL).value
        Else
            Cells(counter, PARAMxDCI_VIRT_ERROR_COL).value = "DCID Does Not Exist Error"
            Cells(counter, PARAMxDCI_VIRT_ERROR_COL).Interior.Color = RGB(255, 0, 0)
            error = True
        End If

        If (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> "") Then
            param_row = Get_Param_ID_Row(Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value)
        Else
            param_row = 0
        End If

        If (param_row <> 0) Then
            Cells(counter, PARAMxDCI_VIRT_PARAM_ID_COL).value = Get_Param_ID_From_Row(param_row)
            pid_length = Get_Param_Length_From_Row(param_row)
            If ((dci_length <> pid_length) And (pid_length <> 0)) Then
                Cells(counter, PARAMxDCI_VIRT_ERROR_COL).Interior.Color = RGB(255, 0, 0)
                Cells(counter, PARAMxDCI_VIRT_ERROR_COL).value = "Size Mismatch"
            End If
        Else
            Cells(counter, PARAMxDCI_VIRT_ERROR_COL).value = "PID Does Not Exist Error"
            Cells(counter, PARAMxDCI_VIRT_ERROR_COL).Interior.Color = RGB(255, 0, 0)
            error = True
        End If
        
        If (Cells(counter, PARAMxDCI_VIRT_SUBSCRIPTION_COL).value <> "") Then
            'subscription_list_items = subscription_list_items + 1
            client_truth_subscription_list_items = client_truth_subscriptions_list_items + 1
            server_truth_subscription_list_items = server_truth_subscriptions_list_items + 1
'            Cells(counter, PARAMxDCI_VIRT_SUBSCRIPTION_COL).Value = "x"
        End If

        'If (Cells(counter, PARAMxDCI_VIRT_PUBLICATION_COL).Value <> "") Then
        '    publication_list_items = publication_list_items + 1
        '    Cells(counter, PARAMxDCI_VIRT_PUBLICATION_COL).Value = "x"
        'End If
    ...
    '****************************************************************************************************************************
        ...
        FILE_DCI_H.WriteLine ("*")
        FILE_DCI_H.WriteLine ("*   @brief Provides the Virtual Device definition.")
        FILE_DCI_H.WriteLine ("*")
        FILE_DCI_H.WriteLine ("*   @details A virtual device needs a definition passed into it.  This definition needs")
        FILE_DCI_H.WriteLine ("*   a subscription list, a publication list and a DCID to Param ID mapping.")
        FILE_DCI_H.WriteLine ("*")
    ...
    '****************************************************************************************************************************
    '******     Gotta sort the data first by Param ID.
    '****************************************************************************************************************************
        If (num_ids <> 0) Then
            range_string = PARAMxDCI_VIRT_DATA_START_ROW & ":" & (num_ids + PARAMxDCI_VIRT_DATA_START_ROW)
    ...
    '******     Fill in the sort key list.
        sort_key_counter = 0
        counter = PARAMxDCI_VIRT_DATA_START_ROW
        While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
    ...
    '****************************************************************************************************************************
    '******     Handle The Param ID to DCID List.
    '****************************************************************************************************************************
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("//******     Parameter ID to DCID")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("")
        ...
        If (num_ids <> 0) Then
            ...
            While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                ...
                FILE_DCI_C.WriteLine (Chr(9) & Chr(9) & gen_data_namespace_colons & Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value & "_PID,      //" & Cells(counter, PARAMxDCI_VIRT_PARAM_DESCRIPT_COL).value)
                FILE_DCI_C.WriteLine (Chr(9) & Chr(9) & Cells(counter, PARAMxDCI_VIRT_DCI_DEFINE_COL).value & "_DCID,      //" & Cells(counter, PARAMxDCI_VIRT_DCI_DESCRIPT_COL).value)
                FILE_DCI_C.WriteLine (Chr(9) & "},")
                counter = counter + 1
            Wend
            FILE_DCI_C.WriteLine ("};")
        Else
            FILE_DCI_C.WriteLine ("static const pid_to_dcid_map_t* const " & upper_case_file_name & _
    ...
    '****************************************************************************************************************************
        ...
        If (num_ids <> 0) Then
            ...
            While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                offset_counter = 0
                bit_list_word = 0
                While (offset_counter < BIT_LIST_PACK_LEN) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                    If (Cells(counter, PARAMxDCI_VIRT_OWNER_PATRON_COL).value <> "") Then
                    bit_list_word = bit_list_word + 2 ^ offset_counter
                    FILE_DCI_C.WriteLine ("//    " & Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value & "_PID,")
                    End If
        ...
        While (list_counter < 2)
            ...
            If (num_ids <> 0) Then
                If (general_access = True) Then
                    FILE_DCI_C.WriteLine ("static const uint32_t " & upper_case_file_name & _
                            "_GEN_SUBSCRIPTION_BIT_LIST[" & upper_case_file_name & "_BIT_LIST_LEN] =")
                Else
                    FILE_DCI_C.WriteLine ("static const uint32_t " & upper_case_file_name & _
                            "_SUBSCRIPTION_BIT_LIST[" & upper_case_file_name & "_BIT_LIST_LEN] =")
                End If
                FILE_DCI_C.WriteLine ("{")
                ...
                While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                    ...
                    bit_list_word = 0
                    While (offset_counter < BIT_LIST_PACK_LEN) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                        If (Cells(counter, PARAMxDCI_VIRT_SUBSCRIPTION_COL).value <> "") And (Cells(counter, PARAMxDCI_VIRT_SOUCE_OF_TRUTH_COL).value = "") Then
            ...
            Else
                FILE_DCI_C.WriteLine ("static const uint32_t* const " & upper_case_file_name & _
                            "_CLIENT_TRUTH_BIT_LIST = nullptr;")
            End If
            ...
            If (num_ids <> 0) Then
                If (general_access = True) Then
                    FILE_DCI_C.WriteLine ("static const uint32_t " & upper_case_file_name & _
                                "_GEN_CLIENT_SOT_BIT_LIST[" & upper_case_file_name & "_BIT_LIST_LEN] =")
                Else
                    FILE_DCI_C.WriteLine ("static const uint32_t " & upper_case_file_name & _
                                "_CLIENT_SOT_BIT_LIST[" & upper_case_file_name & "_BIT_LIST_LEN] =")
                End If
                FILE_DCI_C.WriteLine ("{")
    ...
    '****************************************************************************************************************************
        ...
        If (num_ids <> 0) Then
            ...
            counter = PARAMxDCI_SRVR_DATA_START_ROW
            While (Cells(counter, PARAMxDCI_SRVR_DCI_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_SRVR_DCI_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                offset_counter = 0
                bit_list_word = 0
                While (offset_counter < BIT_LIST_PACK_LEN) And (Cells(counter, PARAMxDCI_SRVR_DCI_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                    If (Cells(counter, PARAMxDCI_SRVR_SUBSCRIPTION_COL).value = "c") Or (Cells(counter, PARAMxDCI_SRVR_SUBSCRIPTION_COL).value = "C") Then
                    bit_list_word = bit_list_word + 2 ^ offset_counter
                    FILE_DCI_C.WriteLine ("//    " & Cells(counter, PARAMxDCI_SRVR_PARAM_DEFINE_COL).value & "_PID,")
                    End If
                    offset_counter = offset_counter + 1
                    counter = counter + 1
    ...
    '****************************************************************************************************************************
    '******     Now sort the list for converting from DCID to PID.
    '****************************************************************************************************************************
        If (num_ids <> 0) Then
            range_string = PARAMxDCI_VIRT_DATA_START_ROW & ":" & (num_ids + PARAMxDCI_VIRT_DATA_START_ROW)
            Rows(range_string).Sort Key1:=Range("I9"), Order1:=xlAscending, _
                Header:=xlGuess, OrderCustom:=1, MatchCase:=False, _
                Orientation:=xlTopToBottom
        End If
        ...
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("//******     DCID To Parameter ID")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("")
        ...
        If (num_ids <> 0) Then
            ...
            counter = PARAMxDCI_VIRT_DATA_START_ROW
            While (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> Empty) And (Cells(counter, PARAMxDCI_VIRT_PARAM_DEFINE_COL).value <> BEGIN_IGNORED_DATA)
                FILE_DCI_C.WriteLine ("        " & Cells(counter, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL).value & "U,      //" & Cells(counter, PARAMxDCI_VIRT_PARAM_DESCRIPT_COL).value)
                counter = counter + 1
            Wend
            FILE_DCI_C.WriteLine ("};")
        Else
        ...
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("//******     Virtual Device Data Definition")
        FILE_DCI_C.WriteLine ("//*******************************************************")
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("const Delta_Data::data_def_t " & upper_case_file_name & " = ")
        FILE_DCI_C.WriteLine ("{")
        FILE_DCI_C.WriteLine ("    " & upper_case_file_name & "_PID_DCID_LIST_LEN,      //const uint16_t m_num_of_params;")
        FILE_DCI_C.WriteLine ("    " & upper_case_file_name & "_PID_TO_DCID,      //const pid_to_dcid_map_t *m_pid_to_dcid;")
        FILE_DCI_C.WriteLine ("    " & upper_case_file_name & "_DCID_TO_PID,      //const uint16_t *m_dcid_to_pid;")
        ...
        FILE_DCI_C.WriteLine ("")
        FILE_DCI_C.WriteLine ("    reinterpret_cast<const uint8_t*>(" & upper_case_file_name & "_SUBSCRIPTION_BIT_LIST),      //uint8_t * subscription_bit_list;")
        FILE_DCI_C.WriteLine ("    reinterpret_cast<const uint8_t*>(" & upper_case_file_name & "_CLIENT_SOT_BIT_LIST),      //uint8_t * client_sot_subscription_bit_list;")
        FILE_DCI_C.WriteLine ("    reinterpret_cast<const uint8_t*>(" & upper_case_file_name & "_GEN_SUBSCRIPTION_BIT_LIST),      //uint8_t * general_subscription_bit_list;")
        FILE_DCI_C.WriteLine ("    reinterpret_cast<const uint8_t*>(" & upper_case_file_name & "_GEN_CLIENT_SOT_BIT_LIST),      //uint8_t * general_client_sot_subscription_bit_list;")
        FILE_DCI_C.WriteLine ("")
    ...
    '****************************************************************************************************************************
        ...
        Range(Cells(PARAMxDCI_VIRT_DATA_START_ROW, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL), Cells(PARAMxDCI_VIRT_MAX_ROW, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL)).Clear
        If (num_ids <> 0) Then
            Cells(PARAMxDCI_VIRT_DATA_START_ROW, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL).value = "---"
            Range(Cells(PARAMxDCI_VIRT_DATA_START_ROW, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL), Cells(num_ids + PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_PARAM_ID_SORT_KEY_COL)).FillDown
        End If
        
    End If
        
    If (error = True) Then
        Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_ERROR_COL).Interior.Color = RGB(255, 0, 0)
    End If
    If (warning = True) Then
        Cells(PARAMxDCI_VIRT_DATA_START_ROW - 1, PARAMxDCI_VIRT_WARN_COL).Interior.Color = RGB(255, 255, 0)
    End If
