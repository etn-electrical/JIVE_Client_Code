/**
 **********************************************************************************************
 * @file            KSZ8041 source File for driver class.
 *
 * @brief           The file contains KSZ8041 driver class to read/write KSZ8041
 *                  single/dual port switch registers over a MII/RMII communication interface.
 * @details         This file is used to communicate KSZ8041.
...
const uint8_t* const KSZ8041::s_phy_err_name[] =
{
	"link_up",
	"link_down",
	"jabber",
 ...
 *****************************************************************************************
 */
KSZ8041::KSZ8041( BF_Lib::Output* reset_ctrl, EMAC_Driver* emac_ctrl, bool_t switch_enable,
				  bool_t alr_learn,
				  uint8_t PhyAddr ) :
 ...
 *****************************************************************************************
 */
bool_t KSZ8041::Read_PHY_Reg( uint16_t phy_addr, uint16_t reg_addr,
							  uint16_t* read_ptr )
{
 ...
 *****************************************************************************************
 */
bool_t KSZ8041::Write_PHY_Reg( uint16_t phy_addr, uint16_t reg_addr,
							   uint16_t* write_ptr )
{
	BF_ASSERT( Valid_Phy_Addr( phy_addr ) );
	BF_ASSERT( Valid_Reg_Addr( reg_addr ) );
 ...
 *****************************************************************************************
 */
bool_t KSZ8041::Write_Port_Config( uint8_t port, PHY_Driver::port_config_t* config_word )
{
	uint16_t reg_val;
	uint16_t reg_addr;
	bool_t result = true;
 ...
 *****************************************************************************************
 */
bool_t KSZ8041::Read_Port_Config( uint8_t port, PHY_Driver::port_config_t* config_word )
{
	uint16_t reg_addr;
	bool_t read_status;
	uint16_t Phy_port;
				...
				if ( Is_Link_Good( basic_status_value, static_cast<uint16_t>( PHY_STATUS_LINK ) ) )
				{
						 ...
						 ( config_word->fields.auto_neg_done == AUTO_NEG_DONE ) )
					{
						...
						if ( read_status )
						{
							...
							/* storing the port and duplex information to be compared with previous polled information
							 */
 ...
 *****************************************************************************************
 */
void KSZ8041::Update_PHY_Error_Counts( uint16_t Phy_port, PHY_Driver::port_config_t* config_word,
 ...
 *****************************************************************************************
 */
const uint8_t* const* KSZ8041::Get_Phy_Driver_Error_List()
{
