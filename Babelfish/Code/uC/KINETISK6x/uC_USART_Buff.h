...
class uC_USART_Buff
{
	public:
		...
		static const cback_status_t RX_BUS_IDLE = 2U;
		static const cback_status_t TX_BUFF_EMPTY = 3U;					// < The data buffer is empty and ready to
																		// transmit again.
		static const cback_status_t TX_COMPLETE = 4U;			// < The shift register is empty and the wire should be
																// clear.
		static const cback_status_t TXRX_HOLDOFF_COMPLETE = 5U;	// < Half duplex holdoff.  RX resets holdoff.  This will
																// override TX_HOLDOFF_COMPLETE.
		static const cback_status_t TX_HOLDOFF_COMPLETE = 6U;		// < We only monitor the TX for holdoff.
		static const cback_status_t RX_BYTE_COMPLETE = 7U;		// < To detect end of frame.
