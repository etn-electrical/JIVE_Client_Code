...
uC_I2C::uC_I2C( uint8_t channel, uint16_t device_address, i2c_mode_t speed_mode,
				...
				duty_type_t fastmode_duty_cycle ) :
	...
	m_io_ctrl(
		reinterpret_cast<uC_BASE_I2C_IO_PORT_STRUCT*>( nullptr ) ),
	m_f_duty_cycle(
	...
	m_tx_dma(
		reinterpret_cast<uC_DMA*>( nullptr ) )
{
	m_i2c_speed_type = speed_mode;
					   ...
					   duty_type_t fmode_duty_cycle )
{
	...
	if ( I2C_FAST_MODE == i2c_mode )
	{
		...
		m_i2c->TRISE =
			static_cast<uint16_t>( ( i2c_pclk1_freq * 3U ) / ( 10000000U ) ) + 1U;	// in fast mode, the maximum
																					// allowed SCL rise time is 300
																					// ns
	}
	else
	{
...
{
	// Function updated to fix ICDUNIKITY-417 (Ethernet port Auto negotiation value didn�t update other than 4)
	...
	{	/* State for sending device address/ start frame ACK */
		case START_STATE:
			if ( true == static_cast<bool_t>( Get_SR1() & I2C_SR1_SB ) )
			{
				...
				else if ( ( m_chip_address > MAX_END_ADDR_7_BIT ) && ( m_chip_address < MAX_END_ADDR_10_BIT ) )
				{
