 ...
 *****************************************************************************************
 */
#define START_K60_INT_VEC_INDEX         ( 0x40U >> DIV_BY_4 )
#define NVIC_ARRAY_SIZE                     ( static_cast<uint_fast16_t>( NVIC_ENUM_SIZE ) + START_K60_INT_VEC_INDEX )
...
#define NVIC_VECT_TBLE_ALIGN_FORCE_MASK         ( 0x0000FF00U )
#define NVIC_VECT_TBLE_ALIGN_CHECK_MASK         ( 0xFFU )

/*
 *@remark Coding Standard Deviation:
 *@n MISRA-C++[2008] Required Rule 17-0-2, Re-use of C++ identifier pattern: __vector_table
 *@n Justification: The pattern of __region_RAM2_start__.
 *@n are determined by the linker and cannot be changed.
 */
/*lint -e{1960} # MISRA Deviation */
extern "C" {
extern uint8_t __region_RAM2_start__;
}
// Let us initialize the private static const member of the class
uint32_t const uC_Interrupt::NVIC_VectTab_RAM = ( uint32_t ) ( &__region_RAM2_start__ );
							   ...
							   uint16_t priority )
{
	...
	vector_index = static_cast<uint_fast16_t>( vector_number ) +
		static_cast<uint_fast16_t>( START_K60_INT_VEC_INDEX );
	/**
	 *@remark Coding Standard Deviation:
	 *@n MISRA-C++[2008] Required Rule 5-2-7, 5-2-8, 5-2-9, cast from enum to unsigned short
	 *@n Justification: PC-lint false alarm.  5-2-7, 5-2-8, and 5-2-9 involve casting pointers, not enums.
...
void uC_Interrupt::Clear_Vector( IRQn_Type vector_number )
{
	 ...
	 */
	/*lint -e{930} # MISRA Deviation */
	vector_index = static_cast<uint_fast16_t>( vector_number ) + START_K60_INT_VEC_INDEX;
...
bool uC_Interrupt::Is_Vector_Empty( IRQn_Type vector_number )
{
	 ...
	 */
	/*lint -e{930} # MISRA Deviation */
	vector_index = static_cast<uint16_t>( vector_number ) + START_K60_INT_VEC_INDEX;
