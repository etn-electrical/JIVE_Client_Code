...
UINT16 baud_divisors[16] = { 2, 4, 6, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768 };
UINT8 baud_prescalers[4] = { 2, 3, 5, 7 };
...
uC_SPI::uC_SPI( uint8_t spi_select, bool master, uC_USER_IO_STRUCT const* const* chip_sel_ctrls,
				uint8_t total_chip_sel, uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl, bool enable_dma ) :
	m_config( reinterpret_cast<uC_BASE_SPI_IO_STRUCT const*>( nullptr ) ),
	m_spi( reinterpret_cast<SPI_Type*>( nullptr ) ),
	...
	m_semaphore( nullptr )
{
	...
	// switch( spi_select )
	// {
	// case uC_BASE_SPI_CTRL_0:
	// SIM->SCGC6 |= SIM_SCGC6_DSPI0_MASK;
	// break;
	// case uC_BASE_SPI_CTRL_1:
	// SIM->SCGC6 |= SIM_SCGC6_DSPI1_MASK;
	// break;
	// case uC_BASE_SPI_CTRL_2:
	...
	if ( master == true )
	{
		...
		uC_IO_Config::Enable_Periph_Output_Pin( io_ctrl->cs_pio, TRUE );// still want to configure the pin, just set
																		// it as GPIO
	}
	else
	{
		...
		uC_IO_Config::Enable_Periph_Input_Pin( io_ctrl->cs_pio );
	}
	m_spi->MCR &= ~( SPI_MCR_HALT_MASK | SPI_MCR_MDIS_MASK );
	...
	{
		m_rx_dma =
			new uC_DMA( m_config->rx_dma_channel, uC_DMA_DIR_FROM_PERIPH, static_cast<uint8_t>( sizeof( uint8_t ) ),
						uC_DMA_MEM_INC_TRUE, uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );

		m_tx_dma =
			new uC_DMA( m_config->tx_dma_channel, uC_DMA_DIR_FROM_MEMORY, static_cast<uint8_t>( sizeof( uint8_t ) ),
						uC_DMA_MEM_INC_TRUE, uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );
		m_spi->RSER |= SPI_RSER_TFFF_DIRS_MASK;
		m_spi->RSER |= SPI_RSER_RFDF_DIRS_MASK;
	}
	// m_spi->CR2 = m_base_cfg.cr2;	//The SSOE bit has to happen before we do a SPI enable.
	// m_spi->CR1 = m_base_cfg.cr1;
}
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	/*
	        m_chip_sel_cfg[chip_select].cr1 = m_base_cfg.cr1;	//( m_spi->CR1 & ~SPI_CR1_SPE );
	        m_chip_sel_cfg[chip_select].cr2 = m_base_cfg.cr2;	//m_spi->CR2;
	        ...
	        switch ( spi_mode )
	        {
	            ...
	            case uC_SPI_MODE_3:
	                m_chip_sel_cfg[chip_select].cr1 |= static_cast<uint16_t>( SPI_CR1_CPOL | SPI_CR1_CPHA );
	                break;
	 ...
	 */
	m_spi->MCR |= SPI_MCR_HALT_MASK;
	if ( bits_to_transfer == 16U )
	{
		m_spi->CTAR[0] &= ~SPI_CTAR_FMSZ_MASK;
		m_spi->CTAR[0] |= SPI_CTAR_FMSZ( 15 );
	}
	else if ( bits_to_transfer == 8U )
	{
		m_spi->CTAR[0] &= ~SPI_CTAR_FMSZ_MASK;
		m_spi->CTAR[0] |= SPI_CTAR_FMSZ( 7 );
	}
	else
	{
		BF_ASSERT( false );
	...
	// baud rate = (f_sys/PBR) * ((1+DBR)/BR)
	// baud rate = 120M/PBR * 1/BR
	// baud rate = 120M/2 * 1/BR
	// baud rate = 60M/BR
	// BR = 60M / baud rate
	// f_sys = 120000000
	// DBR = 0, 1
	// PBR = baud_prescalers[x]
	// BR  = baud_divisors[y]
	//
	// start with DBR = 1

	spi_clock_freq = APB2_PERIPH_CLOCK_FREQ;
	...
	// test with the double scaling, PBR can only = [0] = 2
	while ( BR < 16 )
	{
		clock_calc = ( ( spi_clock_freq / baud_prescalers[PBR] ) * ( 1 + DBR ) ) / baud_divisors[BR];
		if ( clock_calc <= desired_spi_clock_freq )
		{
			diff = desired_spi_clock_freq - clock_calc;
			break;
		}
		BR++;
	}

	// if a baud was found under or equal to the desired, keep the masks
	if ( ( BR < 16 ) && ( diff != 0xFFFFFFFF ) )
	{
		baud_masks = SPI_CTAR_PBR( PBR ) | SPI_CTAR_BR( BR ) | SPI_CTAR_DBR_MASK;
		baud_masks |= SPI_CTAR_PCSSCK( PBR ) | SPI_CTAR_CSSCK( BR ) | SPI_CTAR_DT( BR ) | SPI_CTAR_ASC( BR );	// delay
																												// scalers
	}

	// try again without double scaling
	DBR = 0;
	PBR = 0;
	BR = 0;
	done = 0;
	while ( PBR < 4 )
	{
		while ( BR < 16 )
		{
			...
			if ( clock_calc <= desired_spi_clock_freq )
			{
				// if we find a baud under or equal to the desired, check
				// against the previous find. If the new find is the same
				// or closer, use the new. Else use the old.
				if ( ( desired_spi_clock_freq - clock_calc ) <= diff )
				{
					diff = desired_spi_clock_freq - clock_calc;
					baud_masks = SPI_CTAR_PBR( PBR ) | SPI_CTAR_BR( BR );
					baud_masks |= SPI_CTAR_PCSSCK( PBR ) | SPI_CTAR_CSSCK( BR ) | SPI_CTAR_DT( BR ) | SPI_CTAR_ASC(
						BR );																				// delay
																											// scalers
				}
				done = 1;
	...
	// if nothing was found, diff will not have changed
	BF_ASSERT( diff != 0xFFFFFFFF );

	// apply the calculated masks
	m_spi->CTAR[0] |= baud_masks;

	/*		spi_clock_freq = (uC_Base::Self())->Get_PClock_Freq( m_config->periph_def.pclock_reg_index );
	        ...
	        if ( clock_div_counter > TOTAL_SPI_CLOCK_PRESC )
	        {
	            //clock_div_counter = TOTAL_SPI_CLOCK_PRESC;
	            BF_ASSERT(false); //  We probably should not use a clock speed faster than the passed
	               in value.
	        }

	        clock_div_counter--;	//Due to the fact that 0 = a div by 2.
	        m_chip_sel_cfg[chip_select].cr1 |= static_cast<uint16_t>( SPI_CR1_BR & ( clock_div_counter<<3U ) );
	 */
	uC_IO_Config::Set_Pin_To_Output( m_chip_sel_ctrls[chip_select], static_cast<bool>( uC_IO_OUT_INIT_HIGH ) );
...
{
	// K6x specific 1us delay prior to start of transaction
	uC_Delay( 1U );
	...
	/*	temp_cr1 = static_cast<uint16_t>(m_spi->CR1 & static_cast<uint32_t>(SPI_CR1_SPE));	//Push the SPI enable
	   register so we can choose to properly enable it.
...
{
	// K6x specific 1us delay prior to close of transaction
	uC_Delay( 1U );
...
{
	m_tx_dma->Disable();
	m_tx_dma->Set_Buff( data, &m_spi->PUSHR, length );
}
...
{
	m_rx_dma->Disable();
	m_rx_dma->Set_Buff( data, &m_spi->POPR, length );
}
...
void uC_SPI::Enable_XChange_DMA( void )
{
	...
	Dump();
	m_spi->RSER |= ( SPI_RSER_RFDF_DIRS_MASK | SPI_RSER_TFFF_DIRS_MASK );
	m_rx_dma->Enable();
	m_tx_dma->Enable();
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	Activate_SPI_IO( chip_select, ( ( m_spi->SPI_MR & AT91C_SPI_MSTR ) != 0 ), manual_chip_select );
