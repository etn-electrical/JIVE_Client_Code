...
class uC_IO_Config
{
	public:
		...
		{ return ( Test_Bit( pio_ctrl->gio_ctrl->PDIR, pio_ctrl->pio_num ) ); }
		...
		{ pio_ctrl->gio_ctrl->PSOR = static_cast<uint32_t>( 1 << pio_ctrl->pio_num ); }
		...
		{ pio_ctrl->gio_ctrl->PCOR = static_cast<uint32_t>( 1U << pio_ctrl->pio_num ); }
		...
		{ return ( Test_Bit( pio_ctrl->gio_ctrl->PDOR, pio_ctrl->pio_num ) ); }
		...
		static void Set_Pin_Interrupt_State( uC_USER_IO_STRUCT const* pio_ctrl, UINT32 int_state );
		 ...
		 *****************************************************************************************
		 */
		static uint32_t Get_Input_Bits( GPIO_Type* pio_reg )
		{ return ( pio_reg->PDIR ); }

	private:
		static void Set_Pin_Remap( uC_PERIPH_IO_STRUCT const* pio_ctrl );

		static void Set_Pin_To_GPIO_MUX( uC_USER_IO_STRUCT const* pio_ctrl );
