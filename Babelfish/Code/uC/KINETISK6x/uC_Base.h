...
/*
 ********************************
 *  LP Timer Block Definition Structure
 */
typedef struct uC_BASE_LP_TIMER_CTRL_STRUCT
{
	LPTMR_Type* reg_ctrl;
	PERIPH_DEF_ST periph_def;
	uint8_t clock_sel;
...
typedef struct uC_BASE_USART_IO_PORT_STRUCT
{
	...
	uC_PERIPH_IO_STRUCT const* rts_port;
...
typedef struct uC_BASE_SPI_IO_PORT_STRUCT
{
	...
	const uC_PERIPH_IO_STRUCT* cs_pio;
...
{
	DMA_Type* dma_ctrl;
	DMAMUX_Type* dmamux_ctrl;
	PERIPH_DEF_ST periph_def[2];
...
class uC_Base
{
	...
	public:
		...
		virtual uC_BASE_LP_TIMER_CTRL_STRUCT const* Get_LP_Timer_Ctrl( UINT32 timer_block ) = 0;
