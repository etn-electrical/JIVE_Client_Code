...
class uC_USART_HW
{
	public:
		...
		inline void Enable_RX_Ready_Int( void )     {  m_usart->C2 |= UART_C2_RIE_MASK; }

		inline void Disable_RX_Ready_Int( void )    {  m_usart->C2 &= ~UART_C2_RIE_MASK; }

		inline bool RX_Ready_Int_Enabled( void )            { return ( ( m_usart->C2 & UART_C2_RIE_MASK ) != 0 ); }

		inline bool RX_Ready_Int_Status( uint32_t status )  { return ( ( status & UART_S1_RDRF_MASK ) != 0 ); }

		inline void Enable_TX_Empty_Int( void )     { m_usart->C2 |= UART_C2_TIE_MASK; }

		inline void Disable_TX_Empty_Int( void )    { m_usart->C2 &= ~UART_C2_TIE_MASK; }

		inline bool TX_Empty_Int_Enabled( void )                { return ( ( m_usart->C2 & UART_C2_TIE_MASK ) != 0 ); }

		inline bool TX_Empty_Int_Status( uint32_t status )      { return ( ( status & UART_S1_TDRE_MASK ) != 0 ); }

		inline bool TX_Empty_Status( void )                     { return ( ( m_usart->S1 & UART_S1_TC_MASK ) != 0 ); }

		inline bool TX_Data_Empty_Status( void )                { return ( ( m_usart->S1 & UART_S1_TDRE_MASK ) != 0 ); }

		inline void Enable_Parity_Error_Int( void )     {  m_usart->C3 |= UART_C3_PEIE_MASK; }

		inline void Disable_Parity_Error_Int( void )    {  m_usart->C3 &= ~UART_C3_PEIE_MASK; }

		inline bool Parity_Error_Int_Enabled( void )            { return ( ( m_usart->C3 & UART_C3_PEIE_MASK ) != 0 ); }

		inline bool Parity_Error_Int_Status( uint32_t status )  { return ( ( status & UART_S1_PF_MASK ) != 0 ); }

		inline void Clear_Int_Status( void )                { m_usart->S1 = 0; }
		...
		{
			m_usart->C2 &= ~( UART_C2_TIE_MASK | UART_C2_TCIE_MASK | UART_C2_RIE_MASK | UART_C2_ILIE_MASK );
			m_usart->C3 &= ~UART_C3_PEIE_MASK;
		}

		inline uint32_t Get_Int_Status_Bits( void )           { return ( m_usart->S1 ); }

		inline uint8_t Get_RX_Data( void )         { return ( m_usart->D & m_parity_bit_mask ); }

		inline void Set_TX_Data( uint32_t data )       { m_usart->D = data; }
