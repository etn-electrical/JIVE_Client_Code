...
class uC_USART_Buff
{
	public:
		...
		static const cback_status_t RX_BYTE_TIMEOUT = 0U;		// < This is triggered by a bus quiet
																// time.
		static const cback_status_t RX_BUFF_FULL = 1U;			// < The passed in array has hit the
																// end of the buffer.
		static const cback_status_t TX_BUFF_EMPTY = 2U;			// < The data buffer is empty and
																// ready to transmit again.
		static const cback_status_t TX_COMPLETE = 3U;			// < The shift register is empty and
																// the wire should be clear.
		static const cback_status_t TXRX_HOLDOFF_COMPLETE = 4U;	// < Half duplex holdoff.  RX resets
																// holdoff.  This will override
																// TX_HOLDOFF_COMPLETE.
		static const cback_status_t TX_HOLDOFF_COMPLETE = 5U;		// < We only monitor the TX for
																	// holdoff.
		static const cback_status_t RX_BYTE_COMPLETE = 6U;		// < To detect end of frame.
		 ...
		 * @param rx_byte_time (microseconds) - Wire quiet time before we consider it End of
		 * Message.  This can also be 0
		 * which indicates no timeout.
		 * @param tx_holdoff_time (microseconds) - Wire quiet time before we consider it End of
		 * Message.  This can also be 0
		 * which indicates no timeout.
		 */
