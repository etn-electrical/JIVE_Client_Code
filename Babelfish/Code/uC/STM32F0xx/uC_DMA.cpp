...
{
	uint8_t active_dma_channel_check = 0;

#if defined ( uC_STM32F030CC_USAGE ) || defined ( uC_STM32F030RC_USAGE )
	m_total_item_count = dma_channel & 0x0F;
	dma_channel = ( ( dma_channel & 0xF0 ) >> 4 );
#endif
	m_dma = ( uC_Base::Self() )->Get_DMA_Ctrl( dma_channel );

	Set_Bit32( active_dma_channel_check, dma_channel );
	BF_ASSERT( ( m_dma != NULL ) && ( !( m_active_dma & active_dma_channel_check ) ) );
...
#if defined ( uC_STM32F030CC_USAGE ) || defined ( uC_STM32F030RC_USAGE )
	m_dma->dma_ctrl->CSELR |= ( ( uint32_t ) m_total_item_count << ( dma_channel * 4 ) );
#endif
...
void uC_DMA::Set_Buff( uint8_t* mem_ptr, volatile void* periph_ptr, uint16_t length )
{
	...
	m_dma->dma_channel_ctrl->CCR |= DMA_CCR_PL_1;
...
{
	m_dma->dma_ctrl->IFCR = static_cast<uint32_t>( 1U )
		<< ( static_cast<uint32_t>( m_dma->dma_num ) << MULT_BY_4 );	// Clear the appropriate
																		// flag.
...
{
	m_dma->dma_channel_ctrl->CCR &= ( ~DMA_CCR_EN );
	m_dma->dma_ctrl->IFCR = static_cast<uint32_t>( uC_DMA_ALL_STATUS_FLAGS_MASK )
...
bool uC_DMA::Busy( void ) const
{
	return ( ( m_dma->dma_ctrl->ISR
			   & ( static_cast<uint32_t>( DMA_ISR_TCIF1 )
