 ...
 *****************************************************************************************
 */
#define uC_FLASH_TEST_START_POINT           0x08020000	// Roughly at the midpoint of the main
														// memory.
...
bool uC_TEST_uC_Flash_Init( void )
{
...
#if 0
	return_status = flash_ctrl->Erase( flash_cfg_ptr->start_address, flash_cfg_ptr->page_size * 2,
									   0x55, false );
	return_status = flash_ctrl->Erase( flash_cfg_ptr->mirror_start_address,
									   flash_cfg_ptr->page_size * 2, 0x55, false );
	...
	return_status = flash_ctrl->Write( test_flash_data,
									   ...
									   4, true );
	return_status = flash_ctrl->Write( test_flash_data,
									   flash_cfg_ptr->start_address + flash_cfg_ptr->page_size - 4,
									   4, true );
	return_status = flash_ctrl->Write( test_flash_data,
									   flash_cfg_ptr->start_address + flash_cfg_ptr->page_size - 5,
									   4, true );
	...
	return_status = flash_ctrl->Erase(
		flash_cfg_ptr->start_address + ( flash_cfg_ptr->page_size / 2 ) +
		( flash_cfg_ptr->page_size - 2 ), 4, 0x55, false );

	return_status =
		flash_ctrl->Write( test_flash_data, flash_cfg_ptr->start_address + 4, 4, false );
	return_status = flash_ctrl->Write( test_flash_data,
									   flash_cfg_ptr->start_address + flash_cfg_ptr->page_size - 2,
									   4, false );
	return_status = flash_ctrl->Write( test_flash_data,
	...
	return_status = flash_ctrl->Erase(
		flash_cfg_ptr->start_address + ( flash_cfg_ptr->page_size / 2 ) +
		( flash_cfg_ptr->page_size - 2 ), 4, 0x55, true );
#endif
...
bool uC_TEST_AT25DF_Init( void )
{
	...
	return_status =
		at25_ctrl->Write( device_id,
						  ( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 2 ), 4,
						  true );
	return_status =
		at25_ctrl->Write( device_id,
						  ( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 4 ), 4,
						  true );
	return_status =
		at25_ctrl->Write( device_id,
						  ( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 5 ), 4,
						  true );
	...
	return_status =
		at25_ctrl->Erase(
			( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 2 ),
			4, 0x55, false );
	return_status =
		at25_ctrl->Erase(
			( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 4 ),
			4, 0x55, false );
	return_status =
		at25_ctrl->Erase(
			( chip_config->erase_page_size / 2 ) + ( chip_config->write_page_size - 5 ),
			4, 0x55, false );
