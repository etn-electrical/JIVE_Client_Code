...
OS_Binary_Semaphore* Stm32_Eth_Driver::ETHRxBinSemaphore =
	...
	EMAC_Driver()
{
	...
	uC_PERIPH_IO_STRUCT ETH_MCO_CLOCK_OUT_PIO =
	{ GPIOA, static_cast<uint8_t>( IO_PIN_8 ),
	  static_cast<uint8_t>( uC_IO_ALTERNATE_MAP_UNSUPPORTED ), 0 };

	BF_ASSERT( m_emac_pio != 0 );
	...
	if ( emac_interface == uC_EMAC_MII_MODE )
	{
		...
		{
			ETH_MCO_CLOCK_OUT_PIO.remap_num = static_cast<uint8_t>( uC_IO_ALTERNATE_MAP_0 );
			uC_IO_Config::Enable_Periph_Output_Pin( &ETH_MCO_CLOCK_OUT_PIO );
		}
		else
		{
			// uC_IO_Config::Enable_Periph_Output_Pin( &ETH_MCO_CLOCK_OUT_PIO );
		...
		AFIO->MAPR &= ~m_emac_pio->rmii_sel_mask;
		for ( uint8_t i = 0U; i < m_emac_pio->mii_pio_out_count; i++ )
		{
			uC_IO_Config::Enable_Periph_Output_Pin( m_emac_pio->mii_pio_out[i] );
		}
		for ( uint8_t i = 0U; i < m_emac_pio->mii_pio_in_count; i++ )
		{
			uC_IO_Config::Enable_Periph_Input_Pin( m_emac_pio->mii_pio_in[i] );
		}
	}
	else
	{
		...
		{
			uC_IO_Config::Enable_Periph_Output_Pin( m_emac_pio->rmii_pio_out[i] );
		}
		for ( uint8_t i = 0U; i < m_emac_pio->rmii_pio_in_count; i++ )
		{
			uC_IO_Config::Enable_Periph_Input_Pin( m_emac_pio->rmii_pio_in[i] );
		}
	}
	...
	{
		/*
		   Although new hal layer files are integrated from ST, the pll3 configuration
		   is missing from the new rcc files so we have imported patch from old style
		   rcc hal layer files and called from here.
		 */
...
void Stm32_Eth_Driver::ETH_Init( void ) const
{
	...
	ETH->MACFFR = static_cast<uint32_t>( m_ETH_InitStructure
										 .ETH_ReceiveAll |
										 m_ETH_InitStructure.ETH_SourceAddrFilter |
										 m_ETH_InitStructure.ETH_PassControlFrames |
										 m_ETH_InitStructure.ETH_BroadcastFramesReception |
										 m_ETH_InitStructure.ETH_DestinationAddrFilter |
										 m_ETH_InitStructure.ETH_PromiscuousMode |
										 m_ETH_InitStructure.ETH_MulticastFramesFilter |
										 m_ETH_InitStructure.ETH_UnicastFramesFilter );
	/* --------------- ETHERNET MACHTHR and MACHTLR Configuration ---------------
	   Write to ETHERNET MACHTHR*/
	ETH->MACHTHR = static_cast<uint32_t>( m_ETH_InitStructure.ETH_HashTableHigh );
	/* Write to ETHERNET MACHTLR */
	ETH->MACHTLR = static_cast<uint32_t>( m_ETH_InitStructure.ETH_HashTableLow );
	...
	ETH->MACFCR = static_cast<uint32_t>( tmpreg );
	/* ----------------------- ETHERNET MACVLANTR Configuration -----------------
	   Set the ETV bit according to ETH_VLANTagComparison value
	   Set the VL bit according to ETH_VLANTagIdentifier value*/
	ETH->MACVLANTR = static_cast<uint32_t>( m_ETH_InitStructure.ETH_VLANTagComparison |
	...
	ETH->DMABMR = static_cast<uint32_t>( m_ETH_InitStructure.ETH_AddressAlignedBeats |
										 ...
										 m_ETH_InitStructure.ETH_RxDMABurstLength |	/* !! if 4xPBL
																					   is selected
																					   for Tx or Rx
																					   it is applied
																					   for the other
										 ...
										 ( m_ETH_InitStructure.ETH_DescriptorSkipLength << 2U ) |
										 m_ETH_InitStructure.ETH_DMAArbitration |
										 ETH_DMABMR_USP );	/* Enable use of separate PBL for Rx and
															   Tx */
...
void Stm32_Eth_Driver::ETH_MACAddressConfig( uint32_t MacAddr, uint8_t* Addr )
{
	...
	tmpreg = ( static_cast<uint32_t>( ( ( ( ( static_cast<uint32_t>( Addr[5U] ) ) << 8U ) ) )
									  | ( static_cast<uint32_t>( Addr[4U] ) ) ) );
	/* Load the selectecd MAC address high register */
	( *( reinterpret_cast<__IO uint32_t*>( ETH_MAC_ADDR_HBASE + MacAddr ) ) ) = tmpreg;
	/* Calculate the selectecd MAC address low register */
	tmpreg = ( static_cast<uint32_t>( ( ( ( ( static_cast<uint32_t>( Addr[3U] ) ) << 24U ) ) )
									  | ( ( ( ( static_cast<uint32_t>( Addr[2U] ) ) << 16U ) ) )
									  | ( ( ( ( static_cast<uint32_t>( Addr[1U] ) ) << 8U ) ) )
...
void Stm32_Eth_Driver::Read_MAC_Address_Reg( uint32_t MacAddrOffSet, uint8_t* Addr ) const
{
	...
	Addr[5U] = static_cast<uint8_t>( ( ( tmpreg >> 8U ) & ( static_cast<uint8_t>( 0xFFU ) ) ) );
	...
	/* Calculate the selectecd MAC address buffer */
	Addr[3U] = static_cast<uint8_t>( ( tmpreg >> 24U ) & ( static_cast<uint8_t>( 0xFFU ) ) );
	Addr[2U] = static_cast<uint8_t>( ( tmpreg >> 16U ) & ( static_cast<uint8_t>( 0xFFU ) ) );
										...
										uint16_t* read_ptr )
{
	...
	if ( success )
	{
		*read_ptr = static_cast<uint16_t>( ETH->MACMIIDR );	// *read_ptr = (uint16_t) ( ETH->MACMIIDR
															// );
	}
...
data_states_t Stm32_Eth_Driver::Get_Next_Rx_Pkt( uint32_t* new_payload, uint8_t* pkt_index )
{
	...
	error = ( ( ( static_cast<bool_t>( ( DMARxDescToGet->Status & ETH_DMARxDesc_ES ) ) ) != 0U ) ||
...
void Stm32_Eth_Driver::ETH_Handler( void )
{
	...
	END_SWITCHING_ISR( xHigherPriorityTaskWoken );
}
