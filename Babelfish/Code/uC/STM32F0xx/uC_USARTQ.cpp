...
uC_USARTQ::uC_USARTQ( uC_USART* usart_handle, uint16_t tx_buff_size, uint16_t rx_buff_size )
{
	...
	m_tx_dma_ctrl->Set_Buff( m_tx_buff, ( uint8_t* )&m_usart->DR, m_tx_buff_size, true );
	m_rx_dma_ctrl->Set_Buff( m_rx_buff, ( uint8_t* )&m_usart->DR, m_rx_buff_size, true );
						  ...
						  uint32_t stop_bit_count )
{
	...
	{
		BF_ASSERT( char_size == uC_USART_CHAR_SIZE_8BIT );
...
void uC_USART::Set_TX_DMA( uint8_t* data, uint16_t length )
{
	...
	m_tx_dma_ctrl->Set_Buff( data, ( uint8_t* )&m_usart->DR, length, false );
...
void uC_USART::Set_Baud_Rate( uint32_t baud_rate, bool sample_count )
{
	...
	temp_baud = Round_Div( uC_USART_MASTER_CLOCK_FREQ, temp_baud );	// uC_USART_MASTER_CLOCK_FREQ /
																	// temp_baud;
