			...
			while ( ( m_status == NV_Ctrl::SUCCESS ) && ( length != 0 ) )
			{
				...
				else
				{
					...
					/* The following adjustment in "sector_num" is done because of discontinuity in sector number for
					   "INTERNAL_FLASH_PAGE_SIZES" in "uC_IO_Define_STM32F767.h" file. This discontinuity is because
...
bool uC_Flash::Erase_Page( uint32_t address )
{
	...
	if ( status == uC_FLASH_COMPLETE )
	{
		if ( ( address >= BANK1_START_ADDR ) && ( address <= BANK2_END_ADDR ) )
		{
			...
			/*
			    Added 2 DSB instructions here as per note from STM32F767 manual. Note: When setting the STRT bit in the
...
NV_Ctrl::nv_status_t uC_Flash::Erase_All( void )
{
	...
	BF_ASSERT( ( m_chip_cfg->start_address >= BANK1_START_ADDR ) &&
...
uint32_t uC_Flash::Get_Sector( uint32_t address ) const
{
	...
	BF_ASSERT( ( address >= BANK1_START_ADDR ) && ( address <= BANK2_END_ADDR ) );
							  ...
							  NV_CTRL_LENGTH_TD length ) const
{
	...
	read_time = Round_Div( read_time, MICROSECONDS_ADJUSTMENT );		// Because the byte
	return ( read_time );
}
	...
	if ( status == uC_FLASH_COMPLETE )
	{
		...
		else
		{
			...
			/* The following adjustment in "m_asynch_erase_sector_head" is done because of discontinuity in sector
			   number for
			 * "INTERNAL_FLASH_PAGE_SIZES" in "uC_IO_Define_STM32F767.h" file. This discontinuity is because values from
			 * 12 to 15 are not allowed in SNB field of FLASH_CR. For more information please refer TRM for the
			 * controller.
			 */
