 ...
 *****************************************************************************************
 */
STM32F7xx_SPI::STM32F7xx_SPI( uint8_t channel, uint8_t no_slaves, bool_t master,
							  uC_BASE_SPI_IO_PORT_STRUCT const* io_ctrl,
							  uC_USER_IO_STRUCT const* const pio[], uint8_t datatype_size ) :
...
{
	uint8_t int_priority = static_cast<uint8_t>( uC_INT_HW_PRIORITY_11 );
	...
	if ( m_config != nullptr )
	{
		dma_stream_id = static_cast<uint8_t>( Get_DMA_Stream_Num( m_config->rx_dma_channel ) );
		m_rx_dma = ( uC_Base::Self() )->Get_DMA_Ctrl( dma_stream_id );

		dma_stream_id = static_cast<uint8_t>( Get_DMA_Stream_Num( m_config->tx_dma_channel ) );
		m_tx_dma = ( uC_Base::Self() )->Get_DMA_Ctrl( dma_stream_id );
 ...
 *****************************************************************************************
 */
STM32F7xx_SPI::~STM32F7xx_SPI( void )
{
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::Start_Exchange( bool_t new_exchange )
{
	...
	else
	{
		...
		// Set dma channel select bits
		Set_Mask( rx_cr_value, DMA_SxCR_CHSEL,
				  ( DMA_SxCR_CHSEL_0 * Get_DMA_Channel_Num( m_config->rx_dma_channel ) ) );

		tx_cr_value = DMA_SxCR_PL_1 |
			DMA_SxCR_DIR_0;

		// Set dma channel select bits
		Set_Mask( tx_cr_value, DMA_SxCR_CHSEL,
				  ( DMA_SxCR_CHSEL_0 * Get_DMA_Channel_Num( m_config->tx_dma_channel ) ) );
		...
		switch ( m_datatype_size )
		{
			case DATA_SIZE_16_BIT:
				Set_Mask( rx_cr_value, ( DMA_SxCR_PSIZE | DMA_SxCR_MSIZE ),
						  ( DMA_SxCR_MSIZE_0 | DMA_SxCR_PSIZE_0 ) );
				Set_Mask( tx_cr_value, ( DMA_SxCR_PSIZE | DMA_SxCR_MSIZE ),
						  ( DMA_SxCR_MSIZE_0 | DMA_SxCR_PSIZE_0 ) );
				break;

			case DATA_SIZE_32_BIT:
				Set_Mask( rx_cr_value, ( DMA_SxCR_PSIZE | DMA_SxCR_MSIZE ),
			...
			case DATA_SIZE_8_BIT:
			default:
				Set_Mask( rx_cr_value, ( DMA_SxCR_PSIZE | DMA_SxCR_MSIZE ), 0U );
				Set_Mask( tx_cr_value, ( DMA_SxCR_PSIZE | DMA_SxCR_MSIZE ), 0U );
				break;
		}
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::DMA_ISR( void )
{
	bool_t failure;
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::SPI_ISR( void )
{
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::Assume_IO_Control( const uC_BASE_SPI_IO_PORT_STRUCT* spi_io,
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::Initialize_Peripheral_Clock( const PERIPH_DEF_ST* periph_def ) const
{
 ...
 *****************************************************************************************
 */
bool_t STM32F7xx_SPI::Clear_DMA_Interrupts()
{
	...
	return ( ( ( ( flag_settings & ( DMA_LISR_TEIF0 | DMA_LISR_DMEIF0 |
									 ...
									 DMA_LISR_TEIF3 | DMA_LISR_DMEIF3 ) ) != 0U ) ) ? true : false );
}
 ...
 *****************************************************************************************
 */
void STM32F7xx_SPI::SPI_Chip_Configuration( spi_mode mode, uint32_t chip_clock_freq,
