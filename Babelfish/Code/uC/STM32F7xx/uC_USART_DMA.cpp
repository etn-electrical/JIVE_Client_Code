...
void uC_USART_DMA::Start_RX( uint8_t* data, uint16_t length )
{
	...
	/* Clean D-cache
	   Make sure the address is 4-byte aligned
	   Not necessary to add 32 bytes to length for cacheline overlap  */
	uint32_t temp_data = static_cast<uint32_t>( *data ) & ~static_cast<uint32_t>( 0x03 );
	SCB_InvalidateDCache_by_Addr( &temp_data, length );
	#endif
...
void uC_USART_DMA::Start_TX( uint8_t* data, uint16_t length )
{
	...
	#if defined( DATA_CACHE_ENABLE )
	/* Clean D-cache
	   Make sure the address is 4-byte aligned and add 32-bytes to length, in case it overlaps cacheline*/
	uint32_t temp_data = static_cast<uint32_t>( *data ) & ~static_cast<uint32_t>( 0x03 );
	SCB_CleanDCache_by_Addr( &temp_data, length + 32 );
	#endif
