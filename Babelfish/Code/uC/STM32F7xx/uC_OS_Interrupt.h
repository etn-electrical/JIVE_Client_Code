...
#define uC_OS_INT_PRIORITY_HIGHEST      uC_OS_INT_PRIORITY_0
	...
	#error The max OS Interrupt priority (lowest priority) is beyond the reach of the priority descriptor.
#endif
...
class uC_OS_Interrupt
{
	public:
		...
		static bool Set_OS_Vect( uC_OS_INT_CALLBACK_FUNC* int_callback,
								 IRQn_Type irq_n, BF_Lib::MBASE priority, BF_Lib::MBASE trigger_type = 0 );

		static void Set_SVC_Handler_Vector( void );

		static void Enable_Int( IRQn_Type vector_number )       { uC_Interrupt::Enable_Int( vector_number ); }

		static void Disable_Int( IRQn_Type vector_number )      { uC_Interrupt::Disable_Int( vector_number ); }

		static bool Is_Enabled( IRQn_Type vector_number )       { return ( uC_Interrupt::Is_Enabled( vector_number ) );
		}

		static void Clear_Int( IRQn_Type vector_number )        { uC_Interrupt::Clear_Int( vector_number ); }

		static void Set_Int( IRQn_Type vector_number )          { uC_Interrupt::Set_Int( vector_number ); }
