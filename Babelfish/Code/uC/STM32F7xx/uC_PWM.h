/*! \example  PWM An example code for PWM software module.
   \section  uC_PWM
   The sample code to initialize and control PWM channels  STM32F407.
   \code
   ...
   void Example_Pwm( void )
   {
    ...
    m_eg_pwm = new uC_PWM( uC_BASE_PWM_CTRL_5, uC_BASE_PWM_CHANNEL_1, &TIMER5_PIO1_PA0 );

    // configure active High, left aligned 100 Hz frequency
    m_eg_pwm->Configure( uC_PWM_DUTY_CYCLE_HIGH, uC_PWM_LEFT_ALIGNED, 100 );

    // Set PWM duty cycle to 25%
    m_eg_pwm->Set_Duty_Cycle_Pct( 0x4000 );

    // turn Off PWM Mode
    m_eg_pwm->Turn_Off();

    // read PWM status
    pwm_status = m_eg_pwm->Is_On();
 ...
 *                  over PWM channels on STM32F40x.
 * @details
...
class uC_PWM
{
	public:
		 ...
		 * @n                                 STM32F407 features 4 PWM channels with each PWM control block.
		 * @param[in] port_io                 Port IO associated with PWM channel
		 * @return
		 */
		uC_PWM( uint8_t pwm_ctrl_block, uint8_t pwm_channel,
				uC_PERIPH_IO_STRUCT const* port_io = nullptr );
