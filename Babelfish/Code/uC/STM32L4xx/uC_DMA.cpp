	...
	m_mem_inc( false )
{
	...
	uint32_t request_fun;

	dma_channel_id = static_cast<uint8_t>( Get_DMA_Channel_Num( dma_channel ) );
	m_dma = ( uC_Base::Self() )->Get_DMA_Ctrl( dma_channel_id );

	BF_ASSERT( ( m_dma != NULL ) && ( !Test_Bit( m_active_dma, dma_channel_id ) ) );
	Set_Bit32( m_active_dma, dma_channel_id );
	...
	request_fun = Get_DMA_Request_Num( dma_channel );
	Set_Mask( m_dma->dma_request_ctrl->CSELR, ( 0xFU << ( ( dma_channel_id % 7 ) * 4 ) ),
			  ( request_fun << ( ( dma_channel_id % 7 ) * 4 ) ) );
...
void uC_DMA::Deallocate_DMA_Stream( uint8_t dma_channel )
{
	...
	dma_channel_id = static_cast<uint8_t>( Get_DMA_Channel_Num( dma_channel ) );
	Clr_Bit( m_active_dma, dma_channel_id );
}
