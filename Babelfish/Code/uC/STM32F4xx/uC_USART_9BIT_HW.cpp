 ...
 *****************************************************************************************
 */
uC_USART_9BIT_HW::uC_USART_9BIT_HW( uint8_t port_requested,
									uC_BASE_USART_IO_PORT_STRUCT const* io_port, bool_t enable_dma ) :
	...
	m_int_vect_cback( reinterpret_cast<Advanced_Vect*>( nullptr ) )
{
		 ...
		 ( enable_dma == true ) )
	{
		m_tx_dma_ctrl = new uC_DMA( m_io_ctrl->tx_dma_channel, uC_DMA_DIR_FROM_MEMORY,
									static_cast<uint8_t>( sizeof( uint16_t ) ), uC_DMA_MEM_INC_TRUE,
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Enable_RX( void )
{
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Disable_RX( void )
{
	uint32_t temp_mask;
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Enable_TX( void )
{
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Disable_TX( void )
{
	uint32_t temp_mask;
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Configure( uint32_t baud_rate, char_size_t char_size,
								  parity_t parity, stop_bit_t stop_bit_count )
{
	m_parity_bit_mask = 0x1FFU;	/// A data read will get the data including the parity bit in the response.  We need to
								/// mask it off.

	// if parity is enabled we can only use 7 or 8bit character sizes because the 9th bit is for parity.
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Set_Baud_Rate( uint32_t baud_rate )
{
 ...
 *****************************************************************************************
 */
void uC_USART_9BIT_HW::Set_Int_Vect( cback_func_t int_handler,
									 cback_param_t param, uint8_t priority )
{
	...
	uC_Interrupt::Set_Vector( m_int_vect_cback->Get_Int_Func(),
