	...
	m_int_vect_cback( reinterpret_cast<Advanced_Vect*>( nullptr ) )
{
	...
	if ( ( m_io_ctrl->tx_dma_channel != static_cast<uint8_t>( uC_DMA_STREAM_DNE ) ) &&
		 ( m_io_ctrl->rx_dma_channel != static_cast<uint8_t>( uC_DMA_STREAM_DNE ) ) &&
		 ( enable_dma == true ) )
	{
		m_tx_dma_ctrl = new uC_DMA( m_io_ctrl->tx_dma_channel, uC_DMA_DIR_FROM_MEMORY,
									static_cast<uint8_t>( sizeof( uint8_t ) ), uC_DMA_MEM_INC_TRUE,
...
void uC_USART::Configure( uint32_t char_size, uint32_t parity, uint32_t stop_bit_count )
{
	...
	{
		// configure for 8-bit no-parity 1-stopbit - which is equivalent to 7-bit no-parity 2-stopbits
		m_usart->CR1 &= ~( USART_CR1_PCE | USART_CR1_M );
							 ...
							 cback_param_t param, uint8_t priority ) const
{
	...
	uC_Interrupt::Set_Vector( reinterpret_cast<INT_CALLBACK_FUNC*>( m_int_vect_cback->Get_Int_Func() ),
...
{
	m_int_vect_cback->Set_Vect( int_handler, param );
	uC_OS_Interrupt::Set_OS_Vect( reinterpret_cast<INT_CALLBACK_FUNC*>( m_int_vect_cback->Get_Int_Func() ),
								  m_io_ctrl->irq_num, priority );
}
