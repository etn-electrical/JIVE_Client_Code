...
#include "uC_Test_USART.h"
#include "uC_Test_Flash.h"
...
const uC_FLASH_CHIP_CONFIG_TD ST1_FLASH_CFG =
{
	...
	( uC_FLASH_TEST_START_POINT + uC_FLASH_TEST_PAGE_SIZE ),			// mirror start address
	( uC_FLASH_TEST_START_POINT + uC_FLASH_TEST_LENGTH )				// end of memory
};

uC_FLASH_CHIP_CONFIG_TD const
* g_flash_cfg_ptr = &ST1_FLASH_CFG;

uC_Flash
* g_flash_ctrl;

uint8_t
	g_test_flash_data[uC_FLASH_TEST_STRING_LEN];
...
{
	for ( uint32_t i = 0; i < uC_FLASH_TEST_STRING_LEN; i++ )
	{
		// g_test_flash_data[ i ] = ( i + 1 );
		g_test_flash_data[i] = 50;
	}

	g_flash_ctrl = new uC_Flash( &ST1_FLASH_CFG, FLASH );
}
...
{
	volatile NV_Ctrl::nv_status_t
		r_val;

	r_val = g_flash_ctrl->Erase( 0, g_flash_cfg_ptr->end_address - g_flash_cfg_ptr->start_address, 0x0, false );
...
bool uC_TEST_Flash( void )
{
	...
	for ( uint32_t i = 0; i < uC_FLASH_TEST_STRING_LEN; i++ )
	{
		g_test_flash_data[i] = 40;
	}

	// 2
	r_val = g_flash_ctrl->Write( g_test_flash_data, g_flash_cfg_ptr->mirror_start_address - 1, 2, false );
 ...
 *****************************************************************************************
   ...
   //	while(1){};
    ...
    return_status = at25_ctrl->Write( device_id, ( chip_config->erase_page_size/2 ) + ( chip_config->write_page_size - 2
     *), 4, true );
    return_status = at25_ctrl->Write( device_id, ( chip_config->erase_page_size/2 ) + ( chip_config->write_page_size - 4
     *), 4, true );
    return_status = at25_ctrl->Write( device_id, ( chip_config->erase_page_size/2 ) + ( chip_config->write_page_size - 5
     *), 4, true );
