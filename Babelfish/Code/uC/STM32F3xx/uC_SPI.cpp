	...
	m_tx_dma( reinterpret_cast<uC_DMA*>( nullptr ) )
{
	...
	{	/// This code needs to be flushed out and figured out.
		m_chip_sel_cfg = reinterpret_cast<CHIP_SELECT_CFG_STTD*>( nullptr );
		m_total_chip_sel = 0U;
		m_chip_sel_ctrls = reinterpret_cast<uC_USER_IO_STRUCT const* const*>( nullptr );
		BF_ASSERT( false );
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	m_chip_sel_cfg[chip_select].cr2 &= ( ~static_cast<uint16_t>( SPI_CR2_DS ) );
	switch ( bits_to_transfer )
	{
		case 8:
			m_chip_sel_cfg[chip_select].cr2 |= ( SPI_CR2_DS_0 | SPI_CR2_DS_1 | SPI_CR2_DS_2 );
		...
		default:
			BF_ASSERT( false );			// We are not setup for this yet.
			break;
	}
 ...
 *****************************************************************************************
 */
void uC_SPI::Set_TX_DMA_Interrupt()
{
 ...
 *****************************************************************************************
 */
void uC_SPI::Set_RX_DMA_Interrupt()
{
 ...
 *****************************************************************************************
 */
void uC_SPI::Enable_XChange_DMA_Interrupt( void )
{
	Set_RX_DMA_Interrupt();
	Set_TX_DMA_Interrupt();
}
 ...
 *****************************************************************************************
 */
void uC_SPI::Set_Rx_DMA_Callback( cback_func_t func_callback,
								  cback_param_t func_param, uint8_t int_priority )
{
	m_rx_dma->Attach_Callback( func_callback, func_param, int_priority );
}
 ...
 *****************************************************************************************
 */
void uC_SPI::Set_Tx_DMA_Callback( cback_func_t func_callback,
								  cback_param_t func_param, uint8_t int_priority )
{
	m_tx_dma->Attach_Callback( func_callback, func_param, int_priority );
}
