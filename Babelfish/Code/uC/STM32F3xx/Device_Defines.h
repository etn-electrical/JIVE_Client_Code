...
#define Mem_Aligned_Size( mem_size )    \
	( ( static_cast<UINT32>( mem_size ) + ( PROCESSOR_REGISTER_BASE_SIZE - 1U ) ) & \
	  ( ~static_cast<UINT32>( PROCESSOR_REGISTER_BASE_SIZE - 1U ) ) )
...
/*
 *****************************************************************************************
 *	A very basic AND ROUGH microsecond delay.
 ...
 *	range.  This applies to build configurations that have no compiler optimization
 *	as well as the build configurations that have high compiler optimization.
 *	C9903612
 *****************************************************************************************
 */
#define uC_Delay( microseconds )            \
	for ( volatile uint_fast16_t ijklmno =      \
			  ( static_cast<UINT32>( microseconds ) * ( ( MASTER_CLOCK_FREQUENCY / 1000000U ) / 10U ) ); \
		  ijklmno > 0U; ijklmno-- ){}
