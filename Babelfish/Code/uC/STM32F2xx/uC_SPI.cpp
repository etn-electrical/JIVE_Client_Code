	...
	m_tx_dma( reinterpret_cast<uC_DMA*>( nullptr ) )
{
	...
	if ( master == true )
	{
		m_base_cfg.cr1 = static_cast<uint16_t>( SPI_CR1_MSTR );
		m_base_cfg.cr2 = static_cast<uint16_t>( SPI_CR2_SSOE );
	...
	m_base_cfg.cr1 |= static_cast<uint16_t>( SPI_CR1_SPE );
		 ...
		 ( enable_dma == true ) )
	{
		...
		m_base_cfg.cr2 |= static_cast<uint16_t>( SPI_CR2_TXDMAEN | SPI_CR2_RXDMAEN );
	}
									...
									uint8_t bits_to_transfer, bool manual_chip_select )
{
	...
	switch ( spi_mode )
	{
		...
		case uC_SPI_MODE_3:
			m_chip_sel_cfg[chip_select].cr1 |= static_cast<uint16_t>
	...
	{
		m_chip_sel_cfg[chip_select].cr1 |= static_cast<uint16_t>( SPI_CR1_DFF );
	}
	...
	m_chip_sel_cfg[chip_select].cr1 |= static_cast<uint16_t>
		( SPI_CR1_BR & ( clock_div_counter << 3U ) );
...
void uC_SPI::Select_Chip( uint8_t chip_select )
{
	...
	temp_cr1 = static_cast<uint16_t>( m_spi->CR1 & static_cast<uint32_t>( SPI_CR1_SPE ) );			// Push
																									// the
																									// SPI
																									// enable
																									// register
...
void uC_SPI::Enable_RX_DMA( void )
{
	Push_TGINT();
	m_rx_dma->Enable();
	m_base_cfg.cr2 |= static_cast<uint16_t>( SPI_CR2_RXDMAEN );
	m_spi->CR2 = m_base_cfg.cr2;
	Pop_TGINT();
}
...
void uC_SPI::Enable_TX_DMA( void )
{
	Push_TGINT();
	m_tx_dma->Enable();
	m_base_cfg.cr2 |= static_cast<uint16_t>( SPI_CR2_TXDMAEN );
	m_spi->CR2 = m_base_cfg.cr2;
	Pop_TGINT();
}
...
void uC_SPI::Disable_XChange_DMA( void )
{
	...
	m_rx_dma->Disable();
	m_tx_dma->Disable();
	m_base_cfg.cr2 &= ~( static_cast<uint16_t>( SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN ) );
	m_spi->CR2 = m_base_cfg.cr2;
