...
{
	return ( ( m_pwm_ctrl->reg_ctrl->CCER & static_cast<uint16_t>
			   ( static_cast<uint32_t>
				 ( TIM_CCER_CC1E ) << ( static_cast<uint32_t>( m_pwm_ch ) << MULT_BY_8 ) ) ) !=
			 0U );
}
...
void uC_PWM::Set_Duty_Cycle_Pct( uint16_t percent )
{
	...
	ticks_required = static_cast<uint16_t>
		( ( percent * m_percent_adj ) >> PWM_DUTY_CYCLE_PERCENT_SHIFT );								// PWM_DUTY_CYCLE_PERCENT_CALC;
