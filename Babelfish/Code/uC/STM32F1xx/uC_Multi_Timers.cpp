...
void uC_Multi_Timers::Start_Timeout( uint8_t timer_num )
{
	...
	if ( timer_num < m_num_timers )
	{
		...
		m_tc->SR = ~static_cast<uint16_t>
			( static_cast<uint16_t>( TIM_DIER_CC1IE ) << timer_num );
...
void uC_Multi_Timers::Stop_Timeout( uint8_t timer_num )
{
	if ( timer_num < m_num_timers )
	{
		m_tc->DIER &= ~static_cast<uint16_t>
			( static_cast<uint16_t>( TIM_DIER_CC1IE ) << timer_num );
	}
}
...
bool uC_Multi_Timers::Timeout_Running( uint8_t timer_num ) const
{
	...
	if ( timer_num < m_num_timers )
	{
		if ( ( m_tc->DIER & static_cast<uint16_t>
			   ( static_cast<uint16_t>( TIM_DIER_CC1IE ) << timer_num ) ) != 0U )
		{
			timer_running = true;
		}
	}
...
bool uC_Multi_Timers::Timeout_Pending( uint8_t timer_num ) const
{
	...
	if ( timer_num < m_num_timers )
	{
		...
		if ( ( status & static_cast<uint16_t>
			   ( static_cast<uint16_t>( TIM_SR_CC1IF ) << timer_num ) ) != 0U )
		{
