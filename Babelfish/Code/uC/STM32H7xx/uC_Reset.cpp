...
uC_Reset::uC_Reset( void )
{
	...
	reset_id = RCC->RSR;	// Get the Reset Cause Register.
	RCC->RSR |= RCC_RSR_RMVF;

	if ( ( reset_id & RCC_RSR_PORRSTF ) != 0U )
	{
		Set_Bit( m_reset_id, RESET_POWER_UP_TRIGRD );
	}

	if ( ( ( reset_id & RCC_RSR_WWDG1RSTF ) != 0U ) ||
		 ( ( reset_id & RCC_RSR_IWDG1RSTF ) != 0U ) )
	{
		Set_Bit( m_reset_id, RESET_WATCHDOG_TRIGRD );
	}

	if ( ( reset_id & RCC_RSR_SFTRSTF ) != 0U )
	{
		Set_Bit( m_reset_id, RESET_SOFTWARE_TRIGRD );
	}

	if ( ( ( reset_id & RCC_RSR_LPWRRSTF ) != 0U ) ||
		 ( ( reset_id & RCC_RSR_BORRSTF ) != 0U ) )
	{
		Set_Bit( m_reset_id, RESET_BROWN_OUT_TRIGRD );
	}

	if ( ( reset_id & RCC_RSR_D2RSTF ) != 0U )
	{
		Set_Bit( m_reset_id, RESET_D2_DOMAIN_TRIGRD );
	}

	if ( ( reset_id & RCC_RSR_D1RSTF ) != 0U )
	{
		Set_Bit( m_reset_id, RESET_D1_DOMAIN_TRIGRD );
	}
	/*The pin reset seems to be triggered by any reset.
	   So we mask it off if another reset was the cause.*/
	if ( ( ( reset_id & RCC_RSR_PINRSTF ) != 0U ) &&
		 ( m_reset_id == 0U ) )
	{
