 ...
 *****************************************************************************************
 */
uC_I2C::uC_I2C( uint8_t channel, i2c_mode_t speed_mode,
				bool synchronus_transfer, bool enable_dma,
...
void uC_I2C::Init_I2C( uint8_t i2c_channel, i2c_mode_t i2c_mode )
{
	...
	/**
	 * I2C_1 is set to use D0 and D1 for SDA and SCL respectively on nucleo board.
	 * GPIO is configured no pullup/pulldown. Use a ~2.2k resistor for pullup.
	 */
	...
	if ( true == static_cast<bool>( m_i2c->CR1 & I2C_CR1_PE ) )
	{
	...
	Clr_Mask( m_i2c->CR2, I2C_CR1_DNF );
...
{
	uint32_t timeout_count = 200000U;
	uint32_t elapsed_time = 0;

	uint32_t sample_time = uC_TIME_NSEC_Sample();

	while ( ( ( *reg & flag_mask ) == val ) && ( elapsed_time < timeout_count ) )
	{
		elapsed_time = uC_TIME_NSEC_Elapsed( sample_time );
	}

	return ( ( timeout_count <= elapsed_time ) ? I2C_ERROR : NO_ERROR );
}
...
uC_I2C::return_type uC_I2C::Start_Frame_Timeout() const
{
	...
	while ( ( ( m_i2c->CR2 & I2C_CR2_START ) == ( 0x01 << I2C_CR2_START_Pos ) ) && ( elapsed_time < timeout_count ) )
	{
		elapsed_time = uC_TIME_NSEC_Elapsed( sample_time );
	}
...
{
	m_tx_dma->Disable();
	m_tx_dma->Set_Buff( data, &m_i2c->TXDR, length );
}
...
{
	m_rx_dma->Disable();
	m_rx_dma->Set_Buff( data, &m_i2c->RXDR, length );
}
...
uC_I2C::return_type uC_I2C::Bus_Busy_TimeOut( void )
{
	...
	while ( ( ( m_i2c->ISR & I2C_ISR_BUSY ) == ( 0x01U << I2C_ISR_BUSY_Pos ) ) && ( elapsed_time < timeout_count ) )
	{
		elapsed_time = uC_TIME_NSEC_Elapsed( sample_time );
	}
