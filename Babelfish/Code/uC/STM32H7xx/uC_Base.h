...
enum
{
	...
	RCC_PERIPH_CLCK_APB1H,
	RCC_PERIPH_CLCK_APB2,
	RCC_PERIPH_CLCK_APB3,
	RCC_PERIPH_CLCK_APB4,
	RCC_PERIPH_CLCK_MAX
};
...
typedef struct uC_BASE_USART_IO_STRUCT
{
	...
	IRQn_Type irq_num;
	uint8_t usart_id;
	uint16_t rx_dma_channel;
	uint16_t tx_dma_channel;
...
typedef struct uC_BASE_ATOD_IO_STRUCT
{
	...
	ADC_Common_TypeDef* common_reg_ctrl;
	PERIPH_DEF_ST periph_def;
	// uint8_t pclock_freq_index;
	IRQn_Type irq_num;
...
{
#if defined ( uC_STM32H743_USAGE )
	ETH_TypeDef* reg_ctrl;
	PERIPH_DEF_ST periph_def;
	IRQn_Type irq_num;
	// uint32_t rmii_sel_mask;
	uint32_t phy_if_sel;	/// >MII or RMII
...
class uC_Base
{
	...
	public:
		...
		virtual uC_BASE_USB_IO_STRUCT const* Get_USB_Ctrl( uint8_t port ) {return ( NULL );}
