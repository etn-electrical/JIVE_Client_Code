...
#define NVIC_VectTab_RAM             ( 0x24000000U )
							   ...
							   uint16_t priority )
{
	...
	if ( ( SCB->VTOR & NVIC_VectTab_RAM ) != 0U )
	{
		/**
		 *@remark Coding Standard Deviation:
		 *@n MISRA-C++[2008] Required Rule 5-2-7, 5-2-8, 5-2-9, cast from enum to unsigned short
		...
		{
			BF_ASSERT( false );	// Helpful tip: You should clear the vector first.
		}
	}
	else
	{
		BF_ASSERT( false );	// We did not construct the ram int vectors.
	}
