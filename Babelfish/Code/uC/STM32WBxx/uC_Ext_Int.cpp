	...
	m_ext_pio_ctrl( reinterpret_cast<uC_USER_IO_STRUCT const*>( nullptr ) )
{
	...
	// __HAL_RCC_SYSCFG_CLK_ENABLE(); //not needed for STM32WB, no separate SYSCFG clk enable bit in APB2 enable
	// register
	...
	EXTI->IMR1 &= ~activated_input_mask;
							 ...
							 uint8_t priority ) const
{
	...
	EXTI->RTSR1 &= ~activated_input_mask;
	EXTI->FTSR1 &= ~activated_input_mask;
...
void uC_Ext_Int::External_Int_Vect_5to9( void )
{
	...
	int_request_trigger = EXTI->PR1;
	EXTI->PR1 |= static_cast<uint32_t>( uC_EXT_INT_5to9_CLR_MASK );
...
void uC_Ext_Int::External_Int_Vect_10to15( void )
{
	...
	int_request_trigger = EXTI->PR1;
	EXTI->PR1 |= static_cast<uint32_t>( uC_EXT_INT_10to15_CLR_MASK );
