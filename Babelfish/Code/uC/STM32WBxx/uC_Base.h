...
enum
{
	// new additions for AHB3, AHB4, AHB5 and APB3 for STM32WB
	RCC_PERIPH_CLCK_AHB1,
	RCC_PERIPH_CLCK_AHB2,
	RCC_PERIPH_CLCK_AHB3,
	RCC_PERIPH_CLCK_AHB4,
	RCC_PERIPH_CLCK_AHB5,
	RCC_PERIPH_CLCK_APB1_REG1,
	RCC_PERIPH_CLCK_APB1_REG2,
	RCC_PERIPH_CLCK_APB2,
	RCC_PERIPH_CLCK_APB3,
	RCC_PERIPH_CLCK_MAX
};

typedef struct PERIPH_DEF_ST
{
	uint8_t pclock_reg_index;	// PCLK register
	uint32_t pid;				// bit position in PCLK reigister for the peripheral
} PERIPH_DEF_ST;
...
typedef struct uC_BASE_LPTIMER_IO_STRUCT
{
	LPTIM_TypeDef* reg_ctrl;
	PERIPH_DEF_ST periph_def;
	uint8_t pclock_freq_index;
	IRQn_Type irq_num;
	IRQn_Type irq_num_cc;
...
typedef struct uC_BASE_DMA_IO_STRUCT
{
	...
	DMAMUX_Channel_TypeDef* dmamux_channel_ctrl;
	PERIPH_DEF_ST periph_def;
	IRQn_Type irq_num;
...
{
#if defined ( uC_STM32F407_USAGE ) || defined ( uC_STM32F427_USAGE ) || defined ( uC_STM32F437_USAGE )
...
class uC_Base
{
	...
	public:
		...
		virtual uC_BASE_LPTIMER_IO_STRUCT const* Get_LPTimer_Ctrl( uint32_t timer_block ) = 0;
