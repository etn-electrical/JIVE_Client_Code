...
uC_RTC* uC_RTC::st_RTC_instance = nullptr;
 ...
 *****************************************************************************************
 */
// with use in Rtc_If.cpp this evaluates to duplicate default construct, hence commented
// uC_RTC::uC_RTC( void ):
// m_RTC( nullptr ),
// m_time_recovered( true ),
// m_datetime_format( RTC_BCD_FORMAT )
// {}
...
uC_RTC::uC_RTC( RTC_clock_source_t clk_src, RTC_format_t format )
{
	...
	// TODO understand the functionality and add the eqivalent for WB55 controller
	/* Enable PWR peripheral clock */
	// RCC->APB1ENR |= RCC_APB1ENR_PWREN;

	/* disable backup domain write protection
	   this is required to modify BDCR register*/
	PWR->CR1 |= PWR_CR1_DBP;
	...
	else if ( clk_src == CLOCK_SOURCE_LSI )
	{
		...
		while ( ( RCC->CSR & RCC_CSR_LSI1RDY ) != RCC_CSR_LSI1RDY )
		{}
...
void uC_RTC::Set_Time( RTC_time_t* time )
{
	...
	m_RTC->CR &= static_cast<uint32_t>( ~RTC_CR_BKP );
...
void uC_RTC::Get_Time( RTC_time_t* time )
{
	...
	temp = m_RTC->DR;
	temp = temp;// Misra warning suppress
 ...
 *****************************************************************************************
 */
bool uC_RTC::Backup_Register_Write( uint8_t BackupRegister, uint32_t Data )
{
	uint32_t tmp;
	bool return_value = false;

	tmp = ( uint32_t )&( m_RTC->BKP0R );
	tmp += ( BackupRegister * 4U );

	/* Write the specified register */
	*( __IO uint32_t* )tmp = ( uint32_t )Data;

	if ( Backup_Register_Read( BackupRegister ) == Data )
	{
		return_value = true;
	}
	return ( return_value );
 ...
 *****************************************************************************************
 */
uint32_t uC_RTC::Backup_Register_Read( uint8_t BackupRegister )
{
	uint32_t tmp;
 ...
 *****************************************************************************************
 */
uC_RTC* uC_RTC::Get_Instance()
{
	return ( st_RTC_instance );
}
