...
#define uC_DMA_ALL_STATUS_FLAGS_MASK            0xF
...
typedef struct DMA_Channel_MUX
{
	uint8_t DMA_Requested_by;
	uint8_t DMA_Channel;
} DMA_CHannel_MUX;
...
class uC_DMA
{
	public:
		 ...
		 * @param
		 *					dma_required_for	: the peripheral which is requesting DMA service
		 *					dma_mux_chnl		: The DMA-MUX channel output on which transfer should be routed to
		 *										  among 7 channels each in DMA1 and DMA2
		 *					direction			: direction of transfer MEM-to-PERH or PERH-to-MEM
		 *					datatype_size		: size of each transfer
		 *					mem_inc				: should next memory location be accessed automatically
		 *					periph_inc			: should next peripheral location be accessed automatically
		 *					circular			: should DMA transfer continue from starting address once the limit is
		 *reached
		 *					mem_to_mem			: is this a MEM-to-MEM transfer request
		 * @details Configures the DMA pheripheral and selects the channel needed in DMAMUX
		 **/
		uC_DMA( DMA_Channel_MUX* dma_channel_mux, uC_DMA_DIR_ENUM direction = uC_DMA_DIR_FROM_PERIPH,
		...
		inline bool Empty( void )   { return ( m_dma->dma_channel_ctrl->CNDTR == 0 ); }
