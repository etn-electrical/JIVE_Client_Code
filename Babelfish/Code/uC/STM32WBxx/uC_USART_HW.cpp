	...
	m_int_vect_cback( reinterpret_cast<Advanced_Vect*>( nullptr ) )
{
	...
	if ( ( m_io_ctrl->tx_dma_channel != DMAMUX_MAX_REQ_ID ) &&
		 ( m_io_ctrl->rx_dma_channel != DMAMUX_MAX_REQ_ID ) &&
		 ( enable_dma == true ) )
	{
		{// DMA setup for Tx Channel
			DMA_Channel_MUX dma_TX_channel_mux;		// local variable needed just to transfer value, not requried
													// outside
			dma_TX_channel_mux.DMA_Channel = uC_DMA1_CHANNEL_1;
			dma_TX_channel_mux.DMA_Requested_by = m_io_ctrl->tx_dma_channel;
			m_tx_dma_ctrl = new uC_DMA( &dma_TX_channel_mux, uC_DMA_DIR_FROM_MEMORY,
										static_cast<uint8_t>( sizeof( uint8_t ) ),
										uC_DMA_MEM_INC_TRUE,
										uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );
		}

		{// DMA setup for Rx Channel
			DMA_Channel_MUX dma_RX_channel_mux;		// local variable needed just to transfer value, not required
													// outside
			dma_RX_channel_mux.DMA_Channel = uC_DMA1_CHANNEL_2;
			dma_RX_channel_mux.DMA_Requested_by = m_io_ctrl->rx_dma_channel;
			m_rx_dma_ctrl = new uC_DMA( &dma_RX_channel_mux, uC_DMA_DIR_FROM_PERIPH,
										static_cast<uint8_t>( sizeof( uint8_t ) ),
										uC_DMA_MEM_INC_TRUE,
										uC_DMA_PERIPH_INC_FALSE, uC_DMA_CIRC_FALSE );
		}
 ...
 *****************************************************************************************
 */
uC_USART_HW::~uC_USART_HW()
{
	if ( m_tx_dma_ctrl != nullptr )
	{
...
void uC_USART_HW::Set_Baud_Rate( uint32_t baud_rate )
{
	...
	uint64_t usart_clock_freq;
	uint64_t temp_baud;

	usart_clock_freq =
		( uint64_t )( uC_Base::Self() )->Get_PClock_Freq( m_io_ctrl->periph_def.pclock_reg_index );
	// usart_clock_freq = 8000000U;
	// usart_clock_freq = usart_clock_freq << 4;

	if ( m_io_ctrl->usart_id == uC_BASE_USART_PORT_1 )
	{
		temp_baud = usart_clock_freq / ( baud_rate );
		m_usart->BRR = static_cast<uint16_t>( temp_baud );
	}
	else if ( m_io_ctrl->usart_id == uC_BASE_LPUART_PORT_1 )
	{
		usart_clock_freq = 16000000;	// 16 MHz
		temp_baud = ( usart_clock_freq * 256 ) / ( baud_rate );
		m_usart->BRR = static_cast<uint32_t>( temp_baud & 0xFFFFF );	// BRR is 20 bit (0xFFFFF) in LPUART
	}
	else
	{
		BF_ASSERT( false );
