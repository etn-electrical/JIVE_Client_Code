...
// @brief    The sample code to configure USART port to perform Soft DMA  data RX and data Tx functions on STM32F407
// controller.
// ModbusMAT 1.1 tool or Docklight is used to verify that the data transmit and recive functions using USART DMA are
// successfull.
...
// @details   Correctly configure the USART port, Timer and GPIO pins in APP_IO_CONFIG_STM32F407.h to initialize the
// uC_USART_HW object as below
...
#include "uC_USART_Interrupt.h"
#include "APP_IO_CONFIG_STM32WB55.h"
...
{
	usart1 = new uC_USART_HW( EXAMPLE_USART_PORT, &EXAMPLE_USART_PIO_VCP_STLINK, false );	// third argument is for
																							// using dma. we set it
																							// false to not use dma.
	/**
	...
	usart1->Enable_TX();
	usart1->Enable_RX();
	usart1->Set_TX_Data( 0x35 );// random data just to start tx
}
