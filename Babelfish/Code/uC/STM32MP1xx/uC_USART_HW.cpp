...
/*HSI clock value for UART clock*/
#define UART_Clock 64000000U;
	...
	m_int_vect_cback( reinterpret_cast<Advanced_Vect*>( nullptr ) )
{
	...
	if ( m_io_ctrl != reinterpret_cast<uC_BASE_USART_IO_STRUCT const*>( nullptr ) )
	{
...
void uC_USART_HW::Set_Baud_Rate( uint32_t baud_rate )
{
	...
	usart_clock_freq = UART_Clock;		// using HSI clock frequency as input to UART clock
	temp_baud = ( ( ( ( usart_clock_freq ) + ( ( baud_rate ) / 2U ) ) ) / ( baud_rate ) );
