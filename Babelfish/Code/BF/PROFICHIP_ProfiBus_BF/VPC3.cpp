...
VPC3::VPC3( uC_SPI* spi_ctrl, uint8_t chip_select, VPC3_CHIP_CONFIG_TD const* chip_config, BF_Lib::Output* reset_ctrl,
			ProfiBus_Display* profi_display ) :
	m_dci_access( nullptr ),
	m_dci_access_output( nullptr )
{
 ...
 *****************************************************************************************
 * ----- Function Header -----------------------------------------------------------------
 * ---Function: Calculates required number of wait states for Static Memory Controller----
 * ----master_clk_freq is in hertz--------------------------------------------------------
 * ----wait_time is in (ns)---------------------------------------------------------------
 * -----Returns Number of Wait States (NWS)--------------- (SDT) 6/30/2009----------------
 * -----Desired Formula: NWS= MCK(Hz)*WAIT_TIME(sec) -------------------------------------
 *----------------------------------------------------------------------------------------
 * Wait_Time values are specified in nanoseconds which scales the value by 1000000000
 * Desired formula: NWS/1073741824 = MCK * (1000000000 + 73741824)*WAIT_TIME(sec))
 *		NWS/1073741824 = MCK * (1000000000*WAIT_TIME(sec) + 73741824*WAIT_TIME(sec))
 *		NWS/1073741824 = MCK * (1000000000*WAIT_TIME(sec) + 1000000000*WAIT_TIME(sec)/16)
 *		NWS/1073741824 = MCK * (1000000000*WAIT_TIME(sec) + 62500000*WAIT_TIME(sec))
 *       % error = (1073741824-1062500000)/1073741824 = 0.01047 or 1% error
 *****************************************************************************************
 */

uint16_t VPC3::Calculate_nws( void )
{
	UINT64 master_clk_freq;
	UINT64 wait_time_ns;
	UINT64 calculated_nws;
	uint16_t nws;

	master_clk_freq = MASTER_CLOCK_FREQUENCY;
	wait_time_ns = SMC_VPC3_WAIT_TIME_NS;

	calculated_nws = ( master_clk_freq * ( wait_time_ns + ( wait_time_ns >> 4 ) ) );
	calculated_nws = ( calculated_nws >> 30 );
	nws = ( ( uint16_t )calculated_nws );
	return ( nws );
}
...
void VPC3::SetInterrupt( void )
{
	...
	VPC3_ext_int->Attach_IRQ( Profibus_ISR, EXT_INT_FALLING_EDGE_TRIG, uC_IO_IN_CFG_HIZ, uC_INT_HW_PRIORITY_15 );
...
bool VPC3::Read_Status( uint8_t* status )
{
	...
	if ( m_spi_ctrl->Get_Semaphore( VPC3_SPI_SEMAPHORE_TIMEOUT ) )
	{
		...
		while ( !m_spi_ctrl->TX_Shift_Empty() )
		{}
		while ( !m_spi_ctrl->RX_Data_Reg_Full() )		// Dan Hosko added to ensure Transaction Complete.
		{}
...
void VPC3::MemSet( VPC3_ADR wAddress, UBYTE bValue, UWORD wLength )
{
	...
	if ( m_spi_ctrl->Get_Semaphore( VPC3_SPI_SEMAPHORE_TIMEOUT ) )
	{
		...
		while ( !m_spi_ctrl->TX_Shift_Empty() )
		{}
		// Pre-Conformance issue fix : should handle length,bvalue was
		// incorrect counter
		for ( i = 0; i < wLength; i++ )
		{
