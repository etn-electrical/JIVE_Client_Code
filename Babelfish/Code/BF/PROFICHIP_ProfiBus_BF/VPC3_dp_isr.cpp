...
/* ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
#if ( VPC3_SERIAL_MODE == 1 )		// Updated to 1 by Sagar #stackUpgrade
/*!
...
void VPC3_Isr( void )
{
	...
	CopyFromVpc3_( ( MEM_UNSIGNED8_PTR )&pDpSystem->wInterruptEvent,
				   ( ( uint8_t* )( VPC3_ADR )( Vpc3AsicAddress + 0x02 ) ), 2 );
	...
	if ( VPC3_GET_IND_NEW_EXT_PRM_DATA() )
	{
		...
		do
		{
			...
			CopyFromVpc3_( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0], VPC3_GET_PRM_BUF_PTR(),
						   bPrmLength );

			if ( UserChkNewExtPrmData( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0],
									   bPrmLength ) == DP_OK )
			{
			...
			}	/* else of if( UserChkNewExtPrmData(
				   (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK */
	...
	CopyToVpc3_( ( ( UBYTE* )( VPC3_ADR )( Vpc3AsicAddress + 0x02 ) ), ( MEM_UNSIGNED8_PTR )&pDpSystem->wInterruptEvent,
				 2 );
...
void VPC3_Poll( void )
{
	...
	CopyFromVpc3_( ( MEM_UNSIGNED8_PTR )&pDpSystem->wPollInterruptEvent,
				   ( ( uint8_t* )( VPC3_ADR )( Vpc3AsicAddress ) ), 2 );
	...
	if ( VPC3_POLL_IND_NEW_PRM_DATA() )
	{
		...
		do
		{
			...
			if ( DpPrm_ChkNewPrmData( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK )
			{
				...
				bResult = VPC3_SET_PRM_DATA_OK();
			}	/* if( DpPrm_ChkNewPrmData(
				   (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK )
				 */
			else
			{
				bResult = VPC3_SET_PRM_DATA_NOT_OK();
			}	/* else of if( DpPrm_ChkNewPrmData(
				   (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bPrmLength ) == DP_OK )
				 */
		} while ( bResult == VPC3_PRM_CONFLICT );
	}		/* if( VPC3_POLL_IND_NEW_PRM_DATA() ) */
	...
	if ( VPC3_POLL_IND_NEW_CFG_DATA() )
	{
		...
		do
		{
			...
			CopyFromVpc3_( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0], VPC3_GET_CFG_BUF_PTR(),
						   bCfgLength );
			...
			switch ( DpCfg_ChkNewCfgData( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0], bCfgLength ) )
			{
				...
				case DP_CFG_UPDATE:
				{
					...
					if ( DP_OK !=
						 VPC3_CalculateInpOutpLength( ( MEM_UNSIGNED8_PTR )&pDpSystem->abPrmCfgSsaHelpBuffer[0],
													  bCfgLength ) )
					{
						#if DP_MSAC_C1
						MSAC_C1_DoCfgNotOk();
						...
						bResult = VPC3_SET_CFG_DATA_NOT_OK();
					}	/* if( DP_OK != VPC3_CalculateInpOutpLength(
						   (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bCfgLength ) ) */
					else
					{
						/* set IO-Length */
						...
						bResult = VPC3_SET_CFG_DATA_OK();
					}	/* else of if( DP_OK != VPC3_CalculateInpOutpLength(
						   (MEM_UNSIGNED8_PTR)&pDpSystem->abPrmCfgSsaHelpBuffer[0], bCfgLength ) ) */
					break;
				}	/* case DP_CFG_UPDATE: */
	...
	CopyToVpc3_( ( ( uint8_t* )( VPC3_ADR )( Vpc3AsicAddress + 0x02 ) ),
				 ( MEM_UNSIGNED8_PTR )&pDpSystem->wPollInterruptEvent, 2 );
 ...
 *****************************************************************************************
 */
void VPC3_Poll_DX_Out_And_Param( void )
{
	UWORD wPollInterruptClearEvent = 0;
	UWORD wPollMask = 0;
	...
	wPollMask = ( pDpSystem->wPollInterruptEvent & ( VPC3_INT16_DX_OUT | VPC3_INT16_NEW_PRM_DATA ) );
	...
	if ( VPC3_POLL_IND_DX_OUT() )
	{
		...
		#endif
		DpAppl_IsrDxOut();
		wPollInterruptClearEvent = wPollInterruptClearEvent | VPC3_INT16_DX_OUT;
	}	/* if( VPC3_POLL_IND_DX_OUT() ) */

	// #Pre-Conformance Change:Parameterization frame handling in poll
	if ( VPC3_POLL_IND_NEW_PRM_DATA() )
	{
		pDpSystem->wEvent |= VPC3_EV_NEW_PRM_DATA;
		#if DP_INTERRUPT_MASK_8BIT == 0
		VPC3_POLL_CON_IND_NEW_PRM_DATA();
		#endif	/* #if DP_INTERRUPT_MASK_8BIT == 0 */
		wPollInterruptClearEvent = wPollInterruptClearEvent | VPC3_INT16_NEW_PRM_DATA;
	}
	Vpc3Write( bVpc3WoIntAck_L, ( uint8_t )( wPollInterruptClearEvent & 0xFF ) );
	Vpc3Write( bVpc3WoIntAck_H, ( uint8_t )( wPollInterruptClearEvent >> 8 ) );
}
