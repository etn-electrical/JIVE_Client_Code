 ...
 *****************************************************************************************
 */
#ifndef IEC61850_GOOSE_DCI_H
#define IEC61850_GOOSE_DCI_H
 ...
 **************************************************************************************************
 */
class IEC61850_GOOSE_DCI
{
	public:
		typedef uint16_t group_param_t;	///< IEC61850 GOOSE group param ID

		typedef uint16_t sub_group_param_t;	///< IEC61850 GOOSE Sub group param ID
		...
		/**
		 * @brief GOOSE Message sub group assembly details
		 */
		...
		/**
		 * @brief GOOSE Message main group assembly details
		 */
		struct group_assembly_t
		{
			group_param_t group_dcid;								///< Main Group ID deatils
			const char_t* cb_ref_name;									///< Control block refernce name details
		...
		/**
		 * @brief Container for the entire identity of a IEC61850 GOOSE Publish or subscribe messages
		 */
		struct goose_group_info_t
		{
		...
		/**
		 * @brief Container for the entire identity of a IEC61850 GOOSE Message
		 */
		struct goose_message_info_t
		{
			const goose_group_info_t* pub_group_profile;	///< Pointer to publish group
			const goose_group_info_t* sub_group_profile;	///< Pointer to Subscribe group
		};

		/**
		 *  @brief Constructor
		 *  @details Creates a IEC61850_GOOSE_DCI
		 *  @param[in] goose_message_info , pointer to access IEC61850 GOOSE message continer
		 *  @param[in] unique_id , A source ID for the patron access.
		 */
		IEC61850_GOOSE_DCI( const goose_message_info_t* goose_message_info, DCI_SOURCE_ID_TD unique_id );

		/**
		 *  @brief Destructor
		 *  @details This will get called when object of IEC61850_GOOSE_DCI goes out of scope
		 *  @return None
		 */
		~IEC61850_GOOSE_DCI( void );
	...
	private:
		 ...
		 * @param[in] object IEC61850_GOOSE_DCI class object
		 * @return None
		 */
		IEC61850_GOOSE_DCI( const IEC61850_GOOSE_DCI& object );
		 ...
		 * @return None
		 */
		IEC61850_GOOSE_DCI & operator =( const IEC61850_GOOSE_DCI& object );

		/**
		 *  @brief Provides "GET" data access to single IEC61850_GOOSE_DCI parameter.
		...
		/**
		 * @brief Pointer to GOOSE message continer
		 */
		const goose_message_info_t* m_goose_message_info;
		...
		const sub_group_param_t IEC61850_GOOSE_INVALID_SUBGROUP_ID = 256U;	///< IEC61850 GOOSE invalid subgroup ID
