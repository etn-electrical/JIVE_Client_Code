/**
 *****************************************************************************************
 *	@file
 *	@details See header file for module overview.
 *	@copyright 2013 Eaton Corporation. All Rights Reserved.
 *
 *****************************************************************************************
 */
#include "Includes.h"
#include "Modbus_Prod_Cons.h"
#include "Modbus_Net.h"

/*
 *****************************************************************************************
 * See header file for function definition.
 *****************************************************************************************
 */
Modbus_Prod_Cons::Modbus_Prod_Cons( DCI_ID_TD prod_list_id, DCI_ID_TD prod_data_id,
									DCI_ID_TD cons_list_id, DCI_ID_TD cons_data_id,
									Modbus_DCI* modbus_dci_ctrl ) :
	m_prod_list_owner( reinterpret_cast<DCI_Owner*>( nullptr ) ),
	m_prod_data_owner( reinterpret_cast<DCI_Owner*>( nullptr ) ),
	m_cons_list_owner( reinterpret_cast<DCI_Owner*>( nullptr ) ),
	m_cons_data_owner( reinterpret_cast<DCI_Owner*>( nullptr ) ),
	m_dummy_owner_data( 0U ),
	m_modbus_dci_ctrl( modbus_dci_ctrl )
{
	m_prod_list_owner = new DCI_Owner( prod_list_id );

	m_prod_data_owner = new DCI_Owner( prod_data_id, &m_dummy_owner_data );

	m_cons_list_owner = new DCI_Owner( cons_list_id );

	m_cons_data_owner = new DCI_Owner( cons_data_id, &m_dummy_owner_data );

	m_modbus_dci_ctrl = modbus_dci_ctrl;

	m_prod_data_owner->Attach_Callback( &Prod_Data_Callback_Static,
										reinterpret_cast<DCI_CBACK_PARAM_TD>( this ),
										DCI_ACCESS_GET_RAM_CMD_MSK );
	m_cons_data_owner->Attach_Callback( &Cons_Data_Callback_Static,
										reinterpret_cast<DCI_CBACK_PARAM_TD>( this ),
										( DCI_ACCESS_GET_RAM_CMD_MSK |
										  DCI_ACCESS_SET_RAM_CMD_MSK ) );
}

/*
 *****************************************************************************************
 * See header file for function definition.
 *****************************************************************************************
 */
DCI_CB_RET_TD Modbus_Prod_Cons::Data_Callback( DCI_ACCESS_ST_TD* access_struct,
											   bool prod_data )
{
	DCI_CB_RET_TD return_status = DCI_CBACK_RET_NO_FURTHER_PROCESSING;
	DCI_ERROR_TD dci_error;
	uint8_t* data_ptr;
	uint16_t num_registers;	// uint_fast16_t num_registers; // to check
	uint64_t working_reg = 0U;
	uint16_t list_index;// uint_fast16_t list_index; // to check
	uint8_t temp_byte;
	DCI_Owner* list_owner;

	if ( prod_data == true )
	{
		list_owner = m_prod_list_owner;	// ->Check_Out( list );
	}
	else
	{
		list_owner = m_cons_list_owner;	// ->Check_Out( list );
	}

	list_index = access_struct->data_access.offset >> DIV_BY_2;
	num_registers = access_struct->data_access.length >> DIV_BY_2;
	data_ptr = reinterpret_cast<uint8_t*>( access_struct->data_access.data );
	switch ( access_struct->command )
	{
		case DCI_ACCESS_GET_RAM_CMD:
			while ( num_registers > 0U )
			{
				list_owner->Check_Out_Index( &working_reg,
											 static_cast<DCI_LENGTH_TD>( list_index ) );
				dci_error = m_modbus_dci_ctrl->Get_Reg_Data( working_reg, 1U,
															 data_ptr, MB_ATTRIB_RAM_ACCESS );
				if ( dci_error != DCI_ERR_NO_ERROR )
				{
					data_ptr[0] = 0U;
					data_ptr[1] = 0U;
				}
				else
				{
					temp_byte = data_ptr[0];// Do some swapping here since it will get reswapped once
											// it leaves here.
					data_ptr[0] = data_ptr[1];
					data_ptr[1] = temp_byte;
				}
				data_ptr += 2U;
				list_index++;
				num_registers--;
			}
			break;

		case DCI_ACCESS_SET_RAM_CMD:
			while ( num_registers > 0U )
			{
				list_owner->Check_Out_Index( &working_reg,
											 static_cast<DCI_LENGTH_TD>( list_index ) );

				temp_byte = data_ptr[0];// Do some swapping here since it will get reswapped once it
										// leaves here.
				data_ptr[0] = data_ptr[1];
				data_ptr[1] = temp_byte;

				m_modbus_dci_ctrl->Set_Reg_Data( static_cast<uint16_t>( working_reg ), 1U,
												 data_ptr, MB_ATTRIB_RAM_ACCESS );	// We don't really
																					// care about the
																					// error. Caveat
																					// emptor.
				data_ptr += 2U;
				list_index++;
				num_registers--;
			}
			break;

		/* Following 'case' labels are added to get rid of PC-lint issue 'Info 788' while
		   MISRAfication*/
		case DCI_ACCESS_GET_INIT_CMD:
		case DCI_ACCESS_SET_INIT_CMD:
		case DCI_ACCESS_GET_DEFAULT_CMD:
		case DCI_ACCESS_GET_LENGTH_CMD:
		case DCI_ACCESS_GET_ATTRIB_SUPPORT_CMD:
		case DCI_ACCESS_GET_MIN_CMD:
		case DCI_ACCESS_GET_MAX_CMD:
		case DCI_ACCESS_GET_ENUM_LIST_LEN_CMD:
		case DCI_ACCESS_GET_ENUM_CMD:
		case DCI_ACCESS_RAM_TO_INIT_CMD:
		case DCI_ACCESS_INIT_TO_RAM_CMD:
		case DCI_ACCESS_DEFAULT_TO_INIT_CMD:
		case DCI_ACCESS_DEFAULT_TO_RAM_CMD:
		case DCI_ACCESS_POST_SET_RAM_CMD:
		case DCI_ACCESS_POST_SET_INIT_CMD:
		case DCI_ACCESS_UNDEFINED_COMMAND:

		default:

			break;
	}

	return ( return_status );
}
