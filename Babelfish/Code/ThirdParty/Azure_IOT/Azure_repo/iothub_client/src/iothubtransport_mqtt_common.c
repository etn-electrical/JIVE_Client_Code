...
static const char* REPORTED_PROPERTIES_TOPIC = "$iothub/twin/PATCH/properties/reported/?$rid=%u";
static const char* GET_PROPERTIES_TOPIC = "$iothub/twin/GET/?$rid=%u";
...
static void DisconnectFromClient(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    if (transport_data->currPacketState != DISCONNECT_TYPE)
    {
        ...
        if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_CONNECTED)
        {
			...
			/**************************************************************************************
			  *************************************************************************************
			  ############ Eaton Fix: DO NOT MISS THIS CHANGE WHILE UPDATING AZURE SDK ############
			  *************************************************************************************
			  *************************************************************************************
			  LTK-7768 to fix watchdog reset due to ethernet unplug
			  Here if the disconnect call fails, don't wait (more than once) for the disconnect
			  flag (that will never be set)
			 */
            int rc = mqtt_client_disconnect(transport_data->mqttClient, mqtt_disconnect_cb, &transport_data->disconnect_recv_flag);
            ...
            do
            {
                ...
                disconnect_ctr++;
                ThreadAPI_Sleep(50);
            } while ((rc == 0) && (disconnect_ctr < MAX_DISCONNECT_VALUE) && (transport_data->disconnect_recv_flag == 0));
        }
        DestroyXioTransport(transport_data);
...
TRANSPORT_LL_HANDLE IoTHubTransport_MQTT_Common_Create(const IOTHUBTRANSPORT_CONFIG* config, MQTT_GET_IO_TRANSPORT get_io_transport, TRANSPORT_CALLBACKS_INFO* cb_info, void* ctx)
{
    ...
    {
        LogError("Invalid Argument config: %p, get_io_transport: %p, cb_info: %p", config, (void*) get_io_transport, (void*) cb_info);
        result = NULL;
    }
