...
#define DP_VERSION_FUNCTION         ((UBYTE)0x01)
#define DP_VERSION_BUGFIX           ((UBYTE)0x12)

/*---------------------------------------------------------------------------*/
/* function prototypes                                                       */
/*---------------------------------------------------------------------------*/
static DP_ERROR_CODE    VPC3_SetConstants          ( UBYTE bSlaveAddress, UWORD wIdentNumber );
...
/*---------------------------------------------------------------------------*/
   ...
   #ifdef REDUNDANCY
      UBYTE              bOldDebugCode_1;
      UBYTE              bOldDetail_1;
      UBYTE              bOldDebugCode_2;
      UBYTE              bOldDetail_2;
...
//                      sFdlInit.eC1SapSupported |= C1_SAP_51;
                     // Updated by Sagar #stackUpgrade : to remove enum operations error
                        temp =  sFdlInit.eC1SapSupported ;
                        temp |= C1_SAP_51;
                        sFdlInit.eC1SapSupported = (eC1Saps)temp; 
                        
                        #if DP_ALARM_OVER_SAP50
                           //sFdlInit.eC1SapSupported |= C1_SAP_50;
                        //Updated by Sumit for Stack upgrade
                        temp = sFdlInit.eC1SapSupported;
                        temp |= C1_SAP_50;
                        sFdlInit.eC1SapSupported = (eC1Saps)temp;
                        #endif /* #if DP_ALARM_OVER_SAP50 */
...
DP_ERROR_CODE bError;
   ...
   #if VPC3_SERIAL_MODE
      Vpc3Write( bVpc3WoIntMask_H, (UBYTE)(~(INIT_VPC3_IND_H)) );
      Vpc3Write( bVpc3WoIntMask_L, (UBYTE)(~(INIT_VPC3_IND_L)) );
   #endif /* #if VPC3_SERIAL_MODE */
...
{
DP_ERROR_CODE  bError;
UWORD       wOutBufferLength;    /**< calculated length of output buffer */
UWORD       wInBufferLength;     /**< calculated length of input buffer */
UWORD       wDiagBufferLength;   /**< calculated length of diagnostic buffer */
UWORD       wPrmBufferLength;    /**< calculated length of parameter buffer */
UWORD       wCfgBufferLength;    /**< calculated length of check-config buffer */
UWORD       wSsaBufferLength;    /**< calculated length of set-slave-address buffer */
UWORD       wAux2BufferLength;   /**< calculated length of aux buffer 2*/
...
/*-------------------------------------------------------------------*/
static DP_ERROR_CODE VPC3_CheckDxbLinkTable( UBYTE bNrOfLinks )
{
...
/*-------------------------------------------------------------------*/
static void VPC3_BuildDxbLinkStatus( UBYTE bNrOfLinks )
{
...
*/
void VPC3_GetErrorCounter( UWORD *wFalse, UWORD *wValid )
{
UBYTE abTemp[4];

   CopyFromVpc3_( &abTemp[0], VPC3_GET_ERROR_COUNTER_PTR(), 4 );
   *wFalse = abTemp[0] + (((UWORD)abTemp[1]) << 8);
   *wValid = abTemp[2] + (((UWORD)abTemp[3]) << 8);
} /* void VPC3_GetErrorCounter( UWORD *wFalse, UWORD *wValid ) */
...
/*!
  ...
  \retval 0 - no buffer available
*/
static VPC3_UNSIGNED8_PTR VPC3_UpdateDiagnosis( UBYTE bDiagControl, MEM_UNSIGNED8_PTR pbToUserDiagData, UBYTE bUserDiagLen )
{
...
*/
DP_ERROR_CODE VPC3_SetDiagnosis( MEM_UNSIGNED8_PTR pbToUserDiagData, UBYTE bUserDiagLength, UBYTE bDiagControl, UBYTE bCheckDiagFlag )
{
...
UBYTE           bDpState;
   ...
   if( VPC3_CheckDiagBufPtr() != NULL_PTR )
   {
