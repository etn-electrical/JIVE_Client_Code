...
extern int32 compute_external_cv_signature(ssl_t *ssl);
...
# ifdef USE_EXT_CERTIFICATE_VERIFY_SIGNING
#  ifdef USE_EXT_EXAMPLE_MODULE
        if (rc == PS_PENDING && matrixSslNeedCvSignature(ssl))
        {
            psAssert(g_enable_ext_cv_sig_op);
            rc = compute_external_cv_signature(ssl);
            if (rc < 0)
            {
                psTrace("Could not compute external cv sig\n");
                return MATRIXSSL_ERROR;
            }
            /* Retry encoding now that we have the sig. */
            rc = matrixSslReceivedData(ssl, (int32) transferred, &buf,
                    (uint32 *) &len);
            if (rc < 0)
            {
...
# if defined(USE_EXT_CERTIFICATE_VERIFY_SIGNING) && defined(USE_EXT_EXAMPLE_MODULE)
            g_enable_ext_cv_sig_op = atoi(optarg);
            if (g_enable_ext_cv_sig_op != 0 && g_enable_ext_cv_sig_op != 1)
            {
                Printf("-e option must be followed by either 0 (turn external " \
                    " CertificateVerfiy signing OFF) or 1 (turn it ON).\n");
                return -1;
            }
            break;
# else
