...
# define HANDSHAKE_POOL  "HANDSHAKE_POOL"
# define SESSION_POOL    "SESSION_POOL"
# define KEY_POOL        "G_KEY_POOL"

static psPool_t *getKeyPool(void *memAllocUserPtr);
...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    pool = matrixSslGetSessionPool(options->memAllocPtr);
    if (pool == NULL)
    {
        return PS_MEM_FAIL;
    }
...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
/*
    The DH key material above was stored in own pool on client side
 */
    if (ssl->sec.dhKeyPool)
    {
        psClosePool(ssl->sec.dhKeyPool); ssl->sec.dhKeyPool = NULL;
    }
# endif
...
#  endif
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    /* The DH key material was stored in own pool on client side */
    if (ssl->sec.eccDhKeyPool)
    {
        psClosePool(ssl->sec.eccDhKeyPool); ssl->sec.eccDhKeyPool = NULL;
    }
# endif
 ...
 */
    ...
    psFree(ssl, pool);
#ifdef USE_MATRIX_MEMORY_MANAGEMENT
    matrixSslCloseHandshakePool(hsPool);
    matrixSslCloseSessionPool(pool);
#endif /* USE_MATRIX_MEMORY_MANAGEMENT */
}
...
static psPool_t *getKeyPool(void *memAllocUserPtr)
{
    ...
    pool = psOpenPool(KEY_POOL, SSL_KEY_POOL_SIZE + sizeof(psPool_t), 0, NULL,
        memAllocUserPtr);
    psAddPoolCache(pool, sizeof(psX509Cert_t));
    psAddPoolCache(pool, sizeof(sslKeys_t));

    return pool;
}

/******************************************************************************/
psPool_t *matrixSslGetSessionPool(void *userPtr)
{
    int32 hwOh = 0;
...
# ifdef USE_ALPN
    /*  ALPN proto is technically 255 bytes each but that's too much */
    hwOh += (48 * MAX_PROTO_EXT);
# endif

# ifdef USE_STATELESS_SESSION_TICKETS
    /* Server will optionally allocate this structure to the session pool */
    hwOh += sizeof(sslSessionId_t);
# endif

/*
    Can supply the exact overhead value for pools because all that needs
    to be in the session pool by default is the sizeof(ssl_t) + pool overhead.
    Magic 21 (3 * 7 bytes) is for the max of three roundups of sizeof
 */
    return psOpenPool(SESSION_POOL, sizeof(ssl_t) + sizeof(psPool_t) + hwOh +
        sizeof(psMemHeader_t) + MIN_BLOCK_SIZE + 21, 0, NULL, userPtr);
}

/******************************************************************************/
void matrixSslCloseSessionPool(psPool_t *pool)
{
    psClosePool(pool);
}
...
/******************************************************************************/
/*
    More optimal handshake pool sizes can be determined by a combination
    of cipher type, available key size information, and server/client role.
 */
psPool_t *matrixSslGetHandshakePool(ssl_t *ssl)
{
#  if defined(USE_SERVER_SIDE_SSL) || defined(REQUIRE_DH_PARAMS)
    unsigned short pubsize, privsize;
#  endif
# if defined(USE_SERVER_SIDE_SSL) && defined(USE_ECC)
    const psEccCurve_t *curve;
# endif
    int32 certOverhead, server = 0;

/*
    Always start with the min pool size and increment for each case
 */
    uint32 size = SSL_HS_POOL_SIZE;
    ...
    certOverhead = SSL_CERTIFICATE_MSG_OVERHEAD;

    switch (ssl->cipher->type)
    {
    case CS_RSA:
        if (server)
        {
            ...
            {
                /* Client auth srv must parse the incoming client certs */
                size += certOverhead;
            }
        }
        else
        {
            /* Client must parse the incoming server certs */
            size += certOverhead;
        }
        break;
    case CS_ECDHE_ECDSA:
    case CS_ECDHE_RSA:
        ...
        {
#  ifdef USE_SERVER_SIDE_SSL
            /* Base size on largest possible curve that we agreed to */
#   ifdef USE_ECC
            if (ssl->ecInfo.ecFlags)
            {
                /* User runtime config */
                if (ssl->ecInfo.ecFlags & IS_SECP521R1 ||
                    ssl->ecInfo.ecFlags & IS_BRAIN512R1)
                {
                    privsize = 132;
                }
                else if (ssl->ecInfo.ecFlags & IS_SECP384R1 ||
                         ssl->ecInfo.ecFlags & IS_BRAIN384R1)
                {
                    privsize = 96;
                }
                else if (ssl->ecInfo.ecFlags & IS_SECP256R1 ||
                         ssl->ecInfo.ecFlags & IS_BRAIN256R1)
                {
                    privsize = 64;
                }
                else if (ssl->ecInfo.ecFlags & IS_SECP224R1 ||
                         ssl->ecInfo.ecFlags & IS_BRAIN224R1)
                {
                    privsize = 56;
                }
                else if (ssl->ecInfo.ecFlags & IS_SECP192R1)
                {
                    privsize = 48;
                }
                else
            ...
            {
                /* Compile time config */
                if (getEccParamByName("ECC-521", &curve) == 0)
                {
                    privsize = 132;
                }
                else if (getEccParamByName("BP-521", &curve) == 0)
                {
                    privsize = 132;
                }
                else if (getEccParamByName("ECC-384", &curve) == 0)
                {
                    privsize = 96;
                }
                else if (getEccParamByName("BP-384", &curve) == 0)
                {
                    privsize = 96;
                }
                else if (getEccParamByName("ECC-256", &curve) == 0)
                {
                    privsize = 64;
                }
                else if (getEccParamByName("BP-256", &curve) == 0)
                {
                    privsize = 64;
                }
                else if (getEccParamByName("ECC-224", &curve) == 0)
                {
                    privsize = 56;
                }
                else if (getEccParamByName("BP-224", &curve) == 0)
                {
                    privsize = 56;
                }
                else if (getEccParamByName("ECC-192", &curve) == 0)
                {
                    privsize = 48;
                }
                else
            ...
            if (psEstimatePkiPoolSize(privsize, PS_ECC, &pubsize, &privsize) < 0)
            {
                return NULL;
            }
            size += privsize;
...
#  endif
    ...
    case CS_ECDH_RSA:
        ...
        {
#  ifdef USE_SERVER_SIDE_SSL
            /* Server does a GenSharedSecret on a known size */
            if (psEstimatePkiPoolSize(ssl->keys->privKey.keysize,
                    ssl->keys->privKey.type, &pubsize, &privsize) < 0)
            {
                return NULL;
            }
            size += privsize;
...
#  ifdef REQUIRE_DH_PARAMS
    case CS_DHE_RSA:
        ...
        {
/*
                DHE suites will generate keys in the handshake pool and
                we know the keysize from the start
 */
            if (psEstimatePkiPoolSize(ssl->keys->dhParams.size, PS_DH,
                    &pubsize, &privsize) < 0)
            {
                return NULL;
            }
            ...
            if (ssl->flags & SSL_FLAGS_CLIENT_AUTH)
            {
                /* Client auth server must parse the incoming client certs */
                size += certOverhead;
            }
        }
        else
...
/*
                ...
                The client will also need to generate DH keys but there
                is no knowlege of the keysize for smart pool creation here.
                The client will create a dhKeyPool at SERVER_KEY_EXCHANGE
                time when more is known
 */
            size += certOverhead;
        }
        break;
 ...
 */
        ...
        {
            /* Clients really shouldn't need more memory but the
                pstm_to_unsigned_bin function does a copy for each
                DH key int so this is why we must add */
            size += 2048;
        }
        break;
#  endif /* REQUIRE_DH_PARAMS */
    ...
    case CS_PSK:
        size += 0;     /* Even SSL_HS_POOL_SIZE is too much */
        break;
    default:
        size = SSL_HS_POOL_MAX;
        break;
    }
    return psOpenPool(HANDSHAKE_POOL, size, 0, NULL, ssl->memAllocPtr);
}

/******************************************************************************/
void matrixSslCloseHandshakePool(psPool_t *pool)
{
    if (pool != NULL)
    {
        psClosePool(pool);
