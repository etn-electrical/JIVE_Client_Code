...
/******************************************************************************/
/**
    EAP-FAST TLS modifications.
    @security This RFC modifies aspects of the TLS protocol (including
    SessionTicket processing and master secret derivation). It should only
    be enabled if explicit support for this EAP method is required.
    @see https://tools.ietf.org/html/rfc4851 The Flexible Authentication via
        Secure Tunneling Extensible Authentication Protocol Method (EAP-FAST)
    @see https://tools.ietf.org/html/rfc5422 Dynamic Provisioning Using
        Flexible Authentication via Secure Tunneling Extensible Authentication
        Protocol (EAP-FAST)
 */
/* #define USE_EAP_FAST / **< @security OFF * / */
...
/******************************************************************************/
/*
    MatrixSSL memory pools
 */
extern psPool_t *matrixSslGetHandshakePool(ssl_t *ssl);
extern psPool_t *matrixSslGetSessionPool(void *userPtr);
...
/*
    Default pool sizes
 */
#  define SSL_KEY_POOL_SIZE   (64 * 1024 * 1024 + sizeof(psPool_t))
#  define SSL_HS_POOL_SIZE    (1 * 1024 + sizeof(psPool_t))
#  define SSL_HS_POOL_MAX     (24 * 1024 + sizeof(psPool_t))/*unknown cipher*/

/*
    The CERTIFICATE message represents the largest unknown size of the SSL
    protocol.  This is mainly due to the fact that there is no limit to how
    large a certificate chain can be.  This value might need to be increased
    if it is known that certificate chains will be used

    This value is added to the Handshake Pool when necessary
 */
#  define SSL_CERTIFICATE_MSG_OVERHEAD (16 * 1024)
