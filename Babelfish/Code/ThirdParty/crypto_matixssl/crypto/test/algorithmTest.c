...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    pool = psOpenPool("AES", 4 * 1024, 0, NULL, NULL);
# endif /* USE_MATRIX_MEMORY_MANAGEMENTT */
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        if (psEstimatePkiPoolSize(rsa[i].size, PS_RSA, &pubsize, &privsize) < 0)
        {
            return -1;
        }
        /* For this test, we use the pool to hold at most the privkey, pubkey and the
            parsed certificate */
        if ((pool = psOpenPool("RSAENCRYPT", privsize + pubsize + rsa[i].certsize,
                 0, NULL, NULL)) == NULL)
        {
            return -1;
        }
...
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        if (psEstimatePkiPoolSize(rsae3[i].size, PS_RSA, &pubsize, &privsize) < 0)
        {
            return -1;
        }
        if ((pool = psOpenPool("RSASIGN", privsize + pubsize, 0, NULL, NULL)) == NULL)
        {
            return -1;
        }
#  endif   /* USE_MATRIX_MEMORY_MANAGEMENT */
...
#   ifdef USE_MATRIX_MEMORY_MANAGEMENT
    pool = psOpenPool("OAEP_TEST", 8 * 1024, 0, NULL, NULL);
#   endif /* USE_MATRIX_MEMORY_MANAGEMENT       */
...
#    ifdef USE_MATRIX_MEMORY_MANAGEMENT
    pool = psOpenPool("PSS_TEST", 6 * 1024, 0, NULL, NULL);
#    endif /* USE_MATRIX_MEMORY_MANAGEMENT      */
...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
    if ((pool = psOpenPool("ECDH", 16 * 1024, 0, NULL, NULL)) == NULL)
    {
        return PS_FAIL;
    }
# endif
...
# ifdef USE_CERT_GEN
static int32_t parse_write_and_check_ec_key(psPool_t *pool,
        const unsigned char *keyIn,
        psSize_t keyInLen)
{
    int32_t rc = PS_FAILURE;
    psEccKey_t priv;
    const psEccCurve_t *curve = NULL;
    unsigned char *der = NULL, *copyDer = NULL;
    psSize_t derLen, copyDerLen;
    psEccKey_t *copyPriv = NULL;

    /* Parse from DER. */
    rc = psEccParsePrivKey(pool, keyIn, keyInLen, &priv, curve);
    if (rc < 0)
    {
        _psTrace("ParsePrivKey failed.");
        goto fail;
    ...
    /* Export back to DER. */
    rc = psEccWritePrivKeyMem(pool, &priv, &der, &derLen);
    if (rc < 0)
    {
        _psTrace("psEccWritePrivKeyMem failed.");
        goto fail;
    }

    /* Check that the exported DER matches the original. */
    if (derLen != keyInLen || Memcmp(keyIn, der, derLen))
    {
        _psTrace("psEccWritePrivKeyMem result does not match original.");
        rc = PS_FAILURE;
# ifdef DEBUG_ECC_KEY_INPUT_OUTPUT_TEST
        psTraceBytes("Key in", keyIn, keyInLen);
        psTraceBytes("Key out", der, derLen);
# endif
        goto fail;
    }

    /* Make a copy and export the copy to DER. */
    rc = psEccNewKey(pool, &copyPriv, curve);
    if (rc != PS_SUCCESS)
    {
        _psTrace("psEccNewKey failed\n");
        goto fail;
    }
    rc = psEccCopyKey(copyPriv, &priv);
    if (rc != PS_SUCCESS)
    {
        _psTrace("psEccCopyKey failed\n");
        goto fail;
    }
    rc = psEccWritePrivKeyMem(pool, copyPriv, &copyDer, &copyDerLen);
    if (rc != PS_SUCCESS)
    {
        _psTrace("psEccWritePrivKeyMem failed\n");
        goto fail;
    }
    if (copyDerLen != keyInLen || Memcmp(keyIn, copyDer, copyDerLen))
    {
       _psTrace("psEccWritePrivKeyMem result of copied key " \
...
fail:
    ...
    psEccClearKey(&priv);
    if (copyPriv != NULL)
        psEccDeleteKey(&copyPriv);

    return rc;
}

static int32_t psEccKeyInputOutputTest(void)
{
    psPool_t *pool = NULL;
    int32_t rc;

    _psTrace("  P-256 key input and output...");
    rc = parse_write_and_check_ec_key(pool, EC256KEY, sizeof(EC256KEY));
    if (rc < 0)
    {
        goto fail;
    }
    ...
    _psTrace("  P-384 key input and output...");
    rc = parse_write_and_check_ec_key(pool, EC384KEY, sizeof(EC384KEY));
    if (rc < 0)
    {
        goto fail;
    }
...
# ifdef USE_SECP521R1
    _psTrace("  P-512 key input and output...");
    rc = parse_write_and_check_ec_key(pool, EC521KEY, sizeof(EC521KEY));
    if (rc < 0)
    {
        goto fail;
    }
...
static int32_t psEccKeyGenerateWriteCopy(void)
{
    psPool_t *pool = NULL;
    int32_t rc;
    psEccKey_t *key1 = NULL, *key2 = NULL;
    const psEccCurve_t *curve;
    unsigned char *key2_der;
    psSize_t key2_der_len;

    _psTrace("  P-256 keygen, copy and encode...");

    rc = getEccParamByName("secp256r1", &curve);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }

    rc = psEccNewKey(pool, &key1, curve);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }

    rc = psEccNewKey(pool, &key2, curve);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }

    rc = psEccGenKey(pool, key1, curve, NULL);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }

    rc = psEccCopyKey(key2, key1);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }

    rc = psEccWritePrivKeyMem(pool, key2, &key2_der, &key2_der_len);
    if (rc != PS_SUCCESS)
    {
        goto fail;
    }
...
# ifdef USE_CERT_GEN
    rc = psEccKeyInputOutputTest();
    if (rc != PS_SUCCESS)
    {
        return rc;
    }

    rc = psEccKeyGenerateWriteCopy();
    if (rc != PS_SUCCESS)
    {
        return rc;
    }
