...
# ifdef USE_MATRIX_MEMORY_MANAGEMENT
        /* Each key uses a 'misc' pool to keep the measurement pools clean */
        misc = psOpenPool("misc", keys[i].poolMisc, 0, NULL, NULL);
# endif
        _psTraceStr("Test %s...\n", keys[i].name);
# ifdef STATS
...
# ifdef SIGN_OP
        iter = 0;
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        pool = psOpenPool(keys[i].name, keys[i].poolSign, 0, NULL, NULL);
#  endif
        psGetTime(&start, NULL);
...
# ifdef VERIFY_OP
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        pool = psOpenPool(keys[i].name, keys[i].poolVerify, 0, NULL, NULL);
#  endif
...
# ifdef ENCRYPT_OP
        iter = 0;
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        pool = psOpenPool(keys[i].name, keys[i].poolEncrypt, 0, NULL, NULL);
#  endif
        psGetEntropy(in, keysize, NULL);
        psGetTime(&start, NULL);
        while (iter < keys[i].iter)
...
# ifdef DECRYPT_OP
#  ifdef USE_MATRIX_MEMORY_MANAGEMENT
        pool = psOpenPool(keys[i].name, keys[i].poolDecrypt, 0, NULL, NULL);
#  endif
/**/
        if (in == out)
        {
