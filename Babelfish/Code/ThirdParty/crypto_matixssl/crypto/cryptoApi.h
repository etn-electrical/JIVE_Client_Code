...
/******************************************************************************/
#   ifdef USE_AES_CMAC
PSPUBLIC int32_t psCmacGenerate(psPool_t *pool,
                                const unsigned char *key, psSize_t keyLen,
                                const unsigned char *data, uint32_t dataLen,
                                unsigned char *mac);
PSPUBLIC int32_t psCmacInit(const unsigned char *key, psSize_t keyLen,
                            psAesCmac_t *aesCmacCtx);
PSPUBLIC void psCmacUpdate(psAesCmac_t *aesCmacCtx,
                           const unsigned char *data, uint32_t dataLen);
PSPUBLIC void psCmacFinal(psAesCmac_t *aesCmacCtx, unsigned char *hash);
#   endif

#   ifdef USE_AES_WRAP
PSPUBLIC int32_t psAesWrap(const unsigned char *kek, psSize_t kekLen,
                           const unsigned char *pt, uint32_t ptLen,
                           unsigned char *ct, uint32_t *ctLen);
PSPUBLIC int32_t psAesUnwrap(const unsigned char *kek, psSize_t kekLen,
                             const unsigned char *ct, uint32_t ctLen,
                             unsigned char *pt, uint32_t *ptLen);
#   endif /* USE_AES_WRAP */
# endif   /* USE_AES */
...
PSPUBLIC int32_t psEstimatePkiPoolSize(psSize_t keylen, uint8_t type,
                                       unsigned short *pubSize, unsigned short *privSize);
# endif
...
#   ifdef USE_AES_CMAC
#    define matrixCmacGenerate psCmacGenerate
#    define matrixCmacInit psCmacInit
#    define matrixCmacUpdate psCmacUpdate
#    define matrixCmacFinal psCmacFinal
#   endif /* USE_AES_CMAC */
