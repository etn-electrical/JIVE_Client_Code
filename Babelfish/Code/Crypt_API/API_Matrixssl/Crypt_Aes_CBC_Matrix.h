/**
 *****************************************************************************************
 *	@file Crypt_Aes_CBC_Matrix.h
 *
 *	@brief Class for AES cryptography operations using matrix-ssl library
 *
 *	@details Defines the interface for performing AES encryption with CBC mode of operation.
...
#ifndef CRYPT_AES_CBC_MATRIX_H
#define CRYPT_AES_CBC_MATRIX_H
 ...
 */
#define CRYPT_AES_CBC_BLOCK_LENGTH ( 16U )

/**
 * @brief mask to validate for minimum block size
 */
#define CRYPT_AES_CBC_BLOCK_LENGTH_MASK ( CRYPT_AES_CBC_BLOCK_LENGTH - 1U )
 ...
 */
#define MIN_U8_ENCODED_AES_CBC_STRING_SIZE( input ) ( CRYPT_AES_CBC_BLOCK_LENGTH * \
													  ( ( CRYPT_ENCODED_LENGTH_STORAGE_SIZE + \
														  CRYPT_ENCODED_CHECKSUM_SIZE + input + \
														  CRYPT_AES_CBC_BLOCK_LENGTH_MASK ) / \
														CRYPT_AES_CBC_BLOCK_LENGTH ) )
 ...
 ****************************************************************************************
 */
class Crypt_Aes_CBC_Matrix : public Crypt
{
	public:
		 ...
		 * @param[in] iv        128 bit initialization vector( used as Salt for first block of
		 * operation),
		 *                      A constant and random 16 byte array
		 * @param[in] iv_size   size of iv array of type aes_iv_size_t
		 */
		Crypt_Aes_CBC_Matrix( uint8_t const* key, aes_key_size_t key_size, uint8_t const* iv,
							  aes_iv_size_t iv_size );

		/**
		 *  @brief Destructor
		 *  @details It will get called when object of Crypt_Aes_CBC_Matrix goes out of scope or deleted.
		 *  @return None
		 */
		~Crypt_Aes_CBC_Matrix( void );
		 ...
		 * MIN_U8_ENCODED_AES_CBC_STRING_SIZE.
	...
	private:
		...
		Crypt_Aes_CBC_Matrix( const Crypt_Aes_CBC_Matrix& rhs );
		Crypt_Aes_CBC_Matrix & operator =( const Crypt_Aes_CBC_Matrix& object );
		...
		static const uint8_t BLANK_ENC_STRING_LEN = CRYPT_AES_CBC_BLOCK_LENGTH_MASK;
		static const uint8_t BLANK_ENC_STRING[BLANK_ENC_STRING_LEN];
};

#endif	// CRYPT_AES_CBC_MATRIX_H
