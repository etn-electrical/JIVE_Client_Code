 ...
 *****************************************************************************************
 */
Image_Ctrl::Image_Ctrl( NV_Ctrl* scratch_nv_ctrl, NV_Ctrl* real_nv_ctrl,
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Get_Image_Specs( image_info_resp_t* image_resp )
{
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Status( image_status_resp_t* status_resp )
{
...
fus_op_status_t Image_Ctrl::Write( image_write_req_t* write_req,
				...
				if ( m_image.data != nullptr )
				{
					...
					if ( m_is_scratch_erased == false )
					{
						NV_Ctrl::nv_status_t nv_status = m_image.untrusted_nv_ctrl->Erase_All(
							Image_Ctrl::Image_Status_Cb_Static, this );
					...
					else
					{
						NV_Ctrl::nv_status_t status =
							m_image.untrusted_nv_ctrl->Write( ( const_cast<uint8_t*>( m_image.data ) ),
															  ...
															  Image_Ctrl::Image_Status_Cb_Static,
															  this );
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Validate( image_validate_req_t* validate_req, image_validate_resp_t* validate_resp )
{
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Commit( image_commit_resp_t* commit_resp )
{
	m_image.status = FUS_PENDING_CALL_BACK;
	NV_Ctrl::mem_range_info_t trusted_mem_range_info = {0};
	NV_Ctrl::mem_range_info_t untrusted_mem_range_info = {0};
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Exit_FUM( void )
{
	m_image.status = FUS_NO_ERROR;
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Commit_Handler( void )
{
	if ( m_image.status != FUS_COMMIT_DONE )
	{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_Read_Header_Handler( void )
{
	NV_Ctrl::mem_range_info_t untrusted_mem_range_info = {0};
...
void Image_Ctrl::Validate_Extract_Header_Handler( void )
{
	...
	m_image.untrusted_nv_ctrl->Get_Memory_Info( &untrusted_mem_range_info );
	m_image.address = untrusted_mem_range_info.start_address;
	...
	{
		m_image.fw_length = header->f.data_length - ( sizeof ( header->fw_crc ) );
		...
		FUS_DEBUG_PRINT( DBG_MSG_INFO, "Firmware_version :  %d.%d.%d ", new_firm_rev->major, new_firm_rev->minor,
						 new_firm_rev->build );
	}
}
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_Read_Data_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_CRC_Calculation_Handler()
{
...
void Image_Ctrl::Validate_CRC_Data_Handler( void )
{
	NV_Ctrl::mem_range_info_t untrusted_mem_range_info = {0};
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_Code_Signing_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Validate_Product_Info_Handler( void )
{
 ...
 *****************************************************************************************
 */
bool Image_Ctrl::Check_Rollback_Enabled( void )
{
 ...
 *****************************************************************************************
 */
bool Image_Ctrl::Check_Version( void )
{
	bool status = false;
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Reset_Handler( void )
{
	m_image.state = IDLE;
	m_image.address = 0U;
	m_image.length = 0U;
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Write_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Erase_Handler( void )
{
	NV_Ctrl::mem_range_info_t untrusted_mem_range_info = {0};

	m_image.untrusted_nv_ctrl->Get_Memory_Info( &untrusted_mem_range_info );

	NV_Ctrl::nv_status_t status = m_image.untrusted_nv_ctrl->Erase_All( Image_Ctrl::Image_Status_Cb_Static, this );
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Retry_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Error_Handler( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Image_Status_Cb( NV_Ctrl::nv_status_t status )
{
	switch ( status )
	{
 ...
 *****************************************************************************************
 */
fus_op_status_t Image_Ctrl::Check_Range( uint32_t address, uint32_t length,
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Update_Version( void )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Image_Upgrade_Task( CR_Tasker* cr_task )
{
 ...
 *****************************************************************************************
 */
bool Image_Ctrl::New_Request( uint32_t address, uint8_t length )
{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Set_Validate_Time( uint32_t time_msec )
{
	if ( time_msec != 0U )
	{
 ...
 *****************************************************************************************
 */
void Image_Ctrl::Set_Commit_Time( uint32_t time_msec )
{
	if ( time_msec != 0U )
	{
