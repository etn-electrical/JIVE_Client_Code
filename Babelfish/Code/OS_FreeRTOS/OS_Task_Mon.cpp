...
{
	// Commented the initialization of OS Task Monitor as this not fully tested with new OS_FreeRTOS
...
{
	if ( m_fragile_task != nullptr )
	{
		return ( m_fragile_task->Get_Task_Name() );
	}
	else
	{
		return ( reinterpret_cast<uint8_t const*>( "Not_Available" ) );
	}
}
...
void OS_Task_Mon::Tasker_Stats( uint32_t* priority_run_times, uint32_t& total_time )
{
		  ...
		  priority < OS_TASK_PRIORITY_MAX; priority++ )
	{
		...
		// DO NOT UNCOMMENT THIS
		total_time += 0U;
		priority_run_times[priority] = 0U;
